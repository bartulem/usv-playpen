...
``infer-qlvm-latents``
``infer-qlvm-latents`` embeds a session's spectrograms into the trained QLVM toroidal latent space (loading the ``qmc_decoder_weights.npz`` written by ``train-qlvm``) and merges the torus coordinates + watershed categories into ``usv_summary.csv``.

.. code-block:: text

    usage: infer-qlvm-latents [-h] --root-directory PATH
                            [--weights-npz-path PATH] [--reference-arrays-npz-path PATH]
    ...
    optional arguments:
      ...
      (full parameters under analyses_settings.json -> "infer_qlvm_latents")
