...
.. code-block:: text
    ...
    optional arguments:
      ...
      --masking-type        Apply SAM mask regions from the mask/<session> groups before embedding ("sam", matching how the decoder was trained) or embed raw spectrograms ("none"). With "sam", USVs without a mask get null qlvm_* columns and a session without a mask group raises.
