...
- ``<spectrograms_dir>/qlvm/arrays_{coarse,fine}.npz`` — the QLVM analysis arrays, used ONLY for the ``heatmap`` background, ``ws_labels_periodic`` cluster contours, and ``centers`` (the cluster peaks / path waypoints); the **QLVM clustering type borders** selector picks which one. For sessions embedded with a QLVM model package cell (``qlvm_model`` names it), write that cell's arrays with ``export-qlvm-reference-arrays`` into the ``qlvm`` folder of a separate ``spectrograms_dir``: the reference arrays belong to a different model, and their contours do not match the package cell's coordinates.
