...
* **masking_type** : ``"sam"`` (default) masks each spectrogram by the union of its SAM regions before embedding, matching how the decoder was trained by *Build QLVM training set*; ``"none"`` embeds raw spectrograms (must match training). Whenever the decoder needs masks — ``"sam"``, a mean-frequency conditional cell, or a training contract with ``require_mask`` true (every v2 package cell) — USVs without a mask instance get null ``qlvm_*`` columns rather than being embedded unmasked, and a session H5 without a ``mask/<session>`` group raises
