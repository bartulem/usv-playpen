...
def choose_animal_colors(exp_info_dict: Optional[Dict] = None,
                         visualizations_parameter_dict: Optional[Dict] = None) -> Optional[List]:
    """
    ...
    for sex_idx, sex in enumerate(exp_info_dict['mouse_sex']):
        if sex == "male":
            ...
            mouse_colors.append(visualizations_parameter_dict["female_colors"][n_females])
            n_females += 1

    return mouse_colors


def luminance_equalizer(color_start: Optional[Tuple] = None,
                        color_end: Optional[Tuple] = None,
                        luminance: Optional[bool] = False,
                        match_by: Optional[str] = None,
                        saturation: Optional[bool] = False) -> Optional[Tuple]:
    """
    Description
    ----------
    ...
    hls_start = colorsys.rgb_to_hls(r=color_start[0] / 255., g=color_start[1] / 255., b=color_start[2] / 255.)
    hls_end = colorsys.rgb_to_hls(r=color_end[0] / 255., g=color_end[1] / 255., b=color_end[2] / 255.)

    # match luminance
    if luminance is True or type(luminance) == float:
        if match_by == 'max':
            luminance_start, luminance_end = np.repeat(np.max([hls_start[1], hls_end[1]]), 2)
        elif match_by == 'min':
            luminance_start, luminance_end = np.repeat(np.min([hls_start[1], hls_end[1]]), 2)
        elif match_by == 'mean':
            luminance_start, luminance_end = np.repeat(np.mean([hls_start[1], hls_end[1]]), 2)
        elif match_by == 'set':
    ...
    # convert back to RGB
    color_start = tuple(item * 255. for item in colorsys.hls_to_rgb(hls_start[0], luminance_start, saturation_start))
    color_end = tuple(item * 255. for item in colorsys.hls_to_rgb(hls_end[0], luminance_end, saturation_end))

    return color_start, color_end


def create_colormap(input_parameter_dict: Optional[Dict] = None) -> ListedColormap:
    """
    Description
    ----------
    ...
    if input_parameter_dict is None:
        with open('../input_parameters.json', 'r') as json_file:
            input_parameter_dict = json.load(json_file)['auxiliary_plot_functions']['create_colormap']

    # change luminance|saturation
    if input_parameter_dict["equalize_luminance"] is True \
        ...
        if input_parameter_dict["cm_type"] == 'diverging':
            input_parameter_dict["cm_start"], input_parameter_dict["cm_start_div"] = luminance_equalizer(tuple(input_parameter_dict["cm_start"]),
        ...
        elif input_parameter_dict["cm_type"] == 'sequential' and \
                tuple(input_parameter_dict["cm_end"]) != (255, 255, 255):
            input_parameter_dict["cm_start"], input_parameter_dict["cm_end"] = luminance_equalizer(tuple(input_parameter_dict["cm_start"]),
    ...
    if input_parameter_dict["cm_type"] == 'sequential':
        for rgb in range(3):
            cm_values[:, rgb] = np.linspace(input_parameter_dict["cm_end"][rgb] / input_parameter_dict["cm_length"],
                                            input_parameter_dict["cm_start"][rgb] / input_parameter_dict["cm_length"],
                                            input_parameter_dict["cm_length"])
    elif input_parameter_dict["cm_type"] == 'diverging':
        for rgb in range(3):
            cm_values[:input_parameter_dict["cm_length"] // 2 + 1, rgb] = np.linspace(input_parameter_dict["cm_start"][rgb] / input_parameter_dict["cm_length"],
                                                                                      input_parameter_dict["cm_end"][rgb] / input_parameter_dict["cm_length"],
                                                                                      input_parameter_dict["cm_length"] // 2 + 1)
            cm_values[input_parameter_dict["cm_length"] // 2:, rgb] = np.flip(np.linspace(input_parameter_dict["cm_start_div"][rgb] / input_parameter_dict["cm_length"],
                                                                                          input_parameter_dict["cm_end"][rgb] / input_parameter_dict["cm_length"],
                                                                                          input_parameter_dict["cm_length"] // 2 + 1))
    else:
        print("Do not recognize cm_type, try again!")
        sys.exit()
    cm_values[:, 3] = np.ones(input_parameter_dict["cm_length"]) * input_parameter_dict["cm_opacity"]
