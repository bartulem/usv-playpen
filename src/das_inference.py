...
class FindMouseVocalizations:
    ...
    def summarize_das_findings(self) -> None:
        ...
        try:
            ...
            len_win_signal = self.input_parameter_dict['summarize_das_findings']['len_win_signal']
            low_freq_cutoff = self.input_parameter_dict['summarize_das_findings']['low_freq_cutoff']
            ...
            for index, row in tqdm(usv_summary.iterrows(), desc="Computing spectrogram correlations/variance in progress...", total=usv_summary.shape[0], position=0, leave=True):
                ...
                condition_0_list[index] = peak_amp_ch not in usv_detected_chs or mean_amp_ch not in usv_detected_chs

                # the following section computes channel-wise signal correlations in the frequency domain
                if len(usv_detected_chs) > 1:
                    spectrogram_data_selected_ch = np.abs(librosa.stft(audio_file_data[start_usv:stop_usv, usv_detected_chs].astype('float32').T, n_fft=len_win_signal))
                    ...
                    spectrogram_data_selected_ch = np.abs(librosa.stft(audio_file_data[start_usv:stop_usv, usv_detected_chs[0]].astype('float32').T, n_fft=len_win_signal)) ** 2
                    signal_variance[index] = np.var(spectrogram_data_selected_ch / np.max(spectrogram_data_selected_ch))

            noise_corr_cutoff = max(float(np.nanpercentile(mean_signal_correlations, q=6)), self.input_parameter_dict['summarize_das_findings']['noise_corr_cutoff_min'])
            noise_var_cutoff = min(float(np.nanpercentile(signal_variance, q=94)), self.input_parameter_dict['summarize_das_findings']['noise_var_cutoff_max'])
            self.message_output(f"Spectrogram correlation cutoff (6th percentile of distribution): {noise_corr_cutoff:.2f}")
            self.message_output(f"Single channel variance cutoff (94th percentile of distribution): {noise_var_cutoff:.4f}")
            drop_counter = 0
            ...
            self.message_output(f"Number of detections dropped due to low signal correlation/variance across channels: {drop_counter}")

            fig, ax = plt.subplots(nrows=1, ncols=2, figsize=(24, 4), dpi=300)
            ax[0].hist(x=mean_signal_correlations[~np.isnan(mean_signal_correlations)],
                    bins=20,
                    histtype='stepfilled',
                    color='#BBD5E8',
                    edgecolor='#000000',
            ...
            ax[1].hist(x=signal_variance[~np.isnan(signal_variance)],
                       bins=20,
                       histtype='stepfilled',
                       color='#BBD5E8',
                       edgecolor='#000000',
            ...
            fig.savefig(fname=f"{self.root_directory}{os.sep}audio{os.sep}{session_id}_usv_signal_correlation_histogram.svg", dpi=300)
            plt.close()

            # give ID number to each USV
            usv_summary['usv_id'] = [f"{_num:04d}" for _num in range(usv_summary.shape[0])]

            self.message_output(f"In this session, {usv_summary.shape[0]} USVs were detected.")
