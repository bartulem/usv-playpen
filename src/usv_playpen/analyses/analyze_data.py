...
@click.option('--model-class', 'model_class', type=click.Choice(['gauss', 't', 'ig'], case_sensitive=False), default=None, required=False, help='Mixture model class: "gauss" (log-Gaussian mixture, classical), "t" (Student-t mixture in log-space; heavy-tailed components absorb the long-pause tail), or "ig" (inverse-Gaussian mixture in linear time; first-passage-time family for waiting times).')
