...
def generate_ratemaps(
    ...
    if space_bool:
        ...
        for i in range(1, np.shape(bin_edges)[0], 1):
            for j in range(1, np.shape(bin_edges)[0], 1):
                ratemap[i - 1, j - 1, 0] = np.sum(
                    (
                        (spike_events_x > bin_edges[i - 1])
                        * (spike_events_x <= bin_edges[i])
                    )
                    * (
                        (spike_events_y > bin_edges[j - 1])
                        * (spike_events_y <= bin_edges[j])
                    )
                )
                ratemap[i - 1, j - 1, 1] = (
                    np.sum(
    ...
    for i in range(1, np.shape(bin_edges)[0], 1):
        ratemap[i - 1, 0] = np.sum(
            (spike_event_features > bin_edges[i - 1])
            * (spike_event_features <= bin_edges[i])
        )
        ratemap[i - 1, 1] = (
    ...
    for sh in range(shuffled_spike_arr.shape[0]):
        spike_features_sh = np.take(a=feature_arr, indices=shuffled_spike_arr[sh, :])
        for i in range(1, np.shape(bin_edges)[0], 1):
            sh_counts[sh, i - 1] = np.sum(
                (spike_features_sh > bin_edges[i - 1])
                * (spike_features_sh <= bin_edges[i])
            )
...
def shuffle_spikes(
    ...
    shuffled_amounts = np.random.randint(
    ...
    shuffled_spike_arr[shuffled_spike_arr >= total_fr_num] = (
        shuffled_spike_arr[shuffled_spike_arr >= total_fr_num] - total_fr_num
    )
    ...
    def calculate_neuronal_tuning_curves(self) -> None:
        ...
        for cluster_file in tqdm(cluster_file_lst):
            ...
            for one_offset in self.tuning_parameters_dict["temporal_offsets"]:
                ...
                space_computed_rm = dict.fromkeys(animal_ids, False)
                for column in behavioral_data.columns:
                    ...
                    elif space_computed_rm[f"{column.split('.')[0]}"] is False:
                        neuronal_tuning_curves_data[file_name_addendum_offset][
                            f"{column.split('.')[0]}.space"
                        ] = {}
                        (
                            neuronal_tuning_curves_data[file_name_addendum_offset][
                                f"{column.split('.')[0]}.space"
