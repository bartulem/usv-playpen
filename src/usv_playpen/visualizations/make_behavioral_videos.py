...
def find_region_by_channel(
    ...
    cluster_ch = int(
        cluster_id[cluster_id.index("_ch") + 3 : cluster_id.index("_ch") + 6]
    )
    for probe_id in brain_area_dict:
...
def load_audio_data(root_directory: str = None) -> tuple[np.ndarray, int]:
    ...
    audio_loc = glob.glob(
        pathname=f"{root_directory}{os.sep}**{os.sep}*_int16.mmap*", recursive=True
    )[0]
...
def plot_mouse_data(
    ...
    for mouse_idx in range(data.shape[1]):
        if history_bool:
            ...
            for hist_point_idx, hist_point in enumerate(
                range(frame_number - history_frame_span, frame_number)
            ):
                plot_axes.plot(
                    [
                        data[
                            hist_point,
                            mouse_idx,
                            animal_node_names.index(history_point),
                            0,
                        ],
                        data[
                            hist_point + 1,
                    ...
                    color=animal_cm[mouse_idx](
                        int(255 * hist_point_idx / history_frame_span)
                    ),
                    ls=history_ls,
                    lw=history_lw,
                )
        ...
        for nc_idx, nc in enumerate(node_connections):
            ...
            plot_axes.plot(
                [
                    data[frame_number, mouse_idx, animal_node_names.index(nc[0]), 0],
                    data[frame_number, mouse_idx, animal_node_names.index(nc[1]), 0],
                ],
                [
                    data[frame_number, mouse_idx, animal_node_names.index(nc[0]), 1],
                    data[frame_number, mouse_idx, animal_node_names.index(nc[1]), 1],
                ],
                [
                    data[frame_number, mouse_idx, animal_node_names.index(nc[0]), 2],
                    data[frame_number, mouse_idx, animal_node_names.index(nc[1]), 2],
                ],
                c=line_color,
                lw=animal_line_width,
            )
        ...
        for npol in node_polygons:
            ...
            for i in range(len(npol)):
                xs[i] = data[
                    frame_number, mouse_idx, animal_node_names.index(npol[i]), 0
                ]
                ys[i] = data[
                    frame_number, mouse_idx, animal_node_names.index(npol[i]), 1
                ]
                zs[i] = data[
                    frame_number, mouse_idx, animal_node_names.index(npol[i]), 2
                ]

            vertices = [list(zip(xs, ys, zs, strict=False))]
            plot_axes.add_collection3d(
                Poly3DCollection(
                    verts=vertices,
                    facecolors=[polygon_color[mouse_idx]],
        ...
        if node_bool:
            plot_axes.scatter(
                data[frame_number, mouse_idx, :, 0],
                data[frame_number, mouse_idx, :, 1],
                data[frame_number, mouse_idx, :, 2],
                c=animal_color[mouse_idx],
...
def plot_speaker_data(
    speaker_data: np.ndarray = None,
    plot_axes: plt.Axes = None,
    frame_number: int = None,
    ...
    plot_axes.scatter(
        speaker_data[frame_number, 0, 0, 0],
...
def plot_spectrogram(
    ...
    usv_segments_ypos: int | float = None,
) -> None:
    """
    Description
    ----------
    ...
    img = librosa.display.specshow(
        data=spectrogram_amplitude[:, spec_start:spec_end],
        sr=audio_sr,
        hop_length=stft_hop,
        y_axis="linear",
        x_axis="time",
    ...
    plot_axes.axvline(x=half_window_size_sec, color="#FFFFFF", linewidth=0.25)
    if plot_usv_segments_bool:
        for idx_usv_, usv_ in enumerate(usv_segments_list):
            plot_axes.axhline(
                y=usv_segments_ypos,
                xmin=(
                    (max(-half_window_size_sec, usv_[0]) - (-half_window_size_sec))
                    / (half_window_size_sec - (-half_window_size_sec))
                ),
                xmax=(
                    (min(half_window_size_sec, usv_[1]) - (-half_window_size_sec))
                    / (half_window_size_sec - (-half_window_size_sec))
                ),
    ...
    plot_axes.tick_params(
        axis="both", which="major", colors=color_mode_preferences["tick_color"]
    )
    ...
    if cbar_bool:
        cbar = figure_object.colorbar(
            mappable=img, ax=plot_axes, format="%+2.0f", pad=0.01
        )
        cbar.set_label(
            label=r"Power (dBFS)",
            labelpad=1,
        ...
        cbar.ax.tick_params(axis="both", which="both", length=0, pad=0.5)
        plt.setp(
...
def plot_raster(
    ...
    plot_axes.axvline(x=0, color="#FFFFFF", linewidth=0.05)
    plot_axes.eventplot(
        positions=raster_data,
    ...
    plot_axes.set_ylim(-0.5, unit_num + 0.5)
    ...
    for probe_id in raster_brain_area:
        for brain_region in raster_brain_area[probe_id].keys():
            if (
                len(filtered_brain_areas) == 0 or brain_region in filtered_brain_areas
            ) and brain_region not in brain_area_already_plotted:
                txt = plot_axes.text(
                    x=txt_x_start,
                    y=1.01,
                    s=brain_region,
                ...
                txt_x_start = (
                    txt.get_window_extent(renderer=fig_renderer)
                    .transformed(plot_axes.transAxes.inverted())
                    .x1
                    + 0.01
                )
...
def plot_behavioral_features(
    ...
    for feature_idx, feature_name in enumerate(beh_features_to_plot):
        ...
        plot_axes[ax_num] = figure_object.add_axes(
            [
                beh_features_fig_position[0],
        ...
        plot_axes[ax_num].plot(
            range(beh_window_size_frames),
            beh_feature_data[
                feature_ts_fr_start:feature_ts_fr_end,
        ...
        plot_axes[ax_num].plot(
            beh_half_window_size_frames,
            beh_feature_data[
                feature_ts_fr_middle, beh_feature_data.columns.index(feature_name)
            ],
            marker="o",
            markersize=2,
            color=feature_color,
        ...
        if feature_idx == len(beh_features_to_plot) - 1:
            plot_axes[ax_num].set_xticks(
                ticks=[x_axis_start, x_axis_middle, x_axis_end]
            )
            plot_axes[ax_num].set_xticklabels(
                labels=[-beh_window_size_sec, 0, beh_window_size_sec]
            )
        else:
            plot_axes[ax_num].set_xticks([])
        plot_axes[ax_num].set_ylim(
            ylim_dict[feature_name.split(".")[1]][0],
        ...
        plot_axes[ax_num].set_yticklabels(
            labels=[
                f"{ylim_dict[feature_name.split('.')[1]][0]:.2f}",
        ...
        if feature_idx == len(beh_features_to_plot) - 1:
            plot_axes[ax_num].spines["bottom"].set_position(("axes", -0.35))
            ...
            plot_axes[ax_num].xaxis.set_label_coords(0.5, -0.8)
        else:
        ...
        plot_axes[ax_num].set_ylabel(
            ylabel=ylabel_for_feature[: ylabel_for_feature.index("(")]
            + "\n"
        ...
        plot_axes[ax_num].yaxis.set_label_coords(-0.175, 0.25)


def plot_arena_corners_mics(
    data: np.ndarray = None,
    plot_axes: plt.Axes = None,
    frame_number: int = None,
    session_id: str = None,
    ...
    if plot_corners_bool:
        plot_axes.scatter(
            data[0, 0, arena_node_names.index("North"), 0],
            data[0, 0, arena_node_names.index("North"), 1],
            data[0, 0, arena_node_names.index("North"), 2],
            c="red",
            s=corner_size,
            alpha=corner_opacity,
        )
        plot_axes.scatter(
            data[0, 0, arena_node_names.index("West"), 0],
            data[0, 0, arena_node_names.index("West"), 1],
            data[0, 0, arena_node_names.index("West"), 2],
            c="yellow",
            s=corner_size,
            alpha=corner_opacity,
        )
        plot_axes.scatter(
            data[0, 0, arena_node_names.index("South"), 0],
            data[0, 0, arena_node_names.index("South"), 1],
            data[0, 0, arena_node_names.index("South"), 2],
            c="green",
            s=corner_size,
            alpha=corner_opacity,
        )
        plot_axes.scatter(
            data[0, 0, arena_node_names.index("East"), 0],
            data[0, 0, arena_node_names.index("East"), 1],
            data[0, 0, arena_node_names.index("East"), 2],
            c="blue",
            s=corner_size,
            alpha=corner_opacity,
        )
    ...
    plot_axes.plot(
        ...
        c=color_mode_preferences["arena_line_color"],
        lw=arena_axes_lw,
    )
    ...
    plot_axes.plot(
        ...
        [data[0, 0, arena_node_names.index("North"), 2], 0.25],
        c=color_mode_preferences["arena_line_color"],
        lw=arena_axes_lw,
    )
    ...
    plot_axes.plot(
        ...
        [data[0, 0, arena_node_names.index("West"), 2], 0.25],
        c=color_mode_preferences["arena_line_color"],
        lw=arena_axes_lw,
    )
    ...
    plot_axes.plot(
        ...
        [data[0, 0, arena_node_names.index("South"), 2], 0.25],
        c=color_mode_preferences["arena_line_color"],
        lw=arena_axes_lw,
    )
    ...
    plot_axes.plot(
        ...
        [data[0, 0, arena_node_names.index("East"), 2], 0.25],
        c=color_mode_preferences["arena_line_color"],
        lw=arena_axes_lw,
    )
    ...
    if plot_mesh_walls_bool:
        for wall in ["North-West", "North-East", "South-West", "South-East"]:
            ...
            mesh_wall = [
                [
                    data[0, 0, arena_node_names.index(wall[0]), :],
                    np.array(
                        [
                            data[0, 0, arena_node_names.index(wall[0]), 0],
                            data[0, 0, arena_node_names.index(wall[0]), 1],
                            0.25,
                        ]
                    ),
                    np.array(
                        [
                            data[0, 0, arena_node_names.index(wall[1]), 0],
                            data[0, 0, arena_node_names.index(wall[1]), 1],
                            0.25,
                        ]
                    ),
            ...
            plot_axes.add_collection3d(
                Poly3DCollection(
                    verts=mesh_wall, facecolors=mesh_color, alpha=mesh_opacity
                )
            )
    ...
    if arena_node_connections_bool:
        for arena_nc in arena_node_connections:
            ...
            plot_axes.plot(
                [
                    data[0, 0, arena_node_names.index(arena_nc[0]), 0],
                    data[0, 0, arena_node_names.index(arena_nc[1]), 0],
                ],
                [
                    data[0, 0, arena_node_names.index(arena_nc[0]), 1],
                    data[0, 0, arena_node_names.index(arena_nc[1]), 1],
                ],
    ...
    plot_axes.text2D(
        ...
        s=f"fr {frame_number:06d} | {frame_number / esr:07.2f}s",
    ...
    for animal_idx, animal_key in enumerate(animal_id.keys()):
        plot_axes.text2D(
            x=text_start_coords[0],
            y=text_start_coords[1]
            - mouse_id_text_offset
            - (animal_idx * main_text_offset),
...
def create_spike_sound_file(
    audio_duration: int | float = None,
    ...
    spike_sound_sr, spike_sound = wavfile.read(
        pathlib.Path(__file__).parent.parent / "_config/spike.wav"
    )

    new_spike_sound_array = np.zeros(
        shape=int(np.floor(spike_sound_sr * audio_duration))
    )

    for event in spike_array:
        sound_start = int(np.floor((event / tracking_esr) * spike_sound_sr))
        new_spike_sound_array[sound_start : sound_start + spike_sound.shape[0]] = (
            spike_sound
        )
    ...
    wavfile.write(
        filename=sound_filename, rate=spike_sound_sr, data=new_spike_sound_array
    )
...
class Create3DVideo:
    ...
    node_connections = [
        "TTI-Haunch_left",
        "TTI-Haunch_right",
        "Shoulder_left-Haunch_left",
        "Shoulder_right-Haunch_right",
        "Nose-Ear_L",
        "Nose-Ear_R",
        "Head-Shoulder_left",
        "Head-Shoulder_right",
    ...
    node_polygons = [
        "Nose-Ear_L-Head",
        "Nose-Ear_R-Head",
        "TTI-Haunch_left-Trunk",
        "TTI-Haunch_right-Trunk",
        "Shoulder_left-Neck-Head",
        "Shoulder_right-Neck-Head",
        "Haunch_left-Trunk-Neck-Shoulder_left",
        "Haunch_right-Trunk-Neck-Shoulder_right",
    ]

    arena_node_connections = [
        "North-ch_9",
        "North-ch_10",
        "North-ch_11",
        "North-ch_12",
    ...
    beh_features_ylabels = {
        ...
        "spaceZ": "Disp(cm)",
        "speed": "Speed(cm/s)",
        "acceleration": "Acc(cm/s²)",
        "neck_elevation": "Elev(cm)",
        "neck_elevation_1st_der": "Elev'(cm/s)",
        "neck_elevation_2nd_der": "Elev''(cm/s)",
        "body_dir": "BodyDir(°)",
        "body_dir_1st_der": "BodyDir'(°/s)",
        "body_dir_2nd_der": "BodyDir''(°/s²)",
        "ego_yaw": "EgoYaw(°)",
        "ego_yaw_1st_der": "EgoYaw'(°/s)",
        "ego_yaw_2nd_der": "EgoYaw''(°/s²)",
        "allo_roll": "Roll(°)",
        "allo_roll_1st_der": "Roll'(°/s)",
        "allo_roll_2nd_der": "Roll''(°/s²)",
        "allo_pitch": "Pitch(°)",
        "allo_pitch_1st_der": "Pitch'(°/s)",
        "allo_pitch_2nd_der": "Pitch''(°/s²)",
        "allo_yaw": "Yaw(°)",
        "allo_yaw_1st_der": "Yaw'(°/s)",
        "allo_yaw_2nd_der": "Yaw''(°/s²)",
        "back_pitch": "BPitch(°)",
        "back_pitch_1st_der": "BPitch'(°/s)",
        "back_pitch_2nd_der": "BPitch''(°/s²)",
        "back_yaw": "BYaw(°)",
        "back_yaw_1st_der": "BYaw'(°/s)",
        "back_yaw_2nd_der": "BYaw''(°/s²)",
        "tail_curvature": "Tail(a.u.)",
        "tail_curvature_1st_der": "Tail'(a.u.)",
        "tail_curvature_2nd_der": "Tail''(a.u.)",
        "nose-nose": "ΔN(cm)",
        "nose-nose_1st_der": "ΔN'(cm/s)",
        "nose-nose_2nd_der": "ΔN''(cm/s²)",
        "TTI-TTI": "ΔT(cm)",
        "TTI-TTI_1st_der": "ΔT'(cm/s)",
        "TTI-TTI_2nd_der": "ΔT''(cm/s²)",
        "nose-TTI": "ΔNT(cm)",
        "nose-TTI_1st_der": "ΔNT'(cm/s)",
        "nose-TTI_2nd_der": "ΔNT''(cm/s²)",
        "TTI-nose": "ΔTN(cm)",
        "TTI-nose_1st_der": "ΔTN'(cm/s)",
        "TTI-nose_2nd_der": "ΔTN''(cm/s²)",
        "neck_elevation_diff": "ΔElev(cm)",
        "neck_elevation_diff_1st_der": "ΔElev'(cm/s)",
        "neck_elevation_diff_2nd_der": "ΔElev''(cm/s²)",
        "speed_diff": "ΔSpeed(cm/s)",
        "speed_diff_1st_der": "ΔSpeed'(cm/s²)",
        "speed_diff_2nd_der": "ΔSpeed''(cm/s³)",
        "allo_yaw-nose": "Yaw-N(°)",
        "allo_yaw-nose_1st_der": "Yaw-N'(°/s)",
        "allo_yaw-nose_2nd_der": "Yaw-N''(°/s²)",
        "nose-allo_yaw": "N-Yaw(°)",
        "nose-allo_yaw_1st_der": "N-Yaw'(°/s)",
        "nose-allo_yaw_2nd_der": "N-Yaw''(°/s²)",
        "allo_yaw-TTI": "Yaw-T(°)",
        "allo_yaw-TTI_1st_der": "Yaw-T'(°/s)",
        "allo_yaw-TTI_2nd_der": "Yaw-T''(°/s²)",
        "TTI-allo_yaw": "T-Yaw(°)",
        "TTI-allo_yaw_1st_der": "T-Yaw'(°/s)",
        "TTI-allo_yaw_2nd_der": "T-Yaw''(°/s²)",
    }
    ...
    def load_h5_file(self) -> tuple:
        ...
        h5_file_arena = glob.glob(
            f"{self.arena_directory}{os.sep}video{os.sep}**{os.sep}*_points3d_translated_rotated_metric.h5"
        )[0]
        h5_file_mouse = glob.glob(
        ...
        with h5py.File(name=h5_file_arena, mode="r") as h5_file_arena_obj:
            ...
            arena_node_names = [
                item.decode("utf-8") for item in list(h5_file_arena_obj["node_names"])
            ]
        with h5py.File(name=h5_file_mouse, mode="r") as h5_file_mouse_obj:
            mouse_tracks = np.array(h5_file_mouse_obj["tracks"])
            mouse_track_names = [
                item.decode("utf-8") for item in list(h5_file_mouse_obj["track_names"])
            ]
            mouse_node_names = [
                item.decode("utf-8") for item in list(h5_file_mouse_obj["node_names"])
            ]
            mouse_experimental_code = h5_file_mouse_obj["experimental_code"][()].decode(
                "utf-8"
            )
        ...
        if self.visualizations_parameter_dict["make_behavioral_videos"]["speaker_bool"]:
            h5_file_speaker = glob.glob(
                f"{self.root_directory}{os.sep}video{os.sep}**{os.sep}speaker*_points3d_translated_rotated_metric.h5"
            )[0]
            with h5py.File(name=h5_file_speaker, mode="r") as h5_file_speaker_obj:
                speaker_tracks = np.array(h5_file_speaker_obj["tracks"])
                speaker_track_name = list(h5_file_speaker_obj["track_names"])[0].decode(
                    "utf-8"
                )
                speaker_node_name = list(h5_file_speaker_obj["node_names"])[0].decode(
                    "utf-8"
                )

            return (
                os.path.dirname(h5_file_arena),
                arena_tracks,
                arena_node_names,
                os.path.dirname(h5_file_mouse),
                mouse_tracks,
                mouse_track_names,
                mouse_node_names,
    ...
    def visualize_in_video(self) -> None:
        ...
        self.message_output(
            f"Creating data visualization started at: {datetime.now().hour:02d}:{datetime.now().minute:02d}.{datetime.now().second:02d}"
        )
        smart_wait(app_context_bool=self.app_context_bool, seconds=1)

        # create save directory (if one doesn't exist already)
        putative_save_directory = (
            f"{self.root_directory}{os.sep}data_animation_examples"
        )
        ...
        experiment_info_dict = extract_information(
            experiment_code=mouse_experimental_code
        )
        animal_id_sex_dict = {
            mouse_name: "$\u2642$" if mouse_sex == "male" else "$\u2640$"
            for mouse_name, mouse_sex in zip(
                mouse_track_names, experiment_info_dict["mouse_sex"], strict=False
            )
        }

        animal_colors = choose_animal_colors(
            exp_info_dict=experiment_info_dict,
            visualizations_parameter_dict=self.visualizations_parameter_dict,
        )
        animal_colors_dict = {
            mouse_name: animal_colors[mouse_idx]
            for mouse_idx, mouse_name in enumerate(mouse_track_names)
        }

        animal_colormaps = []
        for mouse_idx, mouse in enumerate(mouse_track_names):
            if (
                self.visualizations_parameter_dict["make_behavioral_videos"][
                    "plot_theme"
                ]
                == "dark"
            ):
            ...
            animal_colormaps.append(
                create_colormap(
                    input_parameter_dict={
                        ...
                        "cm_type": "sequential",
                        "cm_start": (
                            int(animal_colors[mouse_idx][1:3], 16),
        ...
        if (
            self.visualizations_parameter_dict["make_behavioral_videos"]["view_angle"]
            == "top"
        ):
            view_elevation = 90
            view_azimuth = 0
            ...
            if (
                ...
                or self.visualizations_parameter_dict["make_behavioral_videos"][
                    "beh_features_bool"
                ]
                or self.visualizations_parameter_dict["make_behavioral_videos"][
                    "raster_plot_bool"
            ...
            view_elevation = 45
            view_azimuth = self.visualizations_parameter_dict["make_behavioral_videos"][
                "side_azimuth_start"
            ]
        ...
        if (
            ...
            if self.visualizations_parameter_dict["make_behavioral_videos"][
                "raster_plot_bool"
            ]:
                data_start_index = re.search(
                    f"Data{os.sep}{self.root_directory.split(os.sep)[-1]}",
                    self.root_directory,
                    re.IGNORECASE,
                ).start()
                ...
                with open(
                    glob.glob(
                        pathname=f"{ephys_directory}{os.sep}neuropixels_sites_to_anatomy_converter.json"
                    )[0]
                ) as anatomy_converter_json:
                    neuropixels_sites_to_anatomy_converter = json.load(
                        anatomy_converter_json
                    )
                    ...
                    for one_file in cluster_files:
                        ...
                        if (
                            len(
                                ...
                                ]["raster_selection_criteria"]["other"]
                            )
                            > 0
                        ):
                            select_other_bool = all(
                                keyword in one_cluster_file_name
                                for keyword in self.visualizations_parameter_dict[
                                    "make_behavioral_videos"
                                ]["raster_selection_criteria"]["other"]
                            )
                        if (
                            len(
                                self.visualizations_parameter_dict[
                                    "make_behavioral_videos"
                                ]["raster_selection_criteria"]["brain_areas"]
                            )
                            > 0
                        ):
                            cl_brain_region = find_region_by_channel(
                                cluster_id=one_cluster_file_name,
                                brain_area_dict=neuropixels_sites_to_anatomy_converter[
                                    mouse_track_names[0]
                                ][session_id],
                            ...
                            select_area_bool = any(
                                area_keyword == cl_brain_region
                                for area_keyword in self.visualizations_parameter_dict[
                                    "make_behavioral_videos"
                                ]["raster_selection_criteria"]["brain_areas"]
                            )
                ...
                for cluster_file in cluster_files:
                    cluster_data_dict[os.path.basename(cluster_file)[:-4]] = np.load(
                        file=cluster_file
                    )[1, :]
            ...
            if self.visualizations_parameter_dict["make_behavioral_videos"][
                ...
                beh_half_window_size_frames = int(
                    np.floor(
                        (
                            self.visualizations_parameter_dict[
                                "make_behavioral_videos"
                            ]["subplot_specs"]["beh_features_window_size"]
                            / 2
                        )
                        * empirical_camera_sr
                    )
                ...
                if (
                    frame_start - beh_half_window_size_frames < 0
                    or frame_start + frame_span + beh_half_window_size_frames
                    > mouse_data.shape[0]
                ):
                    self.message_output(
                        "Video start time is either too early or too late for behavioral features."
                    )
                    return
                ...
                if (
                    ...
                    if len(mouse_track_names) == 1:
                        ...
                        for tentative_feature in tentative_features:
                            beh_features_to_plot.append(
                                f"{mouse_track_names[0]}.{tentative_feature}"
                            )
                    else:
                        tentative_features = [
                            "speed",
                        ...
                        for mouse_name in mouse_track_names:
                            for tentative_feature in tentative_features:
                                beh_features_to_plot.append(
                                    f"{mouse_name}.{tentative_feature}"
                                )
                        for tentative_social_feature in tentative_social_features:
                            beh_features_to_plot.append(
                                f"{mouse_track_names[0]}-{mouse_track_names[1]}.{tentative_social_feature}"
                            )
                ...
                beh_feature_data = beh_feature_data[
                    beh_window_start:beh_window_end, beh_features_to_plot
                ]
            ...
            if self.visualizations_parameter_dict["make_behavioral_videos"][
                ...
                # check if video start time and end time is within spectrogram window
                half_window_size_frames = int(
                    np.floor(
                        (
                            self.visualizations_parameter_dict[
                                "make_behavioral_videos"
                            ]["subplot_specs"]["spectrogram_plot_window_size"]
                            / 2
                        )
                        * empirical_camera_sr
                    )
                ...
                if (
                    frame_start - half_window_size_frames < 0
                    or frame_start + frame_span + half_window_size_frames
                    > mouse_data.shape[0]
                ):
                    self.message_output(
                        "Video start time is either too early or too late for spectrogram."
                    )
                    return

                active_mic_position = self.visualizations_parameter_dict[
                    "make_behavioral_videos"
                ]["spectrogram_ch"]

                # load microphone data
                audio_data, audio_sr = load_audio_data(
                ...
                camera_frame_in_samples = int(
                    np.ceil(1 / empirical_camera_sr * audio_sr)
                )

                half_window_size_samples = int(
                    np.floor(half_window_size_sec * audio_sr)
                )
                window_start_signal = (
                    int(
                        self.visualizations_parameter_dict["make_behavioral_videos"][
                ...
                first_fr_spectrogram_center = int(
                    (audio_sr * half_window_size_sec) // stft_hop
                )

                if self.speaker_audio_file != "" and os.path.isfile(
                    self.speaker_audio_file
                ):
                    speaker_audio_sr, speaker_audio_data = wavfile.read(
                        self.speaker_audio_file
                    )
                    raspi_input_loc = glob.glob(
                        pathname=f"{self.root_directory}{os.sep}audio{os.sep}cropped_to_video{os.sep}m_*ch03_*.wav",
                        recursive=True,
                    )[0]
                    raspi_input_mic_sr, raspi_input_mic_data = wavfile.read(
                        raspi_input_loc
                    )
                    ttl_start, ttl_end = read_ttl_events(raspi_input_mic_data)
                    time_correction_coefficient = 20000  # 80 ms
                    window_start_signal = (
                        window_start_signal - ttl_start - time_correction_coefficient
                    )
                    window_end_signal = (
                        window_end_signal - ttl_start - time_correction_coefficient
                    )
                    spectrogram_data = librosa.stft(
                        y=speaker_audio_data[
                            window_start_signal:window_end_signal
                        ].astype(np.float32),
                        n_fft=self.visualizations_parameter_dict[
                            "make_behavioral_videos"
                        ]["subplot_specs"]["spectrogram_stft_nfft"],
                    )
                else:
                    spectrogram_data = librosa.stft(
                        y=audio_data[
                ...
                spectrogram_amplitude = librosa.amplitude_to_db(
                    np.abs(spectrogram_data), ref=np.max(np.abs(spectrogram_data))
                )

                # find USV onset and offset times for epoch of interest
                if (
                    self.speaker_audio_file == ""
                    and not self.visualizations_parameter_dict[
                        "make_behavioral_videos"
                    ]["speaker_bool"]
                ):
                    usv_summary_file = glob.glob(
                        f"{self.root_directory}{os.sep}audio{os.sep}*_usv_summary.csv"
                    )[0]
                    usv_summary_df = pls.read_csv(usv_summary_file)
                    usv_summary_df = usv_summary_df.filter(
                        (
                            pls.col("stop")
                            >= self.visualizations_parameter_dict[
                                "make_behavioral_videos"
        ...
        if (
            self.visualizations_parameter_dict["make_behavioral_videos"]["plot_theme"]
            == "dark"
        ):
        ...
        with warnings.catch_warnings():
            ...
            fig, ax = plt.subplots(
                ...
                nrows=1,
                ncols=3 + n_plot_cols,
                dpi=self.visualizations_parameter_dict["make_behavioral_videos"][
            ...
            plot_arena_corners_mics(
                ...
                arena_node_connections_bool=self.visualizations_parameter_dict[
                    "make_behavioral_videos"
                ]["arena_figure_specs"]["arena_node_connections_bool"],
                arena_node_connections=self.arena_node_connections,
                arena_node_names=arena_node_names,
                arena_axes_lw=self.visualizations_parameter_dict[
                    "make_behavioral_videos"
                ]["arena_figure_specs"]["arena_axes_lw"],
                arena_mics_lw=self.visualizations_parameter_dict[
                    "make_behavioral_videos"
                ]["arena_figure_specs"]["arena_mics_lw"],
                arena_mics_opacity=self.visualizations_parameter_dict[
                    "make_behavioral_videos"
                ]["arena_figure_specs"]["arena_mics_opacity"],
                plot_corners_bool=self.visualizations_parameter_dict[
                    "make_behavioral_videos"
                ]["arena_figure_specs"]["plot_corners_bool"],
                corner_size=self.visualizations_parameter_dict[
                    "make_behavioral_videos"
                ]["arena_figure_specs"]["corner_size"],
                corner_opacity=self.visualizations_parameter_dict[
                    "make_behavioral_videos"
                ]["arena_figure_specs"]["corner_opacity"],
                plot_mesh_walls_bool=self.visualizations_parameter_dict[
                    "make_behavioral_videos"
                ]["arena_figure_specs"]["plot_mesh_walls_bool"],
                mesh_color=color_mode_preferences["arena_mesh_color"],
                mesh_opacity=self.visualizations_parameter_dict[
                    "make_behavioral_videos"
                ]["arena_figure_specs"]["mesh_opacity"],
                active_mic_bool=self.visualizations_parameter_dict[
                    "make_behavioral_videos"
                ]["arena_figure_specs"]["active_mic_bool"],
                active_mic_position=active_mic_position,
                active_mic_color=color_mode_preferences["arena_mic_color"],
                inactive_mic_bool=self.visualizations_parameter_dict[
                    "make_behavioral_videos"
                ]["arena_figure_specs"]["inactive_mic_bool"],
                inactive_mic_color=self.visualizations_parameter_dict[
                    "make_behavioral_videos"
                ]["arena_figure_specs"]["inactive_mic_color"],
                text_start_coords=text_start_coords,
                main_text_offset=main_text_offset,
                mouse_id_text_offset=mouse_id_text_offset,
                text_fontsize=self.visualizations_parameter_dict[
                    "make_behavioral_videos"
                ]["arena_figure_specs"]["text_fontsize"],
            )
            ...
            plot_mouse_data(
                ...
                animal_line_width=self.visualizations_parameter_dict[
                    "make_behavioral_videos"
                ]["mouse_figure_specs"]["node_connection_lw"],
                node_bool=self.visualizations_parameter_dict["make_behavioral_videos"][
                    "mouse_figure_specs"
                ]["node_bool"],
                node_connections=self.node_connections,
                node_polygons=self.node_polygons,
                node_lw=self.visualizations_parameter_dict["make_behavioral_videos"][
                    "mouse_figure_specs"
                ]["node_lw"],
                node_size=self.visualizations_parameter_dict["make_behavioral_videos"][
                    "mouse_figure_specs"
                ]["node_size"],
                node_opacity=self.visualizations_parameter_dict[
                    "make_behavioral_videos"
                ]["mouse_figure_specs"]["node_opacity"],
                node_edge_color=color_mode_preferences["node_edge_color"],
                polygon_color=animal_colors,
                polygon_opacity=self.visualizations_parameter_dict[
                    "make_behavioral_videos"
                ]["mouse_figure_specs"]["body_opacity"],
                body_edge_color=color_mode_preferences["body_edge_color"],
                history_bool=self.visualizations_parameter_dict[
                    "make_behavioral_videos"
                ]["history_bool"],
                history_frame_span=int(
                    np.ceil(
                ...
                history_point=self.visualizations_parameter_dict[
                    "make_behavioral_videos"
                ]["mouse_figure_specs"]["history_point"],
                history_ls=self.visualizations_parameter_dict["make_behavioral_videos"][
                    "mouse_figure_specs"
                ]["history_ls"],
                history_lw=self.visualizations_parameter_dict["make_behavioral_videos"][
                    "mouse_figure_specs"
                ]["history_lw"],
                xlim_=plot_xlim,
                ylim_=plot_ylim,
            ...
            if self.visualizations_parameter_dict["make_behavioral_videos"][
                ...
                plot_speaker_data(
                    ...
                    speaker_alpha=self.visualizations_parameter_dict[
                        "make_behavioral_videos"
                    ]["arena_figure_specs"]["speaker_opacity"],
                )
            ...
            if self.visualizations_parameter_dict["make_behavioral_videos"][
                ...
                if (
                    ...
                    frame_usv_summary_df = usv_summary_df.filter(
                        (
                            pls.col("stop")
                            >= self.visualizations_parameter_dict[
                                "make_behavioral_videos"
                    ...
                    usv_segments_list = [
                        ...
                        for usv_start, usv_stop in zip(
                            frame_usv_summary_df["start"],
                            frame_usv_summary_df["stop"],
                            strict=False,
                        )
                    ]

                    usv_segments_colors = [
                        animal_colors_dict[emitter_id]
                        if emitter_id in animal_colors_dict
                        else "#FFFFFF"
                        for emitter_id in frame_usv_summary_df["emitter"]
                    ]
                else:
                    usv_segments_list = []
                    usv_segments_colors = []
                ...
                spec_end = (first_fr_spectrogram_center * 2) + (
                    relative_fr * spectrogram_step
                )
                ...
                plot_spectrogram(
                    ...
                    stft_hop=stft_hop,
                    half_window_size_sec=half_window_size_sec,
                    cbar_bool=self.visualizations_parameter_dict[
                        "make_behavioral_videos"
                    ]["subplot_specs"]["spectrogram_cbar_bool"],
                    color_mode_preferences=color_mode_preferences,
                    spectrogram_amplitude=spectrogram_amplitude,
                    power_limit=self.visualizations_parameter_dict[
                        "make_behavioral_videos"
                    ]["subplot_specs"]["spectrogram_power_limit"],
                    freq_limit=self.visualizations_parameter_dict[
                        "make_behavioral_videos"
                    ]["subplot_specs"]["spectrogram_frequency_limit"],
                    freq_yticks=self.visualizations_parameter_dict[
                        "make_behavioral_videos"
                    ]["subplot_specs"]["spectrogram_yticks"],
                    plot_usv_segments_bool=plot_usv_segments_bool,
                    usv_segments_list=usv_segments_list,
                    usv_segment_lw=self.visualizations_parameter_dict[
                        "make_behavioral_videos"
                    ]["subplot_specs"]["usv_segments_lw"],
                    usv_segments_ypos=self.visualizations_parameter_dict[
                        "make_behavioral_videos"
                    ]["subplot_specs"]["usv_segments_ypos"],
            ...
            if self.visualizations_parameter_dict["make_behavioral_videos"][
                ...
                if (
                    ...
                    and self.visualizations_parameter_dict["make_behavioral_videos"][
                        "spike_sound_bool"
                    ]
                ):
                    for special_unit in self.visualizations_parameter_dict[
                        "make_behavioral_videos"
                    ]["raster_special_units"]:
                        spikes_for_sound = filter_spikes_for_raster(
                            input_arr=cluster_data_dict[special_unit],
                            ra_st_fr=frame_start,
                            ra_end_fr=frame_start + frame_span,
                            fr_start=frame_start,
                        )
                        create_spike_sound_file(
                            audio_duration=self.visualizations_parameter_dict[
                                "make_behavioral_videos"
                            ]["video_duration"],
                ...
                if (
                    ...
                    event_plot_line_lengths = [
                        self.visualizations_parameter_dict["make_behavioral_videos"][
                            "subplot_specs"
                        ]["raster_ll"]
                    ] * len(cluster_data_dict.keys())
                    event_plot_line_widths = [
                        self.visualizations_parameter_dict["make_behavioral_videos"][
                            "subplot_specs"
                        ]["raster_lw"]
                    ] * len(cluster_data_dict.keys())
                    ...
                    special_unit_ll = max(
                        1, int(np.floor(len(cluster_data_dict.keys()) / 36))
                    )
                    event_plot_line_lengths = [
                        self.visualizations_parameter_dict["make_behavioral_videos"][
                            "subplot_specs"
                        ...
                        if one_unit
                        not in self.visualizations_parameter_dict[
                            "make_behavioral_videos"
                        ]["raster_special_units"]
                        else special_unit_ll
                        for one_unit in cluster_data_dict
                    ]
                    event_plot_line_widths = [
                        self.visualizations_parameter_dict["make_behavioral_videos"][
                            "subplot_specs"
                ...
                for cluster_key in cluster_data_dict:
                    raster_data.append(
                        filter_spikes_for_raster(
                            input_arr=cluster_data_dict[cluster_key],
                    ...
                    if (
                        ...
                        cluster_key
                        in self.visualizations_parameter_dict["make_behavioral_videos"][
                            "raster_special_units"
                        ]
                    ):
                        special_brain_region, special_color = find_region_by_channel(
                            cluster_id=cluster_key,
                            brain_area_dict=neuropixels_sites_to_anatomy_converter[
                                mouse_track_names[0]
                            ][session_id],
                ...
                if (
                    ...
                    self.brain_area_color_scheme = {
                        one_key: (
                            "#B8B2AD"
                            if one_key not in change_brain_area_colors
                            else one_value
                        )
                        for one_key, one_value in self.brain_area_color_scheme.items()
                    }
                ...
                plot_raster(
                    ...
                    raster_brain_area=neuropixels_sites_to_anatomy_converter[
                        mouse_track_names[0]
                    ][session_id],
                    raster_line_lengths=event_plot_line_lengths,
                    raster_line_widths=event_plot_line_widths,
                    filtered_brain_areas=self.visualizations_parameter_dict[
                        "make_behavioral_videos"
                    ]["raster_selection_criteria"]["brain_areas"],
                    color_mode_preferences=color_mode_preferences,
            ...
            if self.visualizations_parameter_dict["make_behavioral_videos"][
                ...
                for feature_name in beh_features_to_plot:
                    ...
                    if feature_name_alone not in ylim_dict:
                        ...
                        ylim_dict[feature_name_alone][0] = min(
                            ylim_dict[feature_name_alone][0], new_min
                        )
                        ylim_dict[feature_name_alone][1] = max(
                ...
                plot_behavioral_features(
                    ...
                    ylim_dict=ylim_dict,
                    plot_theme=self.visualizations_parameter_dict[
                        "make_behavioral_videos"
                    ]["plot_theme"],
                    color_mode_preferences=color_mode_preferences,
                    animal_colors=animal_colors,
            ...
            def animate(frame_num):
                ...
                if (
                    ...
                    azim = (
                        ...
                        * ((frame_num + frame_start) / empirical_camera_sr)
                        + self.visualizations_parameter_dict["make_behavioral_videos"][
                            "side_azimuth_start"
                        ]
                    ) % 360
                ...
                plot_mouse_data(
                    ...
                    node_bool=self.visualizations_parameter_dict[
                        "make_behavioral_videos"
                    ]["mouse_figure_specs"]["node_bool"],
                    node_connections=self.node_connections,
                    node_polygons=self.node_polygons,
                    node_lw=self.visualizations_parameter_dict[
                        "make_behavioral_videos"
                    ]["mouse_figure_specs"]["node_lw"],
                    node_size=self.visualizations_parameter_dict[
                        "make_behavioral_videos"
                    ]["mouse_figure_specs"]["node_size"],
                    node_opacity=self.visualizations_parameter_dict[
                        "make_behavioral_videos"
                    ]["mouse_figure_specs"]["node_opacity"],
                    node_edge_color=color_mode_preferences["node_edge_color"],
                    ...
                    history_frame_span=int(
                        np.ceil(
                            self.visualizations_parameter_dict[
                                "make_behavioral_videos"
                            ]["mouse_figure_specs"]["history_span_sec"]
                            * empirical_camera_sr
                        )
                    ),
                    history_point=self.visualizations_parameter_dict[
                    ...
                    history_ls=self.visualizations_parameter_dict[
                        "make_behavioral_videos"
                    ]["mouse_figure_specs"]["history_ls"],
                    history_lw=self.visualizations_parameter_dict[
                        "make_behavioral_videos"
                    ]["mouse_figure_specs"]["history_lw"],
                    xlim_=plot_xlim,
                    ylim_=plot_ylim,
                    zlim_=plot_zlim,
                )
                ...
                if self.visualizations_parameter_dict["make_behavioral_videos"][
                    ...
                    current_spec_end = (first_fr_spectrogram_center * 2) + (
                        current_relative_fr * spectrogram_step
                    )
                    ...
                    if (
                        ...
                        frame_usv_summary_df_temp = usv_summary_df.filter(
                            (
                                pls.col("stop")
                                >= current_video_time - half_window_size_sec
                            )
                            & (
                                pls.col("start")
                                <= current_video_time + half_window_size_sec
                        ...
                        usv_segments_list_temp = [
                            (
                                usv_start - current_video_time,
                                usv_stop - current_video_time,
                            )
                            for usv_start, usv_stop in zip(
                                frame_usv_summary_df_temp["start"],
                                frame_usv_summary_df_temp["stop"],
                                strict=False,
                            )
                        ]
                        ...
                        usv_segments_colors_temp = [
                            ...
                            for emitter_id in frame_usv_summary_df_temp["emitter"]
                        ]
                    else:
                        usv_segments_list_temp = []
                        usv_segments_colors_temp = []
                ...
                if self.visualizations_parameter_dict["make_behavioral_videos"][
                    ...
                    current_raster_data = []
                    for cluster_name in cluster_data_dict:
                        current_raster_data.append(
                            filter_spikes_for_raster(
                                input_arr=cluster_data_dict[cluster_name],
                ...
                if self.visualizations_parameter_dict["make_behavioral_videos"][
                    ...
                    current_feature_ts_fr_end = (
                        frame_num - frame_start
                    ) + beh_window_size_frames
                    current_feature_ts_fr_middle = (
                        frame_num - frame_start
                    ) + beh_half_window_size_frames
            ...
            if self.visualizations_parameter_dict["make_behavioral_videos"][
                ...
                anima.save(
                    filename=animation_file_path,
                    writer=self.visualizations_parameter_dict["make_behavioral_videos"][
                        "general_figure_specs"
                    ]["animation_writer"],
                ...
                if (
                    ...
                    for unit_id in self.visualizations_parameter_dict[
                        "make_behavioral_videos"
                    ]["raster_special_units"]:
                        audio_file_name = (
                            animation_file_name + f"_spike_sound_{unit_id}.wav"
                        )
                        output_video_name = (
                            animation_file_name
                            + f"_spike_sound_{unit_id}.{self.visualizations_parameter_dict['make_behavioral_videos']['general_figure_specs']['animation_format']}"
                        )
                        subprocess.Popen(
                            f"{self.command_addition}ffmpeg -y -i {animation_file_name}.{self.visualizations_parameter_dict['make_behavioral_videos']['general_figure_specs']['animation_format']} -i {audio_file_name} -c:v copy -c:a aac {output_video_name}",
                ...
                if (
                    ...
                    output_video_name = (
                        animation_file_name
                        + f"_with_USV_sound.{self.visualizations_parameter_dict['make_behavioral_videos']['general_figure_specs']['animation_format']}"
                    )
                    subprocess.Popen(
                        args=f"{self.command_addition}ffmpeg -y -i {animation_file_name}.{self.visualizations_parameter_dict['make_behavioral_videos']['general_figure_specs']['animation_format']} -i {self.visualizations_parameter_dict['make_behavioral_videos']['sequence_audio_file']} -c:v copy -c:a aac {output_video_name}",
                        stdout=subprocess.DEVNULL,
                        stderr=subprocess.STDOUT,
                        cwd=putative_save_directory,
                        shell=self.shell_usage_bool,
                ...
                if self.visualizations_parameter_dict["make_behavioral_videos"][
                    ...
                    fig_loc = (
                        f"{putative_save_directory}{os.sep}{session_id}_3D_{frame_start}fr_{name_addition}."
                        f"{self.visualizations_parameter_dict['make_behavioral_videos']['general_figure_specs']['fig_format']}"
                    )
                    fig.savefig(
                        fig_loc,
                        dpi=self.visualizations_parameter_dict[
                            "make_behavioral_videos"
                        ]["general_figure_specs"]["fig_dpi"],
                    )
                    ...
                    if os_type == "Windows":
                        ...
                        if "DISPLAY" in os.environ:
                            subprocess.run(
                                args=["open", os.path.abspath(fig_loc)], check=True
                            )
                    elif os_type == "Linux":
                        if "DISPLAY" in os.environ:
                            subprocess.run(
                                args=["xdg-open", os.path.abspath(fig_loc)], check=True
                            )
                    else:
                        self.message_output(
                            "Unsupported operating system for opening image."
                        )
