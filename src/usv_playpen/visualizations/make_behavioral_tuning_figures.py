...
class RatemapFigureMaker(FeatureZoo):
    ...
    def neuronal_tuning_figures(self) -> None:
        ...
        astropy_kernel_1d = Gaussian1DKernel(
            stddev=self.visualizations_parameter_dict["neuronal_tuning_figures"][
                "smoothing_sd"
            ]
        )
        astropy_kernel_2d = Gaussian2DKernel(
            x_stddev=self.visualizations_parameter_dict["neuronal_tuning_figures"][
                "smoothing_sd"
            ],
            y_stddev=self.visualizations_parameter_dict["neuronal_tuning_figures"][
                "smoothing_sd"
            ],
        )
                    ...
                    for plot_feature_key in plot_features:
                        ...
                        with warnings.catch_warnings():
                            ...
                            if len(plot_features[plot_feature_key]) > 0:
                                row_num = int(
                                    np.ceil(
                                        (len(plot_features[plot_feature_key]) * 2) / 6
                                    )
                                )
                                fig = plt.figure(
                                    figsize=(6.4, float(row_num)), tight_layout=False
                                )
                                ...
                                for feature_idx, feature in enumerate(
                                    ...
                                    if "space" in feature:
                                        ...
                                        ratemap = (
                                            cluster_data[offset][feature]["ratemaps"][
                                                :, :, 0
                                            ]
                                            / cluster_data[offset][feature]["ratemaps"][
                                                :, :, 1
                                        ...
                                        ax1.set_title(
                                            label="Spatial tuning", fontsize=5, pad=1.1
                                        )
                                        ax1.set_xticks([])
                                        ax1.set_xlabel("X (cm)", fontsize=4, labelpad=1)
                                        ax1.set_yticks([])
                                        ax1.set_ylabel("Y (cm)", fontsize=4, labelpad=1)

                                        ax_position_x1 = (
                                            gs[gs_x, gs_y].get_position(fig).x1
                                        )
                                        cb_ax = fig.add_axes(
                                            (
                                                ax_position_x1,
                                        ...
                                        occ = ax2.imshow(
                                            X=cluster_data[offset][feature]["ratemaps"][
                                                :, :, 1
                                            ],
                                            cmap=mouse_colormap,
                                            vmin=0,
                                        ...
                                        ax2.set_title(
                                            label="Spatial tuning (occ)",
                                            fontsize=5,
                                            pad=1.1,
                                        )
                                        ax2.set_xticks([])
                                        ax2.set_xlabel("X (cm)", fontsize=4, labelpad=1)
                                        ax2.set_yticks([])
                                        ax2.set_ylabel("Y (cm)", fontsize=4, labelpad=1)

                                        ax2_position_x1 = (
                                            gs[gs_x, gs_y + 1].get_position(fig).x1
                                        )
                                        cb_ax2 = fig.add_axes(
                                            (
                                                ax2_position_x1,
                                        ...
                                        cbar2_vmin, cbar2_vmax = (
                                            cbar2.mappable.get_clim()
                                        )
                                        cbar2.set_ticks([cbar2_vmin, cbar2_vmax])
                                        cbar2.set_ticklabels(
                                            ticklabels=[
                                                f"{int(cbar2_vmin)}",
                                                f"{int(np.ceil(cbar2_vmax))}",
                                            ],
                                            fontsize=4,
                                        )
                                        ...
                                        width = (
                                            cluster_data[offset][feature]["bin_edges"][
                                                1
                                            ]
                                            - cluster_data[offset][feature][
                                                "bin_edges"
                                        ...
                                        ax1.fill_between(
                                            ...
                                            where=high_end_sh[occ_threshold]
                                            >= low_end_sh[occ_threshold],
                                            facecolor="#D3D3D3",
                                            interpolate=True,
                                        )
                                        ...
                                        ax1.set_title(
                                            feature.split(".")[-1], fontsize=5, pad=1.1
                                        )
                                        ...
                                        ax1.set_xticks(
                                            ticks=[ax1_xmin_temp, ax1_xmax_temp],
                                            labels=[
                                                f"{ax1_xmin_temp:.1f}",
                                                f"{ax1_xmax_temp:.1f}",
                                            ],
                                        ...
                                        ax1.set_xlabel(
                                            f"{self.feature_labels[plot_feature_key.split('.')[0]][feature.split('.')[-1]]}",
                                            fontsize=4,
                                            labelpad=1,
                                        )
                                        ...
                                        ax1.set_yticks(
                                            ticks=[
                                                max(np.floor(temp_ymin), 0),
                                                np.ceil(temp_ymax),
                                            ],
                                            labels=[
                                                f"{max(np.floor(temp_ymin), 0):.1f}",
                                                f"{temp_ymax:.1f}",
                                            ],
                                            rotation=0,
                                            fontsize=4,
                                        ...
                                        ax1.set_ylabel(
                                            "Firing rate (sp/s)", fontsize=4, labelpad=1
                                        )
                                        ...
                                        ax2.bar(
                                            x=cluster_data[offset][feature][
                                                "bin_centers"
                                            ],
                                            height=cluster_data[offset][feature][
                                                "ratemaps"
                                            ][:, 1],
                                            width=width,
                                            align="center",
                                        ...
                                        ax2.set_yticks(
                                            ticks=[0, int(np.ceil(temp_ymax2)) - 10],
                                            labels=[
                                                "0",
                                                f"{int(np.ceil(temp_ymax2)) - 10}",
                                            ],
                                            rotation=0,
                                            fontsize=4,
                                        )
                                        ax2.set_ylabel(
                                            "Occupancy (s)", fontsize=4, labelpad=1
                                        )
