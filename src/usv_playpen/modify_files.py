...
class Operator:
    ...
    def __init__(
        self,
        root_directory: str | list[str] = None,
        ...
        if input_parameter_dict is None:
            with open(
                ...
                self.input_parameter_dict = input_parameter_dict_loaded["modify_files"][
                    "Operator"
                ]
                self.input_parameter_dict_2 = input_parameter_dict_loaded[
                    "synchronize_files"
                ]["Synchronizer"]
        else:
            self.input_parameter_dict = input_parameter_dict["modify_files"]["Operator"]
            self.input_parameter_dict_2 = input_parameter_dict["synchronize_files"][
                "Synchronizer"
            ]
        ...
        if root_directory is None:
            with open(
                ...
                self.root_directory = json.load(json_file)["modify_files"][
                    "root_directory"
                ]
        else:
    ...
    def split_clusters_to_sessions(self) -> None:
        ...
        self.message_output(
            f"Splitting clusters to sessions started at: {datetime.now().hour:02d}:{datetime.now().minute:02d}.{datetime.now().second:02d}"
        )
        ...
        for one_root_dir in self.root_directory:
            for ephys_dir in sorted(
                glob.glob(
                    pathname=f"{one_root_dir.replace('Data', 'EPHYS')[:-7]}_imec*",
                    recursive=True,
                )
            ):
                ...
                self.message_output(
                    f"Working on getting spike times from clusters in: {ephys_dir}, started at {datetime.now()}."
                )
                ...
                with open(
                    sorted(
                        glob.glob(
                            pathname=f"{ephys_dir}{os.sep}changepoints_info_*.json",
                            recursive=True,
                        )
                    )[0]
                    ...
                    for session_key in binary_files_info.keys():
                        binary_files_info[session_key]["root_directory"] = (
                            configure_path(
                                pa=binary_files_info[session_key]["root_directory"]
                            )
                        )
                ...
                for session_key in binary_files_info.keys():
                    ...
                    with open(
                        sorted(
                            glob.glob(
                                f"{binary_files_info[session_key]['root_directory']}{os.sep}video{os.sep}*_camera_frame_count_dict.json"
                            )
                        )[0]
                    ) as frame_count_infile:
                        camera_frame_info = json.load(frame_count_infile)
                        esr_dict[session_key] = camera_frame_info[
                            "median_empirical_camera_sr"
                        ]
                        frame_least_dict[session_key] = camera_frame_info[
                            "total_frame_number_least"
                        ]
                        root_dict[session_key] = binary_files_info[session_key][
                            "root_directory"
                        ]
                    ...
                    if any(
                        [
                            np.isnan(value)
                            for value in binary_files_info[session_key][
                                "tracking_start_end"
                            ]
                        ]
                    ):
                        se_dict[session_key] = binary_files_info[session_key][
                            "session_start_end"
                        ]
                    else:
                        se_dict[session_key] = binary_files_info[session_key][
                    ...
                    pathlib.Path(
                        f"{root_dict[session_key]}{os.sep}ephys{os.sep}{probe_id}{os.sep}cluster_data"
                    ).mkdir(parents=True, exist_ok=True)

                # load the Kilosort output files
                phy_curation_bool = os.path.isfile(
                    f"{ephys_dir}{os.sep}kilosort{self.input_parameter_dict['get_spike_times']['kilosort_version']}{os.sep}cluster_info.tsv"
                )
                spike_clusters = np.load(
                    f"{ephys_dir}{os.sep}kilosort{self.input_parameter_dict['get_spike_times']['kilosort_version']}{os.sep}spike_clusters.npy"
                )
                spike_times = np.load(
                    f"{ephys_dir}{os.sep}kilosort{self.input_parameter_dict['get_spike_times']['kilosort_version']}{os.sep}spike_times.npy"
                )

                if phy_curation_bool:
                    cluster_info = pd.read_csv(
                        filepath_or_buffer=f"{ephys_dir}{os.sep}kilosort{self.input_parameter_dict['get_spike_times']['kilosort_version']}{os.sep}cluster_info.tsv",
                        sep="\t",
                    )
                else:
                    self.message_output(
                        "Phy2 curation has not been done for this session, no cluster_info.tsv file exists."
                    )
                    continue
                ...
                for idx in tqdm(range(cluster_info.shape[0])):
                    if (
                        cluster_info.loc[idx, "group"] == "good"
                        or cluster_info.loc[idx, "group"] == "mua"
                    ):
                        # collect all spikes for any given cluster
                        cluster_indices = np.sort(
                            np.where(
                                spike_clusters == cluster_info.loc[idx, "cluster_id"]
                            )[0]
                        )
                        ...
                        for session_key in binary_files_info.keys():
                            session_spikes_sec = (
                                spike_events[
                                    (spike_events >= se_dict[session_key][0])
                                    & (spike_events < se_dict[session_key][1])
                                ]
                                - se_dict[session_key][0]
                            ) / float(
                                calibrated_sr_config["CalibratedHeadStages"][
                                    binary_files_info[session_key]["headstage_sn"]
                                ]
                            )

                            session_spikes_fps = np.round(
                                session_spikes_sec * esr_dict[session_key]
                            )
                            session_spikes_fps[
                                session_spikes_fps == frame_least_dict[session_key]
                            ] = frame_least_dict[session_key] - 1

                            session_spikes = np.vstack(
                                (session_spikes_sec, session_spikes_fps)
                            )

                            # save spiking data
                            if (
                                session_spikes_sec.shape[0]
                                > self.input_parameter_dict["get_spike_times"][
                                    "min_spike_num"
                                ]
                            ):
                                ...
                                np.save(
                                    file=f"{root_dict[session_key]}{os.sep}ephys{os.sep}{probe_id}{os.sep}cluster_data{os.sep}{cluster_id}",
                                    arr=session_spikes,
                                )

                                unit_count_dict[session_key][
                                    cluster_info.loc[idx, "group"]
                                ] += 1
                ...
                self.message_output(
                    f"For {ephys_dir}, there were {unit_count_dict['noise']} noise clusters and {unit_count_dict['unsorted']} unsorted clusters."
                )
                for session_key in binary_files_info.keys():
                    self.message_output(
                        f"For {root_dict[session_key]} probe {probe_id}, there were {unit_count_dict[session_key]['good']} good and {unit_count_dict[session_key]['mua']} MUA clusters."
                    )
    ...
    def concatenate_binary_files(self) -> None:
        ...
        self.message_output(
            f"E-phys file concatenation started at: {datetime.now().hour:02d}:{datetime.now().minute:02d}.{datetime.now().second:02d}. "
        ...
        for ord_idx, one_root_dir in enumerate(self.root_directory):
            ...
            for one_probe_dir in os.listdir(f"{one_root_dir}{os.sep}ephys"):
                ...
                if (
                    os.path.isdir(f"{one_root_dir}{os.sep}ephys{os.sep}{one_probe_dir}")
                    and "imec" in one_probe_dir
                ):
                    if not any(
                        [
                            True if one_probe_dir in one_concat_dir else False
                            for one_concat_dir in concat_save_dir
                        ]
                    ):
                        concat_save_dir.append(f"{ephys_save_dir_base}_{one_probe_dir}")

        npx_file_type = self.input_parameter_dict_2["validate_ephys_video_sync"][
            "npx_file_type"
        ]
        ...
        for probe_idx, probe_id in enumerate(available_probes):
            ...
            for ord_idx, one_root_dir in enumerate(self.root_directory):
                if os.path.isdir(f"{one_root_dir}{os.sep}ephys{os.sep}{probe_id}"):
                    for one_file, one_meta in zip(
                        sorted(
                            list(
                                pathlib.Path(
                                    f"{one_root_dir}{os.sep}ephys{os.sep}{probe_id}"
                                ).glob(f"*{npx_file_type}.bin*")
                            )
                        ),
                        sorted(
                            list(
                        ...
                        if one_file.is_file() and one_meta.is_file():
                            ...
                            with open(one_meta) as meta_data_file:
                                for line in meta_data_file:
                                    ...
                                    if key == "acqApLfSy":
                                        ...
                                        headstage_sn = value
                                        spike_glx_sr = float(
                                            calibrated_sr_config[
                                                "CalibratedHeadStages"
                                            ][headstage_sn]
                                        )
                                    elif key == "imDatPrb_sn":
                                        imec_probe_sn = value

                            binary_file_info_id = pathlib.Path(one_file).name.split(
                                os.sep
                            )[-1][:-7]
                            binary_files_info[binary_file_info_id] = {
                            ...
                            one_recording = np.memmap(
                                filename=one_file, mode="r", dtype="int16", order="C"
                            )

                            self.message_output(
                                f"File {pathlib.Path(one_file).name}, recorded with hs #{headstage_sn} & probe #{imec_probe_sn} has total length {one_recording.shape[0]}, or {one_recording.shape[0] // total_num_channels} "
                            ...
                            binary_files_info[binary_file_info_id][
                                "file_duration_samples"
                            ] = int(one_recording.shape[0] // total_num_channels)
                            ...
                            if len(changepoints) == 1:
                                ...
                                binary_files_info[binary_file_info_id][
                                    "session_start_end"
                                ][1] = int(one_recording.shape[0] // total_num_channels)
                                changepoints.append(
                                    int(one_recording.shape[0] // total_num_channels)
                                )
                                concatenation_command += f"{one_file} "
                            else:
                                binary_files_info[binary_file_info_id][
                                    "session_start_end"
                                ][0] = changepoints[-1]
            ...
            if not os.path.exists(
                f"{concat_save_dir[probe_idx]}{os.sep}changepoints_info_{concat_save_dir[probe_idx].split(os.sep)[-1]}.json"
            ):
                with open(
                    f"{concat_save_dir[probe_idx]}{os.sep}changepoints_info_{concat_save_dir[probe_idx].split(os.sep)[-1]}.json",
                    "w",
                ...
                for file_key in binary_files_info:
                    if file_key not in changepoint_info_data.keys():
                        ...
                        for component_key in changepoint_info_data[file_key].keys():
                            if (
                                component_key != "tracking_start_end"
                                and component_key != "largest_camera_break_duration"
                                and component_key != "root_directory"
                                and changepoint_info_data[file_key][component_key]
                                != binary_files_info[file_key][component_key]
                            ):
                                changepoint_info_data[file_key][component_key] = (
                                    binary_files_info[file_key][component_key]
                                )
                            elif (
                                component_key == "tracking_start_end"
                                ...
                                changepoint_info_data[file_key][component_key][0] = (
                                    changepoint_info_data[file_key][component_key][0]
                                    + binary_files_info[file_key]["session_start_end"][
                                        0
                                    ]
                                )
                                changepoint_info_data[file_key][component_key][1] = (
                                    changepoint_info_data[file_key][component_key][1]
                                    + binary_files_info[file_key]["session_start_end"][
                                        0
                                    ]
            ...
            subprocess.Popen(
                args=f"{self.command_addition}{concatenation_command}",
                shell=True,
    ...
    def multichannel_to_channel_audio(self) -> None:
        ...
        self.message_output(
            f"Multichannel to single channel audio conversion started at: {datetime.now().hour:02d}:{datetime.now().minute:02d}.{datetime.now().second:02d}"
        )
        smart_wait(app_context_bool=self.app_context_bool, seconds=1)

        pathlib.Path(f"{self.root_directory}{os.sep}audio{os.sep}temp").mkdir(
            parents=True, exist_ok=True
        )

        # separate each channel of every multichannel audio file
        mc_audio_files = sorted(
            glob.glob(
                f"{self.root_directory}{os.sep}audio{os.sep}original_mc{os.sep}*.wav"
            )
        )
        for mc_audio_file in mc_audio_files:
            ...
            for ch in range(1, 13):
                ...
                sep_ch_subp = subprocess.Popen(
                    args=f"""{self.command_addition}sox {mc_audio_file_basename} {output_file} remix {ch}""",
            ...
            while True:
                status_poll = [
                    query_subp.poll() for query_subp in separate_ch_subprocesses
                ]
        ...
        name_origin = sorted(
            glob.glob(
                f"{self.root_directory}{os.sep}audio{os.sep}temp{os.sep}m_*_ch*.wav"
            )
        )[0].split("_")[2]
        ...
        for device_id in ["m", "s"]:
            for ch in range(1, 13):
                mc_to_sc_subp = subprocess.Popen(
                    args=f"""{self.command_addition}sox {device_id}_*_ch{ch:02d}.wav -q {self.root_directory}{os.sep}audio{os.sep}original{os.sep}{device_id}_{name_origin}_ch{ch:02d}.wav""",
    ...
    def hpss_audio(self) -> None:
        ...
        self.message_output(
            f"Harmonic-percussive source separation started at: {datetime.now().hour:02d}:{datetime.now().minute:02d}.{datetime.now().second:02d}"
        )
        smart_wait(app_context_bool=self.app_context_bool, seconds=1)
        ...
        for one_wav_file in wav_file_lst:
            ...
            spectrogram_data = librosa.stft(
                y=audio_data,
                n_fft=self.input_parameter_dict["hpss_audio"][
                    "stft_window_length_hop_size"
                ][0],
                hop_length=self.input_parameter_dict["hpss_audio"][
                    "stft_window_length_hop_size"
                ][1],
            )
            ...
            harmonic_data = librosa.istft(
                ...
                win_length=self.input_parameter_dict["hpss_audio"][
                    "stft_window_length_hop_size"
                ][0],
                hop_length=self.input_parameter_dict["hpss_audio"][
                    "stft_window_length_hop_size"
            ...
            harmonic_data_clipped = np.clip(
                a=harmonic_data, a_min=-32768, a_max=32767
            ).astype("int16")
            ...
            wavfile.write(
                filename=f"{new_dir}{os.sep}{raw_file_name[:-4]}_hpss.wav",
    ...
    def filter_audio_files(self) -> None:
        ...
        freq_lp = self.input_parameter_dict["filter_audio_files"]["filter_freq_bounds"][
            0
        ]
        freq_hp = self.input_parameter_dict["filter_audio_files"]["filter_freq_bounds"][
            1
        ]

        self.message_output(
            f"Filtering out signal between {freq_lp} and {freq_hp} Hz in audio files started at: "
        ...
        for one_dir in self.input_parameter_dict["filter_audio_files"]["filter_dirs"]:
            pathlib.Path(
                f"{self.root_directory}{os.sep}audio{os.sep}{one_dir}_filtered"
            ).mkdir(parents=True, exist_ok=True)

            filter_subprocesses = []
            all_audio_files = sorted(
                glob.glob(
                    f"{self.root_directory}{os.sep}audio{os.sep}{one_dir}{os.sep}"
            ...
            if len(all_audio_files) > 0:
                for one_file in all_audio_files:
                    filter_subp = subprocess.Popen(
                        args=f"""{self.command_addition}sox --ignore-length {one_file.split(os.sep)[-1]} {self.root_directory}{os.sep}audio{os.sep}{one_dir}_filtered{os.sep}{one_file.split(os.sep)[-1][:-4]}_filtered.wav sinc {freq_hp}-{freq_lp}""",
    ...
    def concatenate_audio_files(self) -> None:
        ...
        self.message_output(
            f"Audio concatenation started at: {datetime.now().hour:02d}:{datetime.now().minute:02d}.{datetime.now().second:02d}"
        )
        smart_wait(app_context_bool=self.app_context_bool, seconds=1)

        for audio_file_type in self.input_parameter_dict["concatenate_audio_files"][
            "concat_dirs"
        ]:
            all_audio_files = sorted(
                glob.glob(
                    f"{self.root_directory}{os.sep}audio{os.sep}{audio_file_type}{os.sep}"
            ...
            if len(all_audio_files) > 1:
                data_dict = DataLoader(
                    input_parameter_dict={
                        "wave_data_loc": [
                            f"{self.root_directory}{os.sep}audio{os.sep}{audio_file_type}"
                        ],
                        "load_wavefile_data": {
                            "library": "scipy",
                            "conditional_arg": [],
                ...
                complete_mm_file_name = (
                    f"{self.root_directory}{os.sep}audio{os.sep}{audio_file_type}{os.sep}{name_origin}"
                    f"_concatenated_audio_{audio_file_type}_{sr}_{dim_1}_{dim_2}_int16.mmap"
                )

                audio_mm_arr = np.memmap(
                    filename=complete_mm_file_name,
                    dtype="int16",
                    mode="w+",
                    shape=(dim_1, dim_2),
                )
                ...
                self.message_output(
                    f"There are <2 audio files per provided directory: '{self.root_directory}{os.sep}audio{os.sep}cropped_to_video', "
    ...
    def concatenate_video_files(self) -> None:
        ...
        self.message_output(
            f"Video concatenation started at: {datetime.now().hour:02d}:{datetime.now().minute:02d}.{datetime.now().second:02d}"
        )
        ...
        for sub_directory in os.listdir(f"{self.root_directory}{os.sep}video"):
            if (
                "calibration" not in sub_directory
                and sub_directory.split(".")[-1]
                in self.input_parameter_dict["concatenate_video_files"][
                ...
                vid_extension = self.input_parameter_dict["concatenate_video_files"][
                    "concatenate_video_extension"
                ]
                all_video_files = sorted(
                    glob.glob(f"{current_working_dir}{os.sep}*.{vid_extension}")
                )

                if len(all_video_files) > 1:
                    # create .txt file with video files to concatenate
                    with open(
                        f"{current_working_dir}{os.sep}file_concatenation_list_{sub_directory.split('.')[-1]}.txt",
                        "w",
                        encoding="utf-8",
                    ) as concat_txt_file:
                        for file_path in all_video_files:
                            concat_txt_file.write(
                                f"file '{file_path.split(os.sep)[-1]}'\n"
                            )

                    # concatenate videos
                    one_subprocess = subprocess.Popen(
                        args=f"""{self.command_addition}ffmpeg -loglevel warning -f concat -i file_concatenation_list_{sub_directory.split(".")[-1]}.txt -c copy {vid_name}.{vid_extension}""",
                        stdout=subprocess.DEVNULL,
                        stderr=subprocess.STDOUT,
        ...
        for sub_directory in os.listdir(f"{self.root_directory}{os.sep}video"):
            if (
                ...
                shutil.move(
                    src=f"{current_working_dir}{os.sep}{self.input_parameter_dict['concatenate_video_files']['concatenated_video_name']}_{sub_directory.split('.')[-1]}.{self.input_parameter_dict['concatenate_video_files']['concatenate_video_extension']}",
    ...
    def rectify_video_fps(self, conduct_concat: bool = True) -> None:
        ...
        self.message_output(
            f"Video re-encoding started at: {datetime.now().hour:02d}:{datetime.now().minute:02d}.{datetime.now().second:02d}"
        )

        if (
            not conduct_concat
            and len(next(os.walk(f"{self.root_directory}{os.sep}video"))[2]) == 0
        ):
            for sub_directory in os.listdir(f"{self.root_directory}{os.sep}video"):
                if (
                    "calibration" not in sub_directory
                    and sub_directory.split(".")[-1]
                    in self.input_parameter_dict["rectify_video_fps"][
                    ...
                    shutil.copy(
                        src=f"{current_working_dir}{os.sep}{self.input_parameter_dict['rectify_video_fps']['conversion_target_file']}.{self.input_parameter_dict['rectify_video_fps']['encode_video_extension']}",
        ...
        empirical_camera_sr = np.zeros(
            len(
                self.input_parameter_dict["rectify_video_fps"][
                    "encode_camera_serial_num"
                ]
            )
        )
        ...
        fsp_subprocesses = []
        for sd_idx, sub_directory in enumerate(
            sorted(os.listdir(f"{self.root_directory}{os.sep}video"))
        ):
            if (
                os.path.isdir(
                    f"{self.root_directory}{os.sep}video{os.sep}{sub_directory}"
                ...
                if camera_idx == 0:
                    date_joint = (
                        sub_directory.split(".")[0].split("_")[-2]
                        + sub_directory.split(".")[0].split("_")[-1]
                    )
                ...
                if "calibration" not in sub_directory:
                    ...
                    camera_frame_count_dict[sub_directory.split(".")[-1]] = (
                        total_frame_num,
                        esr,
                    )
                    if total_frame_num == last_frame_num:
                        self.message_output(
                            f"Camera {sub_directory.split('.')[-1]} has {total_frame_num} total frames, no dropped frames, "
                            f"video duration of {video_duration:.4f} seconds, and sampling rate of {esr} fps."
                        )
                        total_frame_number = min(total_frame_number, total_frame_num)
                        total_video_time = min(total_video_time, video_duration)
                    else:
                        self.message_output(
                            f"WARNING: The last frame on camera {sub_directory.split('.')[-1]} is {last_frame_num}, which is more than {total_frame_num} in total, "
                            f"suggesting dropped frames. The video duration is {video_duration:.4f} seconds"
                        )
                    camera_idx += 1

                crf = self.input_parameter_dict["rectify_video_fps"][
                    "constant_rate_factor"
                ]
                enc_preset = self.input_parameter_dict["rectify_video_fps"][
                    "encoding_preset"
                ]
                ...
                fps_subp = subprocess.Popen(
                    args=f"""{self.command_addition}ffmpeg -loglevel warning -y -r {esr} -i {target_file} -fps_mode passthrough -crf {crf} -preset {enc_preset} {new_file}""",
                    stdout=subprocess.DEVNULL,
                    stderr=subprocess.STDOUT,
                    cwd=current_working_dir,
                    shell=self.shell_usage_bool,
        ...
        for sd_idx, sub_directory in enumerate(
            ...
            if (
                ...
                pathlib.Path(
                    f"{self.root_directory}{os.sep}video{os.sep}{date_joint}{os.sep}{sub_directory.split('.')[-1]}{os.sep}calibration_images"
                ).mkdir(parents=True, exist_ok=True)
                ...
                if "calibration" not in sub_directory:
                    ...
                    shutil.move(
                        src=f"{current_working_dir}{os.sep}{new_file}",
                ...
                if self.input_parameter_dict["rectify_video_fps"]["delete_old_file"]:
                    if os.path.isfile(
                        f"{current_working_dir}{os.sep}{self.input_parameter_dict['rectify_video_fps']['conversion_target_file']}_{sub_directory.split('.')[-1]}.{self.input_parameter_dict['rectify_video_fps']['encode_video_extension']}"
                    ):
        ...
        camera_frame_count_dict["median_empirical_camera_sr"] = round(
            number=np.median(empirical_camera_sr), ndigits=4
        )
        with open(
            f"{self.root_directory}{os.sep}video{os.sep}{date_joint}_camera_frame_count_dict.json",
            "w",
        ) as frame_count_outfile:
