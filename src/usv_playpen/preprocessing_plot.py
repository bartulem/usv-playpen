...
class SummaryPlotter:
    ...
    def preprocessing_summary(
        ...
        for device_id in ipi_discrepancy_dict:
            plot_statistics_dict[device_id] = {}
            plot_statistics_dict[device_id]["error_median"] = np.round(
                np.nanmedian(ipi_discrepancy_dict[device_id]["ipi_discrepancy_ms"]),
                decimals=2,
            )
            plot_statistics_dict[device_id]["error_mean"] = np.round(
            ...
            plot_statistics_dict[device_id]["low_ci"] = plot_statistics_dict[device_id][
                "error_mean"
            ] - (2.58 * plot_statistics_dict[device_id]["error_sem"])
            plot_statistics_dict[device_id]["high_ci"] = plot_statistics_dict[
                device_id
            ]["error_mean"] + (2.58 * plot_statistics_dict[device_id]["error_sem"])
        ...
        med_hum = np.round(
            np.nanmedian(phidget_data_dictionary["humidity"]), decimals=2
        )
        ...
        min_temp = np.round(
            np.nanmin(phidget_data_dictionary["temperature"]), decimals=2
        )
        max_temp = np.round(
            np.nanmax(phidget_data_dictionary["temperature"]), decimals=2
        )
        med_temp = np.round(
            np.nanmedian(phidget_data_dictionary["temperature"]), decimals=2
        )
        ...
        for device_num, device_id in enumerate(plot_statistics_dict.keys()):
            ax[0, device_num].hist(
                ...
                bins=np.arange(
                    -plot_statistics_dict[device_id]["most_extreme_value"] - 0.5,
                    plot_statistics_dict[device_id]["most_extreme_value"] + 1,
                    1,
                ),
                histtype="stepfilled",
                color="#BBD5E826",
            ...
            ax[0, device_num].axvline(
                x=plot_statistics_dict[device_id]["error_mean"],
                ls="-",
                lw=1.4,
                c="#00C78C",
            )

            ax[0, device_num].set_xlabel(
                "undershoot <--- A-V IPI start discrepancy (ms) ---> overshoot"
            )
            ...
            ax[0, device_num].text(
                ...
                y=0.18,
                s=r"median$_{error}$: "
                + f"{plot_statistics_dict[device_id]['error_median']} ms",
            ...
            ax[0, device_num].text(
                ...
                y=0.15,
                s=r"mean$_{error}$: "
                + f"{plot_statistics_dict[device_id]['error_mean']} ms",
                verticalalignment="top",
                transform=ax[0, device_num].transAxes,
                fontsize=6,
                color="#00C78C",
            ...
            ax[0, device_num].text(
                x=0.775,
                y=0.12,
                s=f"99% CI [{plot_statistics_dict[device_id]['low_ci']:.3f}, {plot_statistics_dict[device_id]['high_ci']:.3f}]",
                verticalalignment="top",
                transform=ax[0, device_num].transAxes,
                fontsize=6,
            )
            ax[0, device_num].text(
                ...
                y=0.09,
                s=r"N$_{IPI}$="
                + f"{ipi_discrepancy_dict[device_id]['ipi_discrepancy_ms'].size}",
                verticalalignment="top",
                transform=ax[0, device_num].transAxes,
                fontsize=6,
            )

            ax[1, device_num].scatter(
                ipi_discrepancy_dict[device_id]["video_ipi_start_frames"],
                ipi_discrepancy_dict[device_id]["ipi_discrepancy_ms"],
                color="#BBD5E826",
                edgecolor="#00000040",
                s=12,
            ...
            if (
                ...
                axin_nidq_.scatter(
                    ipi_discrepancy_dict[device_id]["nidq_ipi_start_samples"],
                    ipi_discrepancy_dict[device_id]["nidq_ipi_discrepancy_ms"],
                    color="#BBD5E826",
                    edgecolor="#00000040",
                    s=4,
                )
            ...
            if "m_" in device_id:
                ...
                ax[0, device_num].text(
                    x=0.005,
                    y=0.715,
                    s=r"$\bf{duration}$: " + f"{int(round(duration_min))} s",
                    verticalalignment="top",
                    transform=ax[0, device_num].transAxes,
                    fontsize=8,
                )
                ...
                ax[0, device_num].text(
                    x=0.005,
                    y=0.595,
                    s=r"$\bf{user set gain}$: " + f"{camera_gain} dB",
                    verticalalignment="top",
                    transform=ax[0, device_num].transAxes,
                    fontsize=8,
                )
                ax[0, device_num].text(
                    x=0.005,
                    y=0.565,
                    s=r"$\bf{user set exposure}$: " + f"{camera_exposure} μs",
                    verticalalignment="top",
                    transform=ax[0, device_num].transAxes,
                    fontsize=8,
                )
                ax[0, device_num].text(
                    x=0.005,
                    y=0.535,
                    s=r"$\bf{user set framerate}$: " + f"{camera_frame_rate} fps",
                    verticalalignment="top",
                    transform=ax[0, device_num].transAxes,
                    fontsize=8,
                )
                ...
                for cam_idx in range(len(used_cameras)):
                    ax[0, device_num].text(
                        x=0.005,
                        y=cam_y_txt,
                        s=f"{used_cameras[cam_idx]}: {total_frames[cam_idx]} fr, {cam_esr[cam_idx]} fps",
                        verticalalignment="top",
                        transform=ax[0, device_num].transAxes,
                        fontsize=8,
                    )
                ...
                ax[0, device_num].text(
                    x=0.625,
                    y=0.65,
                    s=f"{min_temp}    {med_temp}      {max_temp}",
                    verticalalignment="top",
                    transform=ax[0, device_num].transAxes,
                    fontsize=8,
                )
            ...
            if "s_" in device_id:
                ...
                axin2 = ax[0, device_num].inset_axes(
                ...
                axin4 = ax[0, device_num].inset_axes(
                ...
                axin6 = ax[0, device_num].inset_axes(
