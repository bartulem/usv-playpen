...
class Synchronizer:
    ...
    def __init__(
        ...
        if input_parameter_dict is None:
            with open(
                ...
                self.input_parameter_dict = json.load(json_file)["synchronize_files"][
                    "Synchronizer"
                ]
        else:
            self.input_parameter_dict = input_parameter_dict["synchronize_files"][
                "Synchronizer"
            ]
        ...
        if root_directory is None:
            with open(
                ...
                self.root_directory = json.load(json_file)["synchronize_files"][
                    "root_directory"
                ]
        else:
    ...
    def validate_ephys_video_sync(self) -> None:
        ...
        self.message_output(
            f"Checking e-phys/video sync started at: {datetime.now().hour:02d}:{datetime.now().minute:02d}.{datetime.now().second:02d}"
        )
        smart_wait(app_context_bool=self.app_context_bool, seconds=1)

        # read headstage sampling rates
        calibrated_sr_config = configparser.ConfigParser()
        calibrated_sr_config.read(
            os.path.join(
                os.path.dirname(os.path.abspath(__file__)),
        ...
        with open(
            sorted(
                glob.glob(
                    pathname=f"{self.root_directory}{os.sep}**{os.sep}*_camera_frame_count_dict.json",
                    recursive=True,
                )
            )[0]
        ) as frame_count_infile:
            camera_frame_count_dict = json.load(frame_count_infile)
            total_frame_number_least = camera_frame_count_dict[
                "total_frame_number_least"
            ]
            total_video_time_least = camera_frame_count_dict["total_video_time_least"]

        for npx_idx, npx_recording in enumerate(
            sorted(
                glob.glob(
                    pathname=f"{self.root_directory}{os.sep}**{os.sep}*{self.input_parameter_dict['validate_ephys_video_sync']['npx_file_type']}.bin",
                    recursive=True,
                )
            )
        ):
            ...
            with open(f"{npx_recording[:-3]}meta") as meta_data_file:
                for line in meta_data_file:
                    ...
                    if key == "acqApLfSy":
                        total_probe_ch = int(value.split(",")[0]) + int(
                            value.split(",")[-1]
                        )
                    elif key == "imDatHs_sn":
                        headstage_sn = value
            ...
            self.message_output(
                f"N/V sync for {recording_file_name} with {total_probe_ch} channels, recorded w/ probe #{imec_probe_sn} & headstage #{headstage_sn}."
            )

            sync_ch_file = f"{os.path.dirname(npx_recording)}{os.sep}{os.path.basename(npx_recording)[:-7]}_sync_ch_data".replace(
                ".", "_"
            )
            if not os.path.isfile(f"{sync_ch_file}.npy"):
                # load the binary file data
                one_recording = np.memmap(
                    filename=npx_recording, mode="r", dtype="int16", order="C"
                )
                one_sample_num = one_recording.shape[0] // total_probe_ch

                # reshape the array such that channels are rows and samples are columns
                sync_data = one_recording.reshape(
                    (total_probe_ch, one_sample_num), order="F"
                )[-1, :]
            ...
            (
                tracking_start,
                tracking_end,
                largest_break_duration,
                ttl_break_end_samples,
                largest_break_end_hop,
            ) = self.find_lsb_changes(
                relevant_array=ch_sync_data,
                lsb_bool=False,
                total_frame_number=total_frame_number_least,
            )

            largest_break_duration_sec = round(
                largest_break_duration
                / float(calibrated_sr_config["CalibratedHeadStages"][headstage_sn]),
                3,
            )
            if (tracking_start, tracking_end) != (
                None,
                None,
            ) or largest_break_duration_sec < 2:
                spike_glx_sr = float(
                    calibrated_sr_config["CalibratedHeadStages"][headstage_sn]
                )
                total_npx_recording_duration = (
                    tracking_end - tracking_start
                ) / spike_glx_sr

                duration_difference = round(
                    number=(
                        (total_npx_recording_duration - total_video_time_least) * 1000
                    ),
                    ndigits=2,
                )
                ...
                self.message_output(
                    f"{recording_file_name} is {abs(duration_difference)} ms {comparator_word} than the video recording with {largest_break_duration_sec} s largest camera break duration."
                )

                if (
                    abs(duration_difference)
                    < self.input_parameter_dict["validate_ephys_video_sync"][
                        "npx_ms_divergence_tolerance"
                    ]
                ):
                    # save tracking start and end in changepoint information JSON file
                    root_ephys = (
                        self.root_directory.replace("Data", "EPHYS").replace(
                            self.root_directory.split(os.sep)[-1], recording_date
                        )
                        + f"_{imec_probe_id}"
                    )
                    ...
                    if (
                        len(
                            sorted(
                                glob.glob(
                                    pathname=f"{root_ephys}{os.sep}changepoints_info_*.json",
                                    recursive=True,
                                )
                            )
                        )
                        ...
                        binary_files_info[recording_file_name[:-7]] = {
                            "session_start_end": [np.nan, np.nan],
                    ...
                    binary_files_info[recording_file_name[:-7]][
                        "tracking_start_end"
                    ] = [int(tracking_start), int(tracking_end)]
                    binary_files_info[recording_file_name[:-7]][
                        "largest_camera_break_duration"
                    ] = int(largest_break_duration)

                    with open(
                        f"{root_ephys}{os.sep}changepoints_info_{recording_date}_{imec_probe_id}.json",
                        "w",
                    ) as binary_info_output_file:
                        json.dump(binary_files_info, binary_info_output_file, indent=4)

                    self.message_output(
                        f"SUCCESS! Tracking start/end sample times saved in {sorted(glob.glob(pathname=f'{root_ephys}{os.sep}changepoints_info_*.json', recursive=True))[0]}."
                    )

                else:
                    count_values_in_sync_data = sorted(
                        dict(Counter(ch_sync_data)).items(),
                        key=operator.itemgetter(1),
                        reverse=True,
                    )
                    self.message_output(
                        f"{recording_file_name} has a duration difference (e-phys/tracking) of {duration_difference} ms, so above threshold. "
                        f"Values in original sync data: {count_values_in_sync_data}. Inspect further before proceeding."
                    )

            else:
                self.message_output(
                    f"Tracking end exceeds e-phys recording boundary, so not found for {recording_file_name}."
                )
                continue

    @staticmethod
    def find_lsb_changes(
        relevant_array: np.ndarray = None,
        lsb_bool: bool = True,
        ...
        if lsb_bool:
            ...
            largest_break_end_hop = (
                np.argmax(ttl_break_end_samples[1:] - ttl_break_end_samples[:-1]) + 1
            )
        else:
            ttl_break_end_samples = np.where(
                (relevant_array[1:] - relevant_array[:-1]) > 0
            )[0]
            largest_break_end_hop = (
                np.argmax(ttl_break_end_samples[1:] - ttl_break_end_samples[:-1]) + 1
            )

        largest_break_duration = np.max(
            ttl_break_end_samples[1:] - ttl_break_end_samples[:-1]
        )

        if (total_frame_number + largest_break_end_hop) <= ttl_break_end_samples.shape[
            0
        ]:
            return (
                int(ttl_break_end_samples[largest_break_end_hop] + 1),
                int(
                    ttl_break_end_samples[largest_break_end_hop + total_frame_number]
                    + 1
                ),
                int(largest_break_duration),
                ttl_break_end_samples,
    ...
    @staticmethod
    @njit(parallel=True)
    def find_ipi_intervals(
        sound_array: np.ndarray = None, audio_sr_rate: int = 250000
    ) -> tuple:
        """
        Description
        ----------
        ...
        if ipi_start_samples[0] < ipi_end_samples[0]:
            if ipi_start_samples.size == ipi_end_samples.size:
                ipi_durations_ms = (
                    ((ipi_end_samples - ipi_start_samples) + 1) * 1000 / audio_sr_rate
                )
                audio_ipi_start_samples = ipi_start_samples
            else:
                ipi_durations_ms = (
                    ((ipi_end_samples - ipi_start_samples[:-1]) + 1)
                    * 1000
                    / audio_sr_rate
                )
                audio_ipi_start_samples = ipi_start_samples[:-1]
        elif ipi_start_samples.size == ipi_end_samples.size:
            ipi_durations_ms = (
                ((ipi_end_samples[1:] - ipi_start_samples[:-1]) + 1)
                * 1000
                / audio_sr_rate
            )
            audio_ipi_start_samples = ipi_start_samples[:-1]
            ...
            ipi_durations_ms = (
                ((ipi_end_samples[1:] - ipi_start_samples) + 1) * 1000 / audio_sr_rate
            )
            audio_ipi_start_samples = ipi_start_samples
    ...
    @staticmethod
    @njit(parallel=True)
    def relative_change_across_array(
        input_array: np.ndarray = None, desired_axis: int = 0
    ) -> np.ndarray:
        """
        Description
        ----------
    ...
    def gather_px_information(
        self,
        video_of_interest: str = None,
        ...
        led_px_version = self.input_parameter_dict["find_video_sync_trains"][
            "led_px_version"
        ]
        ...
        for led_idx, led_position in enumerate(
            self.led_px_dict[led_px_version][used_camera].keys()
        ):
            led_dim1, led_dim2 = self.led_px_dict[led_px_version][used_camera][
                led_position
            ]
            ...
            if led_idx == 0:
                ...
                for one_num in range(max_frame_num):
                    fame_pxl_values[one_num] = loaded_video[one_num][
                        dim1_floor_pxl:dim1_ceil_pxl, dim2_floor_pxl:dim2_ceil_pxl
                    ].max()
                peak_intensity_frame_loc = fame_pxl_values.argmax()

            frame_of_choice = loaded_video[peak_intensity_frame_loc][
                dim1_floor_pxl:dim1_ceil_pxl, dim2_floor_pxl:dim2_ceil_pxl
            ]
            screen_results = np.where(frame_of_choice == frame_of_choice.max())
            result_dim1, result_dim2 = [
                dim1_floor_pxl + screen_results[0][0],
                dim2_floor_pxl + screen_results[1][0],
            ]
            self.led_px_dict[led_px_version][used_camera][led_position] = [
                result_dim1,
                result_dim2,
            ]
            self.message_output(
                f"For camera {used_camera}, {led_position} the highest intensity pixel is in position {result_dim1},{result_dim2}."
            )

        # create memmap array to store the data for posterity
        mm_arr = np.memmap(
            filename=f"{self.root_directory}{os.sep}sync{os.sep}sync_px_{video_name[:-4]}",
            dtype=DataLoader(input_parameter_dict={}).known_dtypes["np.uint8"],
            mode="w+",
            shape=(total_frame_number, 3, 3),
        )

        for fr_idx in range(total_frame_number):
            try:
                processed_frame = modify_memmap_array(
                    loaded_video[fr_idx],
                    mm_arr,
                    fr_idx,
                ...
                self.message_output(
                    f"WARNING: Reached end of decodable frames at index {fr_idx}, while PIMS reported {total_frame_number} total frames."
                )
                break
    ...
    def find_video_sync_trains(
        self, camera_fps: list = None, total_frame_number: int = None
    ) -> tuple:
        """
        Description
        ----------
        ...
        for video_subdir in os.listdir(f"{self.root_directory}{os.sep}video"):
            if "_" not in video_subdir and os.path.isdir(
                f"{self.root_directory}{os.sep}video{os.sep}{video_subdir}"
            ):
                sync_cam_idx = 0
                for camera_dir in os.listdir(
                    f"{self.root_directory}{os.sep}video{os.sep}{video_subdir}"
                ):
                    if (
                        camera_dir != ".DS_Store"
                        and os.path.isdir(
                            f"{self.root_directory}{os.sep}video{os.sep}{video_subdir}{os.sep}{camera_dir}"
                        )
                        and camera_dir
                        in self.input_parameter_dict["find_video_sync_trains"][
                            "sync_camera_serial_num"
                        ]
                    ):
                        video_name = sorted(
                            glob.glob(
                                f"{self.root_directory}{os.sep}video{os.sep}{video_subdir}{os.sep}{camera_dir}{os.sep}*.mp4"
                            )
                        )[0].split(os.sep)[-1]
                        if (
                            "calibration" not in video_name
                            and video_name.split("-")[0]
                            in self.input_parameter_dict["find_video_sync_trains"][
                                "sync_camera_serial_num"
                            ]
                            and self.input_parameter_dict["find_video_sync_trains"][
                                "sync_video_extension"
                            ]
                            in video_name
                        ):
                            current_working_dir = f"{self.root_directory}{os.sep}video{os.sep}{video_subdir}{os.sep}{camera_dir}"
                            video_of_interest = (
                                f"{current_working_dir}{os.sep}{video_name}"
                            )

                            if not os.path.exists(
                                f"{self.root_directory}{os.sep}sync{os.sep}sync_px_{video_name[:-4]}"
                            ):
                            ...
                            # load memmap data
                            leds_array = np.memmap(
                                filename=f"{self.root_directory}{os.sep}sync{os.sep}sync_px_{video_name[:-4]}",
                                dtype=DataLoader(input_parameter_dict={}).known_dtypes[
                                    "np.uint8"
                                ],
                                mode="r",
                                shape=(total_frame_number, 3, 3),
                            ...
                            diff_across_leds = np.median(
                                self.relative_change_across_array(
                                    input_array=mean_across_rgb
                                ),
                                axis=1,
                            )
                            ...
                            for txt_file in os.listdir(
                                f"{self.root_directory}{os.sep}sync"
                            ):
                                if "CoolTerm" in txt_file:
                                    with open(
                                        f"{self.root_directory}{os.sep}sync{os.sep}{txt_file}"
                                    ) as ipi_txt_file:
                                        for line_num, line in enumerate(
                                            ipi_txt_file.readlines()
                                        ):
                                            if line_num > 2 and line.strip():
                                                arduino_ipi_durations.append(
                                                    int(line.strip())
                                                )
                                    break
                            ...
                            relative_intensity_threshold = self.input_parameter_dict[
                                "find_video_sync_trains"
                            ]["relative_intensity_threshold"]
                            sequence_found = False
                            for threshold_value in np.arange(
                                0.2, relative_intensity_threshold, 0.01
                            )[::-1]:
                                if not sequence_found:
                                    diff_mask_neg = np.logical_and(
                                        np.logical_and(
                                            diff_across_leds[:-1] > -threshold_value,
                                            diff_across_leds[:-1] < threshold_value,
                                        ),
                                        diff_across_leds[1:] < -threshold_value,
                                    )
                                    neg_significant_events = (
                                        np.where(diff_mask_neg)[0] + 1
                                    )
                                    neg_significant_events = np.delete(
                                        neg_significant_events,
                                        np.argwhere(
                                            np.ediff1d(neg_significant_events)
                                            <= int(
                                                np.ceil(camera_fps[sync_cam_idx] / 2.5)
                                            )
                                        )
                                        + 1,
                                    )

                                    diff_mask_pos = np.logical_and(
                                        np.logical_and(
                                            diff_across_leds[:-1] > -threshold_value,
                                            diff_across_leds[:-1] < threshold_value,
                                        ),
                                    ...
                                    pos_significant_events = np.delete(
                                        pos_significant_events,
                                        np.argwhere(
                                            np.ediff1d(pos_significant_events)
                                            <= int(
                                                np.ceil(camera_fps[sync_cam_idx] / 2.5)
                                            )
                                        )
                                    ...
                                    if (
                                        pos_significant_events.size > 0
                                        and neg_significant_events.size > 0
                                    ):
                                        if 0 <= (
                                            pos_significant_events.size
                                            - neg_significant_events.size
                                        ) < 2 or (
                                            0
                                            <= np.abs(
                                                pos_significant_events.size
                                            ...
                                            if (
                                                ...
                                                neg_significant_events = (
                                                    neg_significant_events[1:]
                                                )

                                            if (
                                                pos_significant_events[0]
                                                < neg_significant_events[0]
                                            ):
                                                if (
                                                    pos_significant_events.size
                                                    == neg_significant_events.size
                                                ):
                                                    ipi_durations_frames = (
                                                        neg_significant_events
                                                        - pos_significant_events
                                                    ) - 1
                                                    temp_ipi_start_frames = (
                                                        pos_significant_events + 1
                                                    )
                                                else:
                                                    ipi_durations_frames = (
                                                        neg_significant_events
                                            ...
                                            ipi_durations_ms = np.round(
                                                ipi_durations_frames
                                                * (1000 / camera_fps[sync_cam_idx])
                                            )

                                            # match IPI sequences
                                            subarray_size = ipi_durations_ms.shape[0]
                                            start_indices = np.arange(
                                                len(arduino_ipi_durations)
                                                - subarray_size
                                                + 1
                                            )
                                            index_matrix = start_indices[
                                                :, np.newaxis
                                            ] + np.arange(subarray_size)
                                            arduino_ipi_durations_subarrays = (
                                                arduino_ipi_durations[index_matrix]
                                            )
                                            ...
                                            all_zero_matches = np.all(
                                                result_array
                                                <= self.input_parameter_dict[
                                                    "find_video_sync_trains"
                                                ]["millisecond_divergence_tolerance"],
                                                axis=1,
                                            )
                                            any_all_zeros = np.any(all_zero_matches)
                                            if any_all_zeros:
                                                sync_sequence_dict[camera_dir] = (
                                                    np.ravel(
                                                        arduino_ipi_durations_subarrays[
                                                            all_zero_matches
                                                        ]
                                ...
                                self.message_output(
                                    f"No IPI sequence match found in video '{video_of_interest.split(os.sep)[-1]}'!"
                                )
    ...
    def find_audio_sync_trains(self) -> dict:
        ...
        self.message_output(
            f"A/V synchronization started at: {datetime.now().hour:02d}:{datetime.now().minute:02d}.{datetime.now().second:02d}"
        )
        smart_wait(app_context_bool=self.app_context_bool, seconds=1)

        wave_data_dict = DataLoader(
            input_parameter_dict={
                "wave_data_loc": [
                    f"{self.root_directory}{os.sep}audio{os.sep}cropped_to_video"
                ],
                "load_wavefile_data": {
                    "library": "scipy",
                    "conditional_arg": [
                        f"_ch{self.input_parameter_dict['find_audio_sync_trains']['sync_ch_receiving_input']:02d}"
                    ],
                },
            }
        ).load_wavefile_data()
        ...
        with open(json_loc) as camera_count_json_file:
            ...
            camera_fr = [
                value[1]
                for key, value in camera_fr_count_dict.items()
                if key
                in self.input_parameter_dict["find_video_sync_trains"][
                    "sync_camera_serial_num"
                ]
        ...
        video_ipi_start_frames, video_sync_sequence_dict = self.find_video_sync_trains(
            total_frame_number=total_frame_number, camera_fps=camera_fr
        )
        video_sync_sequence_array = np.array(list(video_sync_sequence_dict.values()))

        # find NIDQ sync trains
        nidq_file = next(
            pathlib.Path(self.root_directory).glob(f"**{os.sep}*.nidq.bin"), None
        )
        nidq_ipi_data_file = (
            f"{self.root_directory}{os.sep}sync{os.sep}nidq_ipi_data.npy"
        )
        if nidq_file is not None and not os.path.isfile(nidq_ipi_data_file):
            nidq_recording = np.memmap(
                filename=nidq_file, mode="r", dtype=np.int16, order="C"
            )
            nidq_sample_num = (
                nidq_recording.shape[0]
                // self.input_parameter_dict["find_audio_sync_trains"][
                    "nidq_num_channels"
                ]
            )
            nidq_digital_ch = nidq_recording.reshape(
                (
                    self.input_parameter_dict["find_audio_sync_trains"][
                        "nidq_num_channels"
                    ],
            ...
            nidq_digital_bits = (
                (nidq_digital_ch & (2 ** np.arange(16).reshape([1, 16])))
                .astype(bool)
                .astype(int)
            )

            # find start/end of recording
            triggerbox_bit_changes = np.where(
                (
                    nidq_digital_bits[
                        1:,
                        self.input_parameter_dict["find_audio_sync_trains"][
                            "nidq_triggerbox_input_bit_position"
                        ],
                    ]
                    - nidq_digital_bits[
                        :-1,
            ...
            largest_break_end_hop_sec = round(
                (
                    triggerbox_bit_changes[largest_break_end_hop]
                    - triggerbox_bit_changes[largest_break_end_hop - 1]
                )
                / self.input_parameter_dict["find_audio_sync_trains"]["nidq_sr"],
                3,
            )
            self.message_output(
                f"For NIDQ, the largest break in video frame recording is {largest_break_end_hop_sec} seconds."
            )

            loopbio_start_nidq_sample = int(
                triggerbox_bit_changes[largest_break_end_hop] + 1
            )
            loopbio_end_nidq_sample = int(
                triggerbox_bit_changes[largest_break_end_hop + total_frame_number] + 1
            )
            nidq_rec_duration = (
                loopbio_end_nidq_sample - loopbio_start_nidq_sample
            ) / self.input_parameter_dict["find_audio_sync_trains"]["nidq_sr"]
            nidq_video_difference = nidq_rec_duration - total_video_time_least
            self.message_output(
                f"For NIDQ, video recording starts at {loopbio_start_nidq_sample} NIDQ sample and ends at {loopbio_end_nidq_sample} NIDQ sample, giving a total NIDQ duration of {nidq_rec_duration:.4f}, which is {nidq_video_difference:.4f} off relative to video duration."
            )

            # find NIDQ IPI starts and durations in milliseconds
            nidq_rec_ = nidq_digital_bits[
                loopbio_start_nidq_sample:loopbio_end_nidq_sample,
                self.input_parameter_dict["find_audio_sync_trains"][
                    "nidq_sync_input_bit_position"
                ],
            ].copy()
            ...
            if ipi_start_samples[0] < ipi_end_samples[0]:
                if ipi_start_samples.size == ipi_end_samples.size:
                    nidq_ipi_durations_ms = (
                        ((ipi_end_samples - ipi_start_samples) + 1)
                        * 1000
                        / self.input_parameter_dict["find_audio_sync_trains"]["nidq_sr"]
                    )
                ...
                nidq_ipi_durations_ms = (
                    ((ipi_end_samples[1:] - ipi_start_samples) + 1)
                    * 1000
                    / self.input_parameter_dict["find_audio_sync_trains"]["nidq_sr"]
                )
                nidq_ipi_start_samples = ipi_start_samples
        ...
        for af_idx, audio_file in enumerate(sorted(wave_data_dict.keys())):
            ...
            self.message_output(
                f"Working on sync data in audio file: {audio_file[:-4]}"
            )
            smart_wait(app_context_bool=self.app_context_bool, seconds=1)

            ipi_durations_ms, audio_ipi_start_samples = self.find_ipi_intervals(
                sound_array=wave_data_dict[audio_file]["wav_data"],
            ...
            if af_idx == 0:
                ...
                audio_devices_start_sample_differences = (
                    audio_devices_start_sample_differences - audio_ipi_start_samples
                )
                ...
                for video_idx, video_key in enumerate(video_sync_sequence_dict.keys()):
                    if video_idx == 0:
                        diff_array = np.absolute(
                            np.round(ipi_durations_ms)
                            - video_sync_sequence_dict[video_key]
                        )
                        bool_condition_array = (
                            diff_array
                            <= self.input_parameter_dict["find_video_sync_trains"][
                                "millisecond_divergence_tolerance"
                            ]
                        )
                        if not np.all(bool_condition_array):
                            self.message_output(
                                f"IPI sequence match NOT found in audio file! There is/are {(~bool_condition_array).sum()} difference(s) larger "
                                f"than the tolerance and the largest one is {diff_array.max()} ms"
                            )
                        else:
                            video_metadata_search = next(
                                pathlib.Path(
                                    f"{self.root_directory}{os.sep}video{os.sep}"
                                ).glob(f"*.{video_key}{os.sep}metadata.yaml"),
                                None,
                            )
                            if video_metadata_search:
                                img_store = new_for_filename(str(video_metadata_search))
                                frame_times = np.array(
                                    img_store.get_frame_metadata()["frame_time"]
                                )
                                frame_times = frame_times - frame_times[0]
                                video_ipi_start_times = frame_times[
                                    video_ipi_start_frames
                                ]

                            if (
                                video_metadata_search
                                and self.input_parameter_dict["find_audio_sync_trains"][
                                    "extract_exact_video_frame_times_bool"
                                ]
                            ):
                                audio_video_ipi_discrepancy_ms = (
                                    (
                                        audio_ipi_start_samples
                                        / wave_data_dict[audio_file]["sampling_rate"]
                                    )
                                    - video_ipi_start_times
                                ) * 1000
                            else:
                                ...
                                audio_video_ipi_discrepancy_ms = (
                                    ...
                                    - (video_ipi_start_frames / camera_fr[0])
                                ) * 1000
                                ...
                                if next(
                                    pathlib.Path(
                                        f"{self.root_directory}{os.sep}sync{os.sep}"
                                    ).glob(
                                        f"*{audio_device_prefixes[af_idx]}_video_frames_in_audio_samples.txt"
                                    ),
                                    None,
                                ):
                                    with open(
                                        f"{self.root_directory}{os.sep}sync{os.sep}{audio_device_prefixes[af_idx]}_video_frames_in_audio_samples.txt"
                                    ) as txt_file:
                                        video_fr_starts_in_samples = np.array(
                                            [line.rstrip() for line in txt_file],
                                            dtype=np.int64,
                                        )
                                    ...
                                    for ipi_start_sample in audio_ipi_start_samples:
                                        temp_arr = (
                                            video_fr_starts_in_samples
                                            - ipi_start_sample
                                        )
                                        audio_ipi_start_frames.append(
                                            list(temp_arr).index(
                                                max(temp_arr[temp_arr < 0])
                                            )
                                        )

                                    discrepancy_arr = (
                                        np.array(audio_ipi_start_frames)
                                        - video_ipi_start_frames
                                    )
                                    self.message_output(
                                        f"On device {audio_device_prefixes[af_idx]}, the first IPI event had a {discrepancy_arr[0]} fr discrepancy, and the last one had a {discrepancy_arr[-1]} fr discrepancy."
                                    )
                                    self.message_output(
                                        f"Overall, the min discrepancy is {np.min(discrepancy_arr)} fr and the max discrepancy is {np.max(discrepancy_arr)} fr."
                                    )

                            # if the SYNC is acceptable, delete the original audio files
                            if (
                                np.max(np.abs(audio_video_ipi_discrepancy_ms))
                                < self.input_parameter_dict["find_video_sync_trains"][
                                    "millisecond_divergence_tolerance"
                                ]
                            ):
                                if os.path.exists(
                                    f"{self.root_directory}{os.sep}audio{os.sep}original"
                                ):
                                    shutil.rmtree(
                                        f"{self.root_directory}{os.sep}audio{os.sep}original"
                                    )
                            ...
                            if nidq_file is not None and os.path.isfile(
                                nidq_ipi_data_file
                            ):
                                ...
                                ipi_discrepancy_dict[audio_file[:-4]][
                                    ...
                                    * 1000
                                ) - ((video_ipi_start_frames / camera_fr[0]) * 1000)
                                ipi_discrepancy_dict[audio_file[:-4]][
                                    "nidq_ipi_start_samples"
                                ] = nidq_data_arr[1, :]

            else:
                self.message_output(
                    "The IPI sequences on different videos do not match."
                )

        # check if the audio devices match on IPI start samples
        audio_devices_start_sample_differences = np.abs(
            audio_devices_start_sample_differences
        )
        self.message_output(
            f"The smallest IPI start sample difference across master/slave audio devices is {np.nanmin(audio_devices_start_sample_differences)}, "
    ...
    def crop_wav_files_to_video(self) -> None:
        ...
        self.message_output(
            f"Cropping WAV files to video started at: {datetime.now().hour:02d}:{datetime.now().minute:02d}.{datetime.now().second:02d}"
        )
        smart_wait(app_context_bool=self.app_context_bool, seconds=1)
        ...
        wave_data_dict = DataLoader(
            input_parameter_dict={
                ...
                "load_wavefile_data": {
                    "library": "scipy",
                    "conditional_arg": [
                        f"_ch{self.input_parameter_dict['crop_wav_files_to_video']['triggerbox_ch_receiving_input']:02d}"
                    ],
                },
            }
        ).load_wavefile_data()
        ...
        if (
            self.input_parameter_dict["crop_wav_files_to_video"][
        ...
        start_end_video = {
            device: {
                ...
                "largest_break_duration": 0,
                "duration_samples": 0,
                "duration_seconds": 0,
                "audio_tracking_diff_seconds": 0,
            }
            ...
            for audio_file in wave_data_dict.keys():
                if f"{device}_" in audio_file:
                    (
                        ...
                        relevant_array=wave_data_dict[audio_file]["wav_data"],
                        lsb_bool=True,
                        total_frame_number=total_frame_number,
                    )
                    ...
                    if not os.path.isfile(
                        f"{self.root_directory}{os.sep}sync{os.sep}{device}_video_frames_in_audio_samples.txt"
                    ):
                        with open(
                            f"{self.root_directory}{os.sep}sync{os.sep}{device}_video_frames_in_audio_samples.txt",
                            "w",
                            ...
                            for fr in range(total_frame_number):
                                text_file.write(
                                    f"{int(ttl_break_end_samples[largest_break_end_hop + fr] + 1 - int(ttl_break_end_samples[largest_break_end_hop] + 1))}"
                                    + "\n"
                                )

                    start_end_video[device]["duration_samples"] = int(
                        start_end_video[device]["end_last_recorded_frame"]
                        - start_end_video[device]["start_first_recorded_frame"]
                        + 1
                    )
                    start_end_video[device]["duration_seconds"] = round(
                        start_end_video[device]["duration_samples"]
                        / wave_data_dict[audio_file]["sampling_rate"],
                        4,
                    )
                    start_end_video[device]["audio_tracking_diff_seconds"] = round(
                        start_end_video[device]["duration_seconds"] - total_video_time,
                        4,
                    )

                    self.message_output(
                        f"On {device} device, the largest break duration lasted {start_end_video[device]['largest_break_duration'] / wave_data_dict[audio_file]['sampling_rate']:.3f} seconds, "
        ...
        with open(
            f"{self.root_directory}{os.sep}audio{os.sep}audio_triggerbox_sync_info.json",
            "w",
        ) as audio_dict_outfile:
            json.dump(start_end_video, audio_dict_outfile, indent=4)
        new_directory_cropped_files = (
            f"{self.root_directory}{os.sep}audio{os.sep}cropped_to_video"
        )
        ...
        all_audio_files = sorted(
            glob.glob(
                f"{self.root_directory}{os.sep}audio{os.sep}original{os.sep}*.wav"
            )
        )
        ...
        if len(device_ids) > 1:
            if (
                start_end_video["m"]["duration_samples"]
                > start_end_video["s"]["duration_samples"]
            ):
                m_longer = True
                m_original_arr_indices = np.arange(
                    0, start_end_video["m"]["duration_samples"]
                )
                m_new_arr_indices = np.linspace(
                    start=0,
                    stop=start_end_video["m"]["duration_samples"] - 1,
                    num=start_end_video["s"]["duration_samples"],
                )
                base_name_date = next(
                    key
                    for key in wave_data_dict.keys()
                    if "s_" in key
                    and f"_ch{self.input_parameter_dict['crop_wav_files_to_video']['triggerbox_ch_receiving_input']:02d}"
                    in key
                )[2:-9]
            if (
                start_end_video["m"]["duration_samples"]
                ...
                s_longer = True
                s_original_arr_indices = np.arange(
                    0, start_end_video["s"]["duration_samples"]
                )
                s_new_arr_indices = np.linspace(
                    start=0,
                    stop=start_end_video["s"]["duration_samples"] - 1,
                    num=start_end_video["m"]["duration_samples"],
                )
                base_name_date = next(
                    key
                    for key in wave_data_dict.keys()
        ...
        for audio_file in all_audio_files:
            if len(device_ids) == 1:
                ...
                start_cut_sample = start_end_video[device_ids[0]][
                    "start_first_recorded_frame"
                ]
                cut_duration_samples = start_end_video[device_ids[0]][
                    "duration_samples"
                ]
                cut_audio_subp = subprocess.Popen(
                    args=f"""{self.command_addition}sox {os.path.basename(audio_file)} {outfile_loc} trim {start_cut_sample}s {cut_duration_samples}s""",
                ...
                if m_longer:
                    ...
                    modified_base_name = (
                        default_base_name[:2]
                        + base_name_date
                        + default_base_name[2 + len(base_name_date) :]
                    )
                    ...
                    tempo_adjustment_factor = (
                        start_end_video["m"]["duration_samples"]
                        / start_end_video["s"]["duration_samples"]
                    )
                    cut_audio_subp = subprocess.Popen(
                        args=f"""{self.command_addition}sox {os.path.basename(audio_file)} {outfile_loc} trim {m_start_cut_sample}s {m_cut_duration_samples}s tempo -s {tempo_adjustment_factor}""",
                        stdout=subprocess.DEVNULL,
                        stderr=subprocess.STDOUT,
                        cwd=f"{self.root_directory}{os.sep}audio{os.sep}original",
                        shell=self.shell_usage_bool,
                    ...
                    outfile_loc = f"{self.root_directory}{os.sep}audio{os.sep}cropped_to_video{os.sep}{os.path.basename(audio_file)[:-4]}_cropped_to_video.wav"
                    cut_audio_subp = subprocess.Popen(
                        args=f"""{self.command_addition}sox {os.path.basename(audio_file)} {outfile_loc} trim {m_start_cut_sample}s {m_cut_duration_samples}s""",
                        stdout=subprocess.DEVNULL,
                        stderr=subprocess.STDOUT,
                        cwd=f"{self.root_directory}{os.sep}audio{os.sep}original",
                        shell=self.shell_usage_bool,
                ...
                if s_longer:
                    ...
                    cut_audio_subp = subprocess.Popen(
                        args=f"""{self.command_addition}sox {os.path.basename(audio_file)} {outfile_loc} trim {s_start_cut_sample}s {s_cut_duration_samples}s tempo -s {tempo_adjustment_factor}""",
                        stdout=subprocess.DEVNULL,
                        stderr=subprocess.STDOUT,
                        cwd=f"{self.root_directory}{os.sep}audio{os.sep}original",
                        shell=self.shell_usage_bool,
                    ...
                    outfile_loc = f"{self.root_directory}{os.sep}audio{os.sep}cropped_to_video{os.sep}{os.path.basename(audio_file)[:-4]}_cropped_to_video.wav"
                    cut_audio_subp = subprocess.Popen(
                        args=f"""{self.command_addition}sox {os.path.basename(audio_file)} {outfile_loc} trim {s_start_cut_sample}s {s_cut_duration_samples}s""",
                        stdout=subprocess.DEVNULL,
                        stderr=subprocess.STDOUT,
                        cwd=f"{self.root_directory}{os.sep}audio{os.sep}original",
                        shell=self.shell_usage_bool,
                ...
                if "m_" in audio_file:
                    if m_longer:
                        ...
                        m_lsb_original = (
                            m_data_original[
                                start_end_video["m"][
                                    "start_first_recorded_frame"
                                ] : start_end_video["m"]["end_last_recorded_frame"] + 1
                            ]
                            & 1
                        )

                        # resample the LSB data
                        m_lsb_modified = np.where(
                            np.interp(
                                x=m_new_arr_indices,
                                xp=m_original_arr_indices,
                                fp=m_lsb_original,
                            ).astype(np.int16)
                            > 0.5,
                            1,
                        ...
                        m_sr_tempo_adjusted, m_data_tempo_adjusted = wavfile.read(
                            f"{outfile_loc}"
                        )
                        if (
                            m_data_tempo_adjusted.size
                            == start_end_video["s"]["duration_samples"]
                        ):
                            m_data_modified = (
                                m_data_tempo_adjusted & ~1
                            ) ^ m_lsb_modified
                        elif (
                            m_data_tempo_adjusted.size
                            > start_end_video["s"]["duration_samples"]
                            ...
                            padding = np.full(
                                padding_needed,
                                value_for_padded_part,
                                dtype=m_data_tempo_adjusted.dtype,
                            )
                            padded_data = np.concatenate(
                                (m_data_tempo_adjusted, padding)
                            )
                            lsb_value_for_padded_part = m_lsb_modified[-1]
                            lsb_padding = np.full(
                                padding_needed,
                                lsb_value_for_padded_part,
                                dtype=m_lsb_modified.dtype,
                            )
                            extended_lsb_array = np.concatenate(
                                (m_lsb_modified, lsb_padding)
                            )
                            m_data_modified = (padded_data & ~1) ^ extended_lsb_array

                        wavfile.write(
                            filename=outfile_loc,
                            rate=m_sr_original,
                            data=m_data_modified,
                        )
                elif s_longer:
                    # adjust outfile name
                    default_base_name = os.path.basename(audio_file)[:-4]
                    ...
                    s_lsb_original = (
                        s_data_original[
                            start_end_video["s"][
                                "start_first_recorded_frame"
                            ] : start_end_video["s"]["end_last_recorded_frame"] + 1
                        ]
                        & 1
                    )

                    # resample the LSB data
                    s_lsb_modified = np.where(
                        np.interp(
                            x=s_new_arr_indices,
                            xp=s_original_arr_indices,
                            fp=s_lsb_original,
                        ).astype(np.int16)
                        > 0.5,
                        1,
                    ...
                    # load data again and overwrite the LSB
                    s_sr_tempo_adjusted, s_data_tempo_adjusted = wavfile.read(
                        f"{outfile_loc}"
                    )
                    if (
                        s_data_tempo_adjusted.size
                        == start_end_video["m"]["duration_samples"]
                    ):
                        ...
                        padding = np.full(
                            padding_needed,
                            value_for_padded_part,
                            dtype=s_data_tempo_adjusted.dtype,
                        )
                        ...
                        lsb_padding = np.full(
                            padding_needed,
                            lsb_value_for_padded_part,
                            dtype=s_lsb_modified.dtype,
                        )
                        extended_lsb_array = np.concatenate(
                            (s_lsb_modified, lsb_padding)
                        )
                        s_data_modified = (padded_data & ~1) ^ extended_lsb_array

                    wavfile.write(
                        filename=outfile_loc, rate=s_sr_original, data=s_data_modified
                    )

        # create HPSS directory
        pathlib.Path(f"{self.root_directory}{os.sep}audio{os.sep}hpss").mkdir(
