...
class Vocalocator:
    ...
    def prepare_for_vocalocator(self) -> None:
        ...
        self.message_output(
            f"Preparing data for vocal assignment started at: {datetime.now().hour:02d}:{datetime.now().minute:02d}.{datetime.now().second:02d}"
        )
        smart_wait(app_context_bool=self.app_context_bool, seconds=1)

        audio_file_path = next(
            pathlib.Path(f"{self.root_directory}{os.sep}audio{os.sep}").glob(
                f"**{os.sep}*_concatenated_audio_*.mmap"
            ),
            None,
        )
        usv_segments_path = next(
            pathlib.Path(f"{self.root_directory}{os.sep}audio{os.sep}").glob(
                "*_usv_summary.csv"
            ),
            None,
        )
        track_file_path = next(
            pathlib.Path(f"{self.root_directory}{os.sep}video{os.sep}").glob(
                f"**{os.sep}[!speaker]*_points3d_translated_rotated_metric.h5"
            ),
            None,
        )
        arena_info_path = next(
            pathlib.Path(
                f"{self.input_parameter_dict['anipose_operations']['ConvertTo3D']['conduct_anipose_triangulation']['calibration_file_loc']}{os.sep}video{os.sep}"
            ).glob(f"**{os.sep}[!speaker]*_points3d_translated_rotated_metric.h5"),
            None,
        )

        video_frame_count_file_path = next(
            pathlib.Path(f"{self.root_directory}{os.sep}video{os.sep}").glob(
                f"**{os.sep}*_camera_frame_count_dict.json"
            ),
            None,
        )
        with open(video_frame_count_file_path) as frame_count_infile:
            video_frame_rate = json.load(frame_count_infile)[
                "median_empirical_camera_sr"
            ]

        output_path = pathlib.Path(
            f"{self.root_directory}{os.sep}audio{os.sep}sound_localization"
        )
        ...
        usv_onsets_in_samples = (usv_segments[:, 0] * audio_file_sample_rate).astype(
            int
        )
        usv_offsets_in_samples = (usv_segments[:, 1] * audio_file_sample_rate).astype(
            int
        )
        ...
        audio = [
            ...
            for onset, offset in tqdm(
                zip(usv_onsets_in_samples, usv_offsets_in_samples, strict=False),
    ...
    def run_vocalocator(self) -> None:
        ...
        self.message_output(
            f"Vocalization assignment started at: {datetime.now().hour:02d}:{datetime.now().minute:02d}.{datetime.now().second:02d}"
        )
        smart_wait(app_context_bool=self.app_context_bool, seconds=1)

        vcl_conda_name = self.input_parameter_dict["vocalocator"]["vcl_conda_env_name"]
        model_directory = self.input_parameter_dict["vocalocator"][
            "vcl_model_directory"
        ]
        ...
        usv_summary_file_path = next(
            pathlib.Path(f"{self.root_directory}{os.sep}audio{os.sep}").glob(
                f"**{os.sep}*_usv_summary.csv"
            ),
            None,
        )
        ...
        subprocess.run(
            args=f"""{command_addition}conda activate {vcl_conda_name} && python -m vocalocator.assess --config {model_config_path} --data {data_file_path} --inference -o {output_file_path}""",
        ...
        conf_sets, conf_sets_noangle, pdfs = get_conf_sets_6d(
            raw_output, arena_dims, 1.0, True
        )
        ...
        self.message_output(
            f"Vocalization percentage attributed to NO mouse: {none_in_set.sum()}"
        )
        self.message_output(
            f"Vocalization percentage attributed to ONE mouse: {one_in_set.sum()}"
        )
        self.message_output(
            f"Vocalization percentage attributed to mouse #1: {mouse_one_vocalizations.sum()}"
        )
        self.message_output(
            f"Vocalization percentage attributed to mouse #2: {mouse_two_vocalizations.sum()}"
        )
        self.message_output(
            f"Vocalization percentage attributed to BOTH mice: {two_in_set.sum()}"
        )

        np.save(
            f"{self.root_directory}{os.sep}audio{os.sep}sound_localization{os.sep}assessment_assn.npy",
            assignments,
        )
        ...
        sound_loc_assignment_arr = np.load(
            f"{self.root_directory}{os.sep}audio{os.sep}sound_localization{os.sep}assessment_assn.npy"
        )
        ...
        usv_summary_df.write_csv(
            file=usv_summary_file_path, separator=",", include_header=True
        )
