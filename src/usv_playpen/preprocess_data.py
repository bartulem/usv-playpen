...
class Stylist:
    def __init__(
        ...
        if root_directories is None:
            with open(
                ...
                self.root_directories = json.load(json_file)["preprocess_data"][
                    "root_directories"
                ]
        else:
    ...
    def prepare_data_for_analyses(self) -> None:
        ...
        Messenger(
            ...
            receivers=self.input_parameter_dict["send_email"]["Messenger"][
                "send_message"
            ]["receivers"],
            exp_settings_dict=self.exp_settings_dict,
        ).send_message(
            subject=f"{self.input_parameter_dict['send_email']['Messenger']['processing_pc_choice']} PC is busy, do NOT attempt to remote in!",
        ...
        if (
            self.input_parameter_dict["processing_booleans"][
                "conduct_ephys_file_chaining"
            ]
            or self.input_parameter_dict["processing_booleans"]["split_cluster_spikes"]
            or self.input_parameter_dict["processing_booleans"]["prepare_sleap_cluster"]
        ):
            try:
                self.message_output(
                    f"Preprocessing data started at: "
                    f"{datetime.now().hour:02d}:{datetime.now().minute:02d}.{datetime.now().second:02d}"
                )

                # # # concatenate e-phys files
                if self.input_parameter_dict["processing_booleans"][
                    "conduct_ephys_file_chaining"
                ]:
                    Operator(
                        root_directory=self.root_directories,
                ...
                self.message_output(
                    f"Preprocessing data finished at: "
                    f"{datetime.now().hour:02d}:{datetime.now().minute:02d}.{datetime.now().second:02d}"
                )
        ...
        # analyze data in each root directory separately
            ...
            for one_directory_idx, one_directory in enumerate(self.root_directories):
                try:
                    self.message_output(
                        f"Preprocessing data in {one_directory} started at: "
                        f"{datetime.now().hour:02d}:{datetime.now().minute:02d}.{datetime.now().second:02d}."
                    )
                    ...
                    if self.input_parameter_dict["processing_booleans"][
                        ...
                        Operator(
                            ...
                            conduct_concat=self.input_parameter_dict[
                                "processing_booleans"
                            ]["conduct_video_concatenation"]
                        )
                    ...
                    if self.input_parameter_dict["processing_booleans"][
                        ...
                        if (
                            len(
                                os.listdir(
                                    f"{one_directory}{os.sep}audio{os.sep}original"
                                )
                            )
                            == 0
                        ):
                            Operator(
                                ...
                                input_parameter_dict=self.input_parameter_dict,
                                message_output=self.message_output,
                            ).multichannel_to_channel_audio()

                    # # # crop audio files to match video
                    if self.input_parameter_dict["processing_booleans"][
                        "conduct_audio_cropping"
                    ]:
                        Synchronizer(
                            root_directory=one_directory,
                            input_parameter_dict=self.input_parameter_dict,
                    ...
                    if self.input_parameter_dict["processing_booleans"]["anipose_trm"]:
                        if self.input_parameter_dict["anipose_operations"][
                            "ConvertTo3D"
                        ]["conduct_anipose_triangulation"][
                            "triangulate_arena_points_bool"
                        ] or len(
                            self.input_parameter_dict["anipose_operations"][
                                "ConvertTo3D"
                            ]["translate_rotate_metric"]["experimental_codes"]
                        ) == len(self.root_directories):
                            ConvertTo3D(
                                root_directory=one_directory,
                                input_parameter_dict=self.input_parameter_dict,
                                message_output=self.message_output,
                            ).translate_rotate_metric(session_idx=one_directory_idx)
                        else:
                            self.message_output(
                                "Please provide the experimental code for each session in the root directory, their number does not match."
                            )
                    ...
                    self.message_output(
                        f"Preprocessing data in {one_directory} finished at: "
                        f"{datetime.now().hour:02d}:{datetime.now().minute:02d}.{datetime.now().second:02d}."
                    )
        ...
        Messenger(
            ...
            subject=f"{self.input_parameter_dict['send_email']['Messenger']['processing_pc_choice']} PC is available again, processing has been completed",
...
def concatenate_video_files_cli(ctx, root_directory, **kwargs) -> None:
    ...
    Operator(
        root_directory=root_directory, input_parameter_dict=processing_settings_dict
    ).concatenate_video_files()
...
@click.option(
    ...
    default=None,
    required=False,
    help="Name of the target video file.",
)
@click.option(
    "--extension",
    "encode_video_extension",
    type=str,
    default=None,
    required=False,
    help="Video file extension.",
...
@click.option(
    ...
    default=None,
    required=False,
    help="FFMPEG encoding speed preset.",
)
@click.option(
    "--delete-old-file/--no-delete-old-file",
    "delete_old_file",
    default=None,
    help="Delete the original file after encoding.",
)
@click.option(
    "--conduct-concat/--no-conduct-concat",
    "conduct_concat",
    default=None,
    help="Indicate if prior concatenation was performed",
)
@click.pass_context
...
@click.option(
    ...
    type=click.Choice(["both", "m", "r"], case_sensitive=False),
    default=None,
    required=False,
    help="USGH device(s) receiving triggerbox input.",
)
@click.option(
    "--trigger-channel",
    "triggerbox_ch_receiving_input",
    type=int,
    default=None,
    required=False,
    help="USGH channel receiving triggerbox input.",
)
@click.pass_context
def crop_wav_files_to_video_cli(ctx, root_directory, **kwargs) -> None:
    ...
    processing_settings_dict = modify_settings_json_for_cli(
        ctx=ctx, provided_params=provided_params, settings_dict="processing_settings"
    )
...
@click.option(
    ...
    default=None,
    required=False,
    help="Camera serial number for extra data.",
)
@click.option(
    "--audio-sync-ch",
    "sync_ch_receiving_input",
    type=int,
    default=None,
    required=False,
    help="Audio channel receiving sync input.",
)
@click.option(
    "--exact-frame-times/--no-exact-frame-times",
    "extract_exact_video_frame_times_bool",
    default=None,
    help="Extract exact video frame times.",
)
@click.option(
    "--nidq-sr",
    type=float,
...
@click.option(
    ...
    default=None,
    required=False,
    help="Number of NI-DAQ channels.",
)
@click.option(
    "--nidq-trigger-bit",
    ...
    default=None,
    required=False,
    help="NI-DAQ triggerbox input bit position.",
)
@click.option(
    "--nidq-sync-bit",
    ...
    default=None,
    required=False,
    help="NI-DAQ sync input bit position.",
)
@click.option(
    "--video-sync-camera",
    ...
    default=None,
    required=False,
    help="Camera serial number for video sync.",
)
@click.option(
    "--led-version",
    "led_px_version",
    ...
    default=None,
    required=False,
    help="Version of the LED pixel used for sync.",
)
@click.option(
    "--led-dev",
    "led_px_dev",
    ...
    default=None,
    required=False,
    help="LED pixel deviation value.",
)
@click.option(
    "--video-extension",
    ...
    default=None,
    required=False,
    help="Video extension for sync files.",
)
@click.option(
    "--intensity-thresh",
    ...
    default=None,
    required=False,
    help="Relative intensity threshold for LED detection.",
)
@click.option(
    "--ms-tolerance",
    ...
    default=None,
    required=False,
    help="Divergence tolerance (in ms).",
)
@click.pass_context
...
@click.option(
    "--file-type",
    "npx_file_type",
    type=click.Choice(["ap", "lf"], case_sensitive=False),
    default=None,
    required=False,
    help="Neuropixels file type (ap or lf).",
)
@click.option(
    "--tolerance",
...
@click.option(
    ...
    default=None,
    required=False,
    help="STFT window length and hop size.",
)
@click.option(
    "--kernel-size",
    "kernel_size",
    ...
    default=None,
    required=False,
    help="Median filter kernel size (harmonic, percussive).",
)
@click.option(
    "--power",
    "hpss_power",
...
@click.option(
    ...
    default=None,
    required=False,
    help="HPSS margin (harmonic, percussive).",
)
@click.pass_context
...
@click.option(
    ...
    default=None,
    required=False,
    help="Directory/ies containing files to filter.",
)
@click.option(
    "--freq-bounds",
    "filter_freq_bounds",
    ...
    default=None,
    required=False,
    help="Frequency bounds for the band-pass filter (Hz).",
)
@click.pass_context
...
@click.option(
    ...
    default=None,
    required=False,
    help="Directory/ies to search for files to concatenate.",
)
@click.pass_context
...
@click.option(
    ...
    "board_provided_bool",
    is_flag=True,
    help="Indicate that the calibration board is provided.",
)
@click.option(
    "--board-dims",
    "board_xy",
    ...
    default=None,
    required=False,
    help="Checkerboard dimensions (squares_x, squares_y).",
)
@click.option(
    "--square-len",
    type=int,
    default=None,
    required=False,
    help="Length of a checkerboard square (mm).",
)
@click.option(
    "--marker-params",
    "marker_len_bits",
    ...
    default=None,
    required=False,
    help="ArUco marker length (mm) and dictionary bits.",
)
@click.option(
    "--dict-size",
    type=int,
    default=None,
    required=False,
    help="Size of the ArUco dictionary.",
)
@click.option(
    "--img-dims",
    "img_width_height",
    ...
    default=None,
    required=False,
    help="Image dimensions (width, height) in pixels.",
)
@click.pass_context
...
@click.option(
    ...
    default=None,
    required=True,
    help="Path to the Anipose calibration session.",
)
@click.option(
    "--arena-points/--no-arena-points",
    "triangulate_arena_points_bool",
    default=None,
    help="Triangulate arena points instead of animal points.",
)
@click.option(
    "--frame-restriction",
    type=int,
    default=None,
    required=False,
    help="Restrict triangulation to a specific number of frames.",
)
@click.option(
    "--exclude-views",
    "excluded_views",
    ...
    default=None,
    required=False,
    help="Camera views to exclude from triangulation.",
)
@click.option(
    "--display-progress/--no-display-progress",
    "display_progress_bool",
    default=None,
    help="Display the progress bar during triangulation.",
)
@click.option(
    "--use-ransac/--no-use-ransac",
    "ransac_bool",
    default=None,
    help="Use RANSAC for robust triangulation.",
)
@click.option(
    "--rigid-constraint",
    ...
    default=None,
    required=False,
    help="Pair(s) of nodes for a rigid constraint.",
)
@click.option(
    "--weak-constraint",
    ...
    default=None,
    required=False,
    help="Pair(s) of nodes for a weak constraint.",
)
@click.option(
    "--smooth-scale",
    type=float,
    default=None,
    required=False,
    help="Scaling factor for smoothing.",
)
@click.option(
    "--weight-weak",
    type=int,
    default=None,
    required=False,
    help="Weight for weak constraints.",
)
@click.option(
    "--weight-rigid",
    type=int,
    default=None,
    required=False,
    help="Weight for rigid constraints.",
)
@click.option(
...
@click.option(
    ...
    default=None,
    required=False,
    help="Number of derivatives to use for smoothing.",
)
@click.pass_context
...
@click.option(
    ...
    default=None,
    required=True,
    help="Path to the original arena session.",
)
@click.option(
    "--save-data-for",
    "save_transformed_data",
    type=click.Choice(["animal", "arena"], case_sensitive=False),
    default=None,
    required=False,
    help="Data to save after transformation.",
)
@click.option(
    "--delete-original/--no-delete-original",
    "delete_original_h5",
    help="If set, deletes the original HDF5 file.",
)
@click.option(
    "--ref-len",
    ...
    default=None,
    required=False,
    help="Length of the static reference object (e.g., arena side).",
)
@click.pass_context
...
@click.option(
    ...
    default=None,
    required=False,
    help="Name of the DAS conda environment.",
)
@click.option(
    "--model-dir",
    "das_model_directory",
    ...
    default=None,
    required=False,
    help="Directory of the DAS model.",
)
@click.option(
    "--model-name",
    "model_name_base",
    ...
    default=None,
    required=False,
    help="Base name of the DAS model.",
)
@click.option(
    "--output-type",
    "output_file_type",
    type=click.Choice(["csv", "hdf5"], case_sensitive=False),
    default=None,
    required=False,
    help="Output file type for DAS predictions.",
)
@click.option(
    "--confidence-thresh",
    ...
    default=None,
    required=False,
    help="Confidence threshold for segment detection.",
)
@click.option(
    "--min-len",
    "segment_minlen",
    ...
    default=None,
    required=False,
    help="Minimum length for a detected segment (s).",
)
@click.option(
    "--fill-gap",
    "segment_fillgap",
    ...
    default=None,
    required=False,
    help="Gap duration to fill between segments (s).",
)
@click.pass_context
...
@click.option(
    ...
    default=None,
    required=False,
    help="Window length of the signal.",
)
@click.option(
    "--freq-cutoff",
    "low_freq_cutoff",
...
@click.option(
    ...
    default=None,
    required=False,
    help="Minimum noise correlation cutoff.",
)
@click.option(
    "--var-cutoff",
    ...
    default=None,
    required=False,
    help="Maximum noise variance cutoff.",
)
@click.pass_context
...
@click.option(
    ...
    type=str,
    required=True,
    help="Comma-separated string of session root directory paths.",
)
def concatenate_binary_files_cli(root_directories):
    ...
    if len(valid_dirs) > 0:
        Operator(root_directory=valid_dirs).concatenate_binary_files()
...
@click.option(
    ...
    type=str,
    required=True,
    help="A comma-separated string of session root directory paths.",
)
@click.option(
    "--min-spikes",
    "min_spike_num",
    ...
    default=None,
    required=False,
    help="Minimum number of spikes for a cluster to be saved.",
)
@click.option(
    "--kilosort-version",
    type=str,
    default=None,
    required=False,
    help="Version of Kilosort used for spike sorting.",
)
@click.pass_context
def split_clusters_to_sessions_cli(ctx, root_directories, **kwargs):
    ...
    if len(valid_dirs) > 0:
        Operator(
            root_directory=valid_dirs, input_parameter_dict=processing_settings_dict
        ).split_clusters_to_sessions()
