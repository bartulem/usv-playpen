...
from PyQt6.QtGui import QFont, QFontDatabase, QGuiApplication, QIcon, QPainter, QPixmap
...
def replace_name_in_path(
    ...
    if any(
        [
            ...
            for loc in recording_files_destinations
        ]
    ):
        revised_recording_files_destination_win = []
        for path, name_in_path in zip(
            recording_files_destinations,
            [
                name
                for name in experimenter_list
                for loc in recording_files_destinations
            ...
            revised_recording_files_destination_win.append(
                old_name_path[: name_span[0]] + exp_id + old_name_path[name_span[1] :]
            )
...
class USVPlaypenWindow(QMainWindow):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        font_file_loc = QFontDatabase.addApplicationFont(
            f"{basedir}{os.sep}fonts{os.sep}segoeui.ttf"
        )
    ...
    def main_window(self) -> None:
        ...
        self.exp_settings_dict = toml.load(
            Path(__file__).parent / "_config/behavioral_experiments_settings.toml"
        )

        with open(
            Path(__file__).parent / "_parameter_settings/processing_settings.json"
        ) as process_json_file:
            self.processing_input_dict = json.load(process_json_file)

        with open(
            Path(__file__).parent / "_parameter_settings/analyses_settings.json"
        ) as analyses_json_file:
            self.analyses_input_dict = json.load(analyses_json_file)

        with open(
            Path(__file__).parent / "_parameter_settings/visualizations_settings.json"
        ) as visualizations_json_file:
        ...
        self.exp_id_list = sorted(
            self.exp_settings_dict["experimenter_list"],
            key=lambda x: x == self.exp_id,
            reverse=True,
        )
        ...
        self.exp_id_cb.activated.connect(
            partial(self._combo_box_prior_name, variable_id="exp_id")
        )
    ...
    def record_one(self) -> None:
        ...
        self.setWindowTitle(
            f"{app_name} (Record > Select config directories and set basic parameters)"
        )
        ...
        title_label = QLabel(
            "Please select appropriate directories (with config files or executables in them)",
            self.Record,
        )
        ...
        self.recorder_settings_edit = QLineEdit(
            self.avisoft_rec_dir_global, self.Record
        )
        self.recorder_settings_edit.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        recorder_dir_btn.setStyleSheet(
            "QPushButton { min-width: 64px; min-height: 12px; max-width: 64px; max-height: 13px; }"
        )
        ...
        avisoft_base_dir_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        avisoft_base_dir_label.move(5, 70)
        self.avisoft_base_edit = QLineEdit(self.avisoft_base_dir_global, self.Record)
        self.avisoft_base_edit.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        avisoft_base_dir_btn.setStyleSheet(
            "QPushButton { min-width: 64px; min-height: 12px; max-width: 64px; max-height: 13px; }"
        )
        ...
        coolterm_base_dir_btn.setStyleSheet(
            "QPushButton { min-width: 64px; min-height: 12px; max-width: 64px; max-height: 13px; }"
        )
        ...
        recording_files_destination_linux_label = QLabel(
            "File destination(s) Linux:", self.Record
        )
        recording_files_destination_linux_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        recording_files_destination_linux_label.move(5, 130)
        self.recording_files_destination_linux = QLineEdit(
            self.destination_linux_global, self.Record
        )
        self.recording_files_destination_linux.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        self.recording_files_destination_linux.setStyleSheet(
            "QLineEdit { width: 490px; }"
        )
        self.recording_files_destination_linux.move(220, 130)

        recording_files_destination_windows_label = QLabel(
            "File destination(s) Windows:", self.Record
        )
        recording_files_destination_windows_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        recording_files_destination_windows_label.move(5, 160)
        self.recording_files_destination_windows = QLineEdit(
            self.destination_win_global, self.Record
        )
        self.recording_files_destination_windows.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        self.recording_files_destination_windows.setStyleSheet(
            "QLineEdit { width: 490px; }"
        )
        ...
        conduct_audio_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        conduct_audio_label.move(5, 235)
        self.conduct_audio_cb_list = [
            x
            for _, x in sorted(
                zip(
                    [
                        self.exp_settings_dict["conduct_audio_recording"],
                        not self.exp_settings_dict["conduct_audio_recording"],
                    ],
                    self.boolean_list,
                    strict=False,
                ),
        ...
        self.conduct_audio_cb.activated.connect(
            partial(
                self._combo_box_prior_true
                if self.conduct_audio_cb_list[0] == "Yes"
                else self._combo_box_prior_false,
                variable_id="conduct_audio_cb_bool",
            )
        )
        self.conduct_audio_cb.move(220, 235)

        conduct_tracking_cal_label = QLabel("Conduct VIDEO calibration:", self.Record)
        conduct_tracking_cal_label.setFont(
            QFont(self.font_id, 11 + self.font_size_increase)
        )
        conduct_tracking_cal_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        conduct_tracking_cal_label.move(5, 265)
        self.conduct_tracking_calibration_cb_list = [
            x
            for _, x in sorted(
                zip(
                    [
                        self.exp_settings_dict["conduct_tracking_calibration"],
                        not self.exp_settings_dict["conduct_tracking_calibration"],
                    ],
                    self.boolean_list,
                    strict=False,
                ),
        ...
        self.conduct_tracking_calibration_cb.addItems(
            self.conduct_tracking_calibration_cb_list
        )
        self.conduct_tracking_calibration_cb.setStyleSheet(
            "QComboBox { width: 465px; }"
        )
        self.conduct_tracking_calibration_cb.activated.connect(
            partial(
                self._combo_box_prior_true
                if self.conduct_tracking_calibration_cb_list[0] == "Yes"
                else self._combo_box_prior_false,
                variable_id="conduct_tracking_calibration_cb_bool",
            )
        )
        self.conduct_tracking_calibration_cb.move(220, 265)

        disable_ethernet_label = QLabel("Disable ethernet connection:", self.Record)
        disable_ethernet_label.setFont(
            QFont(self.font_id, 11 + self.font_size_increase)
        )
        disable_ethernet_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        disable_ethernet_label.move(5, 295)
        self.disable_ethernet_cb_list = [
            x
            for _, x in sorted(
                zip(
                    [
                        self.exp_settings_dict["disable_ethernet"],
                        not self.exp_settings_dict["disable_ethernet"],
                    ],
                    self.boolean_list,
                    strict=False,
                ),
        ...
        self.disable_ethernet_cb.activated.connect(
            partial(
                self._combo_box_prior_true
                if self.disable_ethernet_cb_list[0] == "Yes"
                else self._combo_box_prior_false,
                variable_id="disable_ethernet_cb_bool",
            )
        )
        ...
        self.video_session_duration = QLineEdit(
            f"{self.exp_settings_dict['video_session_duration']}", self.Record
        )
        self.video_session_duration.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        self.calibration_session_duration = QLineEdit(
            f"{self.exp_settings_dict['calibration_duration']}", self.Record
        )
        self.calibration_session_duration.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        ethernet_network_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        ethernet_network_label.move(5, 385)
        self.ethernet_network = QLineEdit(
            f"{self.exp_settings_dict['ethernet_network']}", self.Record
        )
        ...
        email_notification_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        ...
        self._create_buttons_record(
            seq=0,
            class_option=self.Record,
            button_pos_y=record_one_y - 35,
            next_button_x_pos=record_one_x - 100,
        )
    ...
    def record_two(self) -> None:
        ...
        self.default_audio_settings = {
            "name": f"{self.exp_settings_dict['audio']['mics_config']['name']}",
            "id": f"{self.exp_settings_dict['audio']['mics_config']['id']}",
            "typech": f"{self.exp_settings_dict['audio']['mics_config']['typech']}",
            "deviceid": f"{self.exp_settings_dict['audio']['mics_config']['deviceid']}",
            "channel": f"{self.exp_settings_dict['audio']['mics_config']['channel']}",
            "gain": f"{self.exp_settings_dict['audio']['mics_config']['gain']}",
            "fullscalespl": f"{self.exp_settings_dict['audio']['mics_config']['fullscalespl']}",
            "triggertype": f"{self.exp_settings_dict['audio']['mics_config']['triggertype']}",
            "toggle": f"{self.exp_settings_dict['audio']['mics_config']['toggle']}",
            "invert": f"{self.exp_settings_dict['audio']['mics_config']['invert']}",
            "ditc": f"{self.exp_settings_dict['audio']['mics_config']['ditc']}",
            "ditctime": self.exp_settings_dict["audio"]["mics_config"]["ditctime"],
            "whistletracking": f"{self.exp_settings_dict['audio']['mics_config']['whistletracking']}",
            "wtbcf": f"{self.exp_settings_dict['audio']['mics_config']['wtbcf']}",
            "wtmaxchange": f"{self.exp_settings_dict['audio']['mics_config']['wtmaxchange']}",
            "wtmaxchange2_": f"{self.exp_settings_dict['audio']['mics_config']['wtmaxchange2_']}",
            "wtminchange": f"{self.exp_settings_dict['audio']['mics_config']['wtminchange']}",
            "wtminchange2_": f"{self.exp_settings_dict['audio']['mics_config']['wtminchange2_']}",
            "wtoutside": f"{self.exp_settings_dict['audio']['mics_config']['wtoutside']}",
            "hilowratioenable": f"{self.exp_settings_dict['audio']['mics_config']['hilowratioenable']}",
            "hilowratio": f"{self.exp_settings_dict['audio']['mics_config']['hilowratio']}",
            "hilowratiofc": f"{self.exp_settings_dict['audio']['mics_config']['hilowratiofc']}",
            "wtslope": f"{self.exp_settings_dict['audio']['mics_config']['wtslope']}",
            "wtlevel": f"{self.exp_settings_dict['audio']['mics_config']['wtlevel']}",
            "wtmindurtotal": f"{self.exp_settings_dict['audio']['mics_config']['wtmindurtotal']}",
            "wtmindur": f"{self.exp_settings_dict['audio']['mics_config']['wtmindur']}",
            "wtmindur2_": f"{self.exp_settings_dict['audio']['mics_config']['wtmindur2_']}",
            "wtholdtime": f"{self.exp_settings_dict['audio']['mics_config']['wtholdtime']}",
            "wtmonotonic": f"{self.exp_settings_dict['audio']['mics_config']['wtmonotonic']}",
            "wtbmaxdur": f"{self.exp_settings_dict['audio']['mics_config']['wtbmaxdur']}",
            "rejectwind": f"{self.exp_settings_dict['audio']['mics_config']['rejectwind']}",
            "rwentropy": f"{self.exp_settings_dict['audio']['mics_config']['rwentropy']}",
            "rwfpegel": f"{self.exp_settings_dict['audio']['mics_config']['rwfpegel']}",
            "rwdutycycle": f"{self.exp_settings_dict['audio']['mics_config']['rwdutycycle']}",
            "rwtimeconstant": f"{self.exp_settings_dict['audio']['mics_config']['rwtimeconstant']}",
            "rwholdtime": f"{self.exp_settings_dict['audio']['mics_config']['rwholdtime']}",
            "fpegel": f"{self.exp_settings_dict['audio']['mics_config']['fpegel']}",
            "energy": f"{self.exp_settings_dict['audio']['mics_config']['energy']}",
            "frange": f"{self.exp_settings_dict['audio']['mics_config']['frange']}",
            "entropyb": f"{self.exp_settings_dict['audio']['mics_config']['entropyb']}",
            "entropy": f"{self.exp_settings_dict['audio']['mics_config']['entropy']}",
            "increment": f"{self.exp_settings_dict['audio']['mics_config']['increment']}",
            "fu": f"{self.exp_settings_dict['audio']['mics_config']['fu']}",
            "fo": f"{self.exp_settings_dict['audio']['mics_config']['fo']}",
            "pretrigger": f"{self.exp_settings_dict['audio']['mics_config']['pretrigger']}",
            "mint": f"{self.exp_settings_dict['audio']['mics_config']['mint']}",
            "minst": f"{self.exp_settings_dict['audio']['mics_config']['minst']}",
            "fhold": f"{self.exp_settings_dict['audio']['mics_config']['fhold']}",
            "logfileno": f"{self.exp_settings_dict['audio']['call']['logfileno']}",
            "groupno": f"{self.exp_settings_dict['audio']['call']['groupno']}",
            "callno": f"{self.exp_settings_dict['audio']['call']['callno']}",
            "timeconstant": f"{self.exp_settings_dict['audio']['monitor']['timeconstant']}",
            "timeexpansion": f"{self.exp_settings_dict['audio']['monitor']['timeexpansion']}",
            "startstop": f"{self.exp_settings_dict['audio']['monitor']['startstop']}",
            "sayf": f"{self.exp_settings_dict['audio']['monitor']['sayf']}",
            "over": f"{self.exp_settings_dict['audio']['monitor']['over']}",
            "delay": f"{self.exp_settings_dict['audio']['monitor']['delay']}",
            "center": f"{self.exp_settings_dict['audio']['monitor']['center']}",
            "bandwidth": f"{self.exp_settings_dict['audio']['monitor']['bandwidth']}",
            "fd": f"{self.exp_settings_dict['audio']['monitor']['fd']}",
            "decimation": f"{self.exp_settings_dict['audio']['monitor']['decimation']}",
            "device": f"{self.exp_settings_dict['audio']['monitor']['device']}",
            "mode": f"{self.exp_settings_dict['audio']['monitor']['mode']}",
            "outfovertaps": f"{self.exp_settings_dict['audio']['devices']['outfovertaps']}",
            "outfoverabtast": f"{self.exp_settings_dict['audio']['devices']['outfoverabtast']}",
            "outformat": f"{self.exp_settings_dict['audio']['devices']['outformat']}",
            "outfabtast": f"{self.exp_settings_dict['audio']['devices']['outfabtast']}",
            "outdeviceid": f"{self.exp_settings_dict['audio']['devices']['outdeviceid']}",
            "outtype": f"{self.exp_settings_dict['audio']['devices']['outtype']}",
            "usghflags": f"{self.exp_settings_dict['audio']['devices']['usghflags']}",
            "diff": f"{self.exp_settings_dict['audio']['devices']['diff']}",
            "format": f"{self.exp_settings_dict['audio']['devices']['format']}",
            "type": f"{self.exp_settings_dict['audio']['devices']['type']}",
            "nbrwavehdr": f"{self.exp_settings_dict['audio']['devices']['nbrwavehdr']}",
            "devbuffer": f"{self.exp_settings_dict['audio']['devices']['devbuffer']}",
            "ntaps": f"{self.exp_settings_dict['audio']['devices']['ntaps']}",
            "filtercutoff": f"{self.exp_settings_dict['audio']['devices']['filtercutoff']}",
            "filter": f"{self.exp_settings_dict['audio']['devices']['filter']}",
            "fabtast": f"{self.exp_settings_dict['audio']['devices']['fabtast']}",
            "y2": f"{self.exp_settings_dict['audio']['screen_position']['y2']}",
            "x2": f"{self.exp_settings_dict['audio']['screen_position']['x2']}",
            "y1": f"{self.exp_settings_dict['audio']['screen_position']['y1']}",
            "x1": f"{self.exp_settings_dict['audio']['screen_position']['x1']}",
            "display": f"{self.exp_settings_dict['audio']['general']['display']}",
            "dcolumns": f"{self.exp_settings_dict['audio']['general']['dcolumns']}",
            "total": f"{self.exp_settings_dict['audio']['general']['total']}",
            "dispspectrogramcontrast": f"{self.exp_settings_dict['audio']['general']['dispspectrogramcontrast']}",
            "disprangespectrogram": f"{self.exp_settings_dict['audio']['general']['disprangespectrogram']}",
            "disprangeamplitude": f"{self.exp_settings_dict['audio']['general']['disprangeamplitude']}",
            "disprangewaveform": f"{self.exp_settings_dict['audio']['general']['disprangewaveform']}",
            "fftlength": f"{self.exp_settings_dict['audio']['general']['fftlength']}",
            "usvmonitoringflags": f"{self.exp_settings_dict['audio']['general']['usvmonitoringflags']}",
            "total_mic_number": f"{self.exp_settings_dict['audio']['total_mic_number']}",
            "total_device_num": f"{self.exp_settings_dict['audio']['total_device_num']}",
            "used_mics": ",".join(
                [str(x) for x in self.exp_settings_dict["audio"]["used_mics"]]
            ),
            "cpu_priority": self.exp_settings_dict["audio"]["cpu_priority"],
            "cpu_affinity": ",".join(
                [str(x) for x in self.exp_settings_dict["audio"]["cpu_affinity"]]
            ),
        }
        ...
        row_counter = 0
        column_counter = 0
        for audio_idx, (audio_attr, audio_value) in enumerate(
            self.default_audio_settings.items()
        ):
            ...
            getattr(self, audio_attr).setFont(
                QFont(self.font_id, 10 + self.font_size_increase)
            )
            getattr(self, audio_attr).move(
                row_start_position_line_edit, 45 + (row_counter * 30)
            )
            if (audio_idx + 1) % 25 == 0:
                column_counter += 1
                row_counter = 0
        ...
        self._create_buttons_record(
            seq=1,
            class_option=self.AudioSettings,
            button_pos_y=record_two_y - 35,
            next_button_x_pos=record_two_x - 100,
        )
    ...
    def record_three(self) -> None:
        ...
        self.browser = QLineEdit(
            self.exp_settings_dict["video"]["general"]["browser"], self.VideoSettings
        )
        ...
        self.expected_cameras = QLineEdit(
            ",".join(self.exp_settings_dict["video"]["general"]["expected_cameras"]),
            self.VideoSettings,
        )
        ...
        self.recording_codec_list = sorted(
            [
                "hq",
                "hq-fast",
                "mq",
            ...
            key=lambda x: x == self.recording_codec,
            reverse=True,
        )
        ...
        self.recording_codec_cb.activated.connect(
            partial(self._combo_box_prior_codec, variable_id="recording_codec")
        )
        ...
        self.monitor_recording_cb_list = [
            x
            for _, x in sorted(
                zip(
                    [
                        self.exp_settings_dict["video"]["general"]["monitor_recording"],
                        not self.exp_settings_dict["video"]["general"][
                            "monitor_recording"
                        ],
                    ],
                    self.boolean_list,
        ...
        self.monitor_recording_cb.activated.connect(
            partial(
                self._combo_box_prior_true
                if self.monitor_recording_cb_list[0] == "Yes"
                else self._combo_box_prior_false,
                variable_id="monitor_recording_cb_bool",
            )
        )
        ...
        self.monitor_specific_camera_cb_list = [
            x
            for _, x in sorted(
                zip(
                    [
                        self.exp_settings_dict["video"]["general"][
                            "monitor_specific_camera"
                        ],
                        not self.exp_settings_dict["video"]["general"][
                            "monitor_specific_camera"
                        ],
        ...
        self.monitor_specific_camera_cb.activated.connect(
            partial(
                self._combo_box_prior_true
                if self.monitor_specific_camera_cb_list[0] == "Yes"
                else self._combo_box_prior_false,
                variable_id="monitor_specific_camera_cb_bool",
            )
        )
        self.monitor_specific_camera_cb.move(160, 160)

        specific_camera_serial_label = QLabel("ONE camera serial:", self.VideoSettings)
        specific_camera_serial_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        specific_camera_serial_label.move(5, 190)
        self.specific_camera_serial = QLineEdit(
            self.exp_settings_dict["video"]["general"]["specific_camera_serial"],
            self.VideoSettings,
        )
        self.specific_camera_serial.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        delete_post_copy_label.setFont(
            QFont(self.font_id, 11 + self.font_size_increase)
        )
        delete_post_copy_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        delete_post_copy_label.move(5, 220)
        self.delete_post_copy_cb_list = [
            x
            for _, x in sorted(
                zip(
                    [
                        self.exp_settings_dict["video"]["general"]["delete_post_copy"],
                        not self.exp_settings_dict["video"]["general"][
                            "delete_post_copy"
                        ],
                    ],
        ...
        self.delete_post_copy_cb.activated.connect(
            partial(
                self._combo_box_prior_true
                if self.delete_post_copy_cb_list[0] == "Yes"
                else self._combo_box_prior_false,
                variable_id="delete_post_copy_cb_bool",
            )
        )
        ...
        self.calibration_frame_rate = QSlider(
            Qt.Orientation.Horizontal, self.VideoSettings
        )
        ...
        self.calibration_frame_rate.setValue(
            self.exp_settings_dict["video"]["general"]["calibration_frame_rate"]
        )
        ...
        self.cameras_frame_rate.setValue(
            self.exp_settings_dict["video"]["general"]["recording_frame_rate"]
        )
        ...
        for cam_idx, cam in enumerate(
            self.exp_settings_dict["video"]["general"]["expected_cameras"]
        ):
            self._create_sliders_general(
                camera_id=cam,
                camera_color=camera_colors_global[cam_idx],
                y_start=355 + (cam_idx * 90),
            )
        ...
        self.institution_entry = QLineEdit(
            f"{self.exp_settings_dict['video']['metadata']['institution']}",
            self.VideoSettings,
        )
        self.institution_entry.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        self.laboratory_entry = QLineEdit(
            f"{self.exp_settings_dict['video']['metadata']['laboratory']}",
            self.VideoSettings,
        )
        ...
        self.experimenter_entry.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        self.mice_num_entry = QLineEdit(
            f"{self.exp_settings_dict['video']['metadata']['mice_num']}",
            self.VideoSettings,
        )
        ...
        self.vacant_arena_cb_list = [
            x
            for _, x in sorted(
                zip(
                    [
                        self.exp_settings_dict["video"]["metadata"]["vacant_arena"],
                        not self.exp_settings_dict["video"]["metadata"]["vacant_arena"],
                    ],
                    self.boolean_list,
                    strict=False,
                ),
        ...
        self.vacant_arena_cb.activated.connect(
            partial(
                self._combo_box_prior_true
                if self.vacant_arena_cb_list[0] == "Yes"
                else self._combo_box_prior_false,
                variable_id="vacant_arena_cb_bool",
            )
        )
        ...
        self.ambient_light_cb_list = [
            x
            for _, x in sorted(
                zip(
                    [
                        self.exp_settings_dict["video"]["metadata"]["ambient_light"],
                        not self.exp_settings_dict["video"]["metadata"][
                            "ambient_light"
                        ],
                    ],
                    self.boolean_list,
        ...
        self.ambient_light_cb.activated.connect(
            partial(
                self._combo_box_prior_true
                if self.ambient_light_cb_list[0] == "Yes"
                else self._combo_box_prior_false,
                variable_id="ambient_light_cb_bool",
            )
        )
        ...
        self.record_brain_cb_list = [
            x
            for _, x in sorted(
                zip(
                    [
                        self.exp_settings_dict["video"]["metadata"]["record_brain"],
                        not self.exp_settings_dict["video"]["metadata"]["record_brain"],
                    ],
                    self.boolean_list,
                    strict=False,
                ),
        ...
        self.record_brain_cb.activated.connect(
            partial(
                self._combo_box_prior_true
                if self.record_brain_cb_list[0] == "Yes"
                else self._combo_box_prior_false,
                variable_id="record_brain_cb_bool",
            )
        )
        ...
        self.usv_playback_cb_list = [
            x
            for _, x in sorted(
                zip(
                    [
                        self.exp_settings_dict["video"]["metadata"]["usv_playback"],
                        not self.exp_settings_dict["video"]["metadata"]["usv_playback"],
                    ],
                    self.boolean_list,
                    strict=False,
                ),
        ...
        self.usv_playback_cb.activated.connect(
            partial(
                self._combo_box_prior_true
                if self.usv_playback_cb_list[0] == "Yes"
                else self._combo_box_prior_false,
                variable_id="usv_playback_cb_bool",
            )
        )
        ...
        self.chemogenetics_cb_list = [
            x
            for _, x in sorted(
                zip(
                    [
                        self.exp_settings_dict["video"]["metadata"]["chemogenetics"],
                        not self.exp_settings_dict["video"]["metadata"][
                            "chemogenetics"
                        ],
                    ],
        ...
        self.chemogenetics_cb.activated.connect(
            partial(
                self._combo_box_prior_true
                if self.chemogenetics_cb_list[0] == "Yes"
                else self._combo_box_prior_false,
                variable_id="chemogenetics_cb_bool",
            )
        )
        ...
        self.optogenetics_cb_list = [
            x
            for _, x in sorted(
                zip(
                    [
                        self.exp_settings_dict["video"]["metadata"]["optogenetics"],
                        not self.exp_settings_dict["video"]["metadata"]["optogenetics"],
                    ],
                    self.boolean_list,
                    strict=False,
                ),
        ...
        self.optogenetics_cb.activated.connect(
            partial(
                self._combo_box_prior_true
                if self.optogenetics_cb_list[0] == "Yes"
                else self._combo_box_prior_false,
                variable_id="optogenetics_cb_bool",
            )
        )
        ...
        self.brain_lesion_cb_list = [
            x
            for _, x in sorted(
                zip(
                    [
                        self.exp_settings_dict["video"]["metadata"]["brain_lesion"],
                        not self.exp_settings_dict["video"]["metadata"]["brain_lesion"],
                    ],
                    self.boolean_list,
                    strict=False,
                ),
        ...
        self.brain_lesion_cb.activated.connect(
            partial(
                self._combo_box_prior_true
                if self.brain_lesion_cb_list[0] == "Yes"
                else self._combo_box_prior_false,
                variable_id="brain_lesion_cb_bool",
            )
        )
        ...
        self.devocalization_cb_list = [
            x
            for _, x in sorted(
                zip(
                    [
                        self.exp_settings_dict["video"]["metadata"]["devocalization"],
                        not self.exp_settings_dict["video"]["metadata"][
                            "devocalization"
                        ],
                    ],
        ...
        self.devocalization_cb.activated.connect(
            partial(
                self._combo_box_prior_true
                if self.devocalization_cb_list[0] == "Yes"
                else self._combo_box_prior_false,
                variable_id="devocalization_cb_bool",
            )
        )
        ...
        self.female_urine_cb_list = [
            x
            for _, x in sorted(
                zip(
                    [
                        self.exp_settings_dict["video"]["metadata"]["female_urine"],
                        not self.exp_settings_dict["video"]["metadata"]["female_urine"],
                    ],
                    self.boolean_list,
                    strict=False,
                ),
        ...
        self.female_urine_cb.activated.connect(
            partial(
                self._combo_box_prior_true
                if self.female_urine_cb_list[0] == "Yes"
                else self._combo_box_prior_false,
                variable_id="female_urine_cb_bool",
            )
        )
        ...
        self.female_bedding_cb_list = [
            x
            for _, x in sorted(
                zip(
                    [
                        self.exp_settings_dict["video"]["metadata"]["female_bedding"],
                        not self.exp_settings_dict["video"]["metadata"][
                            "female_bedding"
                        ],
                    ],
        ...
        self.female_bedding_cb.activated.connect(
            partial(
                self._combo_box_prior_true
                if self.female_bedding_cb_list[0] == "Yes"
                else self._combo_box_prior_false,
                variable_id="female_bedding_cb_bool",
            )
        )
        ...
        self.species_entry = QLineEdit(
            f"{self.exp_settings_dict['video']['metadata']['species']}",
            self.VideoSettings,
        )
        ...
        self.strain_entry = QLineEdit(
            f"{self.exp_settings_dict['video']['metadata']['strain']}",
            self.VideoSettings,
        )
        ...
        self.cage_entry = QLineEdit(
            f"{self.exp_settings_dict['video']['metadata']['cage']}", self.VideoSettings
        )
        ...
        self.subject_entry = QLineEdit(
            f"{self.exp_settings_dict['video']['metadata']['subject']}",
            self.VideoSettings,
        )
        ...
        self.dob_entry = QLineEdit(
            f"{self.exp_settings_dict['video']['metadata']['dob']}", self.VideoSettings
        )
        ...
        self.sex_entry = QLineEdit(
            f"{self.exp_settings_dict['video']['metadata']['sex']}", self.VideoSettings
        )
        ...
        self.weight_entry = QLineEdit(
            f"{self.exp_settings_dict['video']['metadata']['weight']}",
            self.VideoSettings,
        )
        ...
        self.housing_entry = QLineEdit(
            f"{self.exp_settings_dict['video']['metadata']['housing']}",
            self.VideoSettings,
        )
        ...
        self.implant_entry = QLineEdit(
            f"{self.exp_settings_dict['video']['metadata']['implant']}",
            self.VideoSettings,
        )
        ...
        self.virus_entry = QLineEdit(
            f"{self.exp_settings_dict['video']['metadata']['virus']}",
            self.VideoSettings,
        )
        ...
        self.notes_entry = QTextEdit(
            f"{self.exp_settings_dict['video']['metadata']['notes']}",
            self.VideoSettings,
        )
        ...
        self._create_buttons_record(
            seq=2,
            class_option=self.VideoSettings,
            button_pos_y=record_three_y - 35,
            next_button_x_pos=record_three_x - 100,
        )
    ...
    def record_four(self) -> None:
        ...
        with open(
            (Path(__file__).parent / "_config/behavioral_experiments_settings.toml"),
            "w",
        ) as updated_toml_file:
            toml.dump(self.exp_settings_dict, updated_toml_file)

        self.run_exp = ExperimentController(
            message_output=self._message,
        ...
        self._create_buttons_record(
            seq=3,
            class_option=self.ConductRecording,
            button_pos_y=record_four_y - 35,
            next_button_x_pos=record_four_x - 100,
        )
    ...
    def process_one(self) -> None:
        ...
        self.processing_dir_edit.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        sleap_conda_label = QLabel(
            "SLEAP conda environment name:", self.ProcessSettings
        )
        sleap_conda_label.setFont(QFont(self.font_id, 12 + self.font_size_increase))
        sleap_conda_label.move(10, 365)
        self.sleap_conda = QLineEdit(
            f"{self.processing_input_dict['anipose_operations']['ConvertTo3D']['sleap_file_conversion']['sleap_conda_env_name']}",
        ...
        self.centroid_model_edit = QLineEdit(
            f"{self.processing_input_dict['prepare_cluster_job']['centroid_model_path']}",
            self.ProcessSettings,
        )
        self.centroid_model_edit.setPlaceholderText("SLEAP centroid model directory")
        self.centroid_model_edit.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        centroid_model_btn.setStyleSheet(
            "QPushButton { min-width: 77px; min-height: 12px; max-width: 77px; max-height: 13px; }"
        )
        self.centroid_model_btn_clicked_flag = False
        centroid_model_btn.clicked.connect(self._open_centroid_dialog)

        self.centered_instance_model_edit = QLineEdit(
            f"{self.processing_input_dict['prepare_cluster_job']['centered_instance_model_path']}",
            self.ProcessSettings,
        )
        self.centered_instance_model_edit.setPlaceholderText(
            "SLEAP centered instance model directory"
        )
        self.centered_instance_model_edit.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        centered_instance_btn.setStyleSheet(
            "QPushButton { min-width: 77px; min-height: 12px; max-width: 77px; max-height: 13px; }"
        )
        ...
        self.inference_root_dir_edit = QLineEdit(
            self.sleap_inference_dir_global, self.ProcessSettings
        )
        self.inference_root_dir_edit.setPlaceholderText("SLEAP inference directory")
        self.inference_root_dir_edit.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        inference_root_dir_btn.setStyleSheet(
            "QPushButton { min-width: 77px; min-height: 12px; max-width: 77px; max-height: 13px; }"
        )
        ...
        self.calibration_file_loc_edit.setPlaceholderText(
            "Tracking calibration / Arena root directory"
        )
        self.calibration_file_loc_edit.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        calibration_file_loc_btn.setFont(
            QFont(self.font_id, 8 + self.font_size_increase)
        )
        calibration_file_loc_btn.move(310, 485)
        calibration_file_loc_btn.setStyleSheet(
            "QPushButton { min-width: 77px; min-height: 12px; max-width: 77px; max-height: 12px; }"
        )
        ...
        self.das_conda = QLineEdit(
            self.processing_input_dict["usv_inference"]["FindMouseVocalizations"][
        ...
        self.das_model_dir_edit = QLineEdit(
            self.das_model_dir_global, self.ProcessSettings
        )
        self.das_model_dir_edit.setPlaceholderText("DAS model directory")
        self.das_model_dir_edit.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        vcl_conda_label = QLabel(
            "Vocalocator conda environment name:", self.ProcessSettings
        )
        vcl_conda_label.setFont(QFont(self.font_id, 12 + self.font_size_increase))
        vcl_conda_label.move(10, 605)
        self.vcl_conda = QLineEdit(
            self.processing_input_dict["vocalocator"]["vcl_conda_env_name"],
            self.ProcessSettings,
        )
        ...
        self.vcl_model_dir_edit = QLineEdit(
            self.vcl_model_dir_global, self.ProcessSettings
        )
        self.vcl_model_dir_edit.setPlaceholderText("Vocalocator model directory")
        self.vcl_model_dir_edit.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        vcl_model_dir_btn.setStyleSheet(
            "QPushButton { min-width: 77px; min-height: 12px; max-width: 77px; max-height: 12px; }"
        )
        self.vcl_model_dir_btn_clicked_flag = False
        vcl_model_dir_btn.clicked.connect(self._open_vcl_model_dialog)

        pc_usage_process_label = QLabel(
            "Notify e-mail(s) of PC usage:", self.ProcessSettings
        )
        pc_usage_process_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        ...
        self.loaded_processing_pc_list = sorted(
            self.processing_input_dict["send_email"]["Messenger"]["processing_pc_list"],
            key=lambda x: x
            == self.processing_input_dict["send_email"]["Messenger"][
                "processing_pc_choice"
            ],
            reverse=True,
        )
        ...
        self.processing_pc_cb.activated.connect(
            partial(
                self._combo_box_prior_processing_pc_choice,
                variable_id="processing_pc_choice",
            )
        )
        ...
        conduct_nv_sync_cb_label.setFont(
            QFont(self.font_id, 11 + self.font_size_increase)
        )
        conduct_nv_sync_cb_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        ...
        self.conduct_nv_sync_cb.activated.connect(
            partial(self._combo_box_prior_false, variable_id="conduct_nv_sync_cb_bool")
        )
        self.conduct_nv_sync_cb.move(225, 765)

        conduct_ephys_file_chaining_label = QLabel(
            "Concatenate e-phys files:", self.ProcessSettings
        )
        conduct_ephys_file_chaining_label.setFont(
            QFont(self.font_id, 11 + self.font_size_increase)
        )
        conduct_ephys_file_chaining_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        ...
        self.conduct_ephys_file_chaining_cb.activated.connect(
            partial(
                self._combo_box_prior_false,
                variable_id="conduct_ephys_file_chaining_cb_bool",
            )
        )
        self.conduct_ephys_file_chaining_cb.move(225, 795)

        split_cluster_spikes_cb_label = QLabel(
            "Split clusters to sessions:", self.ProcessSettings
        )
        split_cluster_spikes_cb_label.setFont(
            QFont(self.font_id, 11 + self.font_size_increase)
        )
        split_cluster_spikes_cb_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        ...
        self.split_cluster_spikes_cb.activated.connect(
            partial(
                self._combo_box_prior_false, variable_id="split_cluster_spikes_cb_bool"
            )
        )
        ...
        self.min_spike_num = QLineEdit(
            f"{self.processing_input_dict['modify_files']['Operator']['get_spike_times']['min_spike_num']}",
            self.ProcessSettings,
        )
        ...
        conduct_video_concatenation_label = QLabel(
            "Run video concatenation:", self.ProcessSettings
        )
        conduct_video_concatenation_label.setFont(
            QFont(self.font_id, 11 + self.font_size_increase)
        )
        conduct_video_concatenation_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        ...
        self.conduct_video_concatenation_cb.activated.connect(
            partial(
                self._combo_box_prior_false,
                variable_id="conduct_video_concatenation_cb_bool",
            )
        )
        self.conduct_video_concatenation_cb.move(column_two_x2, 40)

        conduct_video_fps_change_cb_label = QLabel(
            "Run video re-encoding:", self.ProcessSettings
        )
        conduct_video_fps_change_cb_label.setFont(
            QFont(self.font_id, 11 + self.font_size_increase)
        )
        conduct_video_fps_change_cb_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        ...
        self.conduct_video_fps_change_cb.activated.connect(
            partial(
                self._combo_box_prior_false,
                variable_id="conduct_video_fps_change_cb_bool",
            )
        )
        self.conduct_video_fps_change_cb.move(column_two_x2, 70)

        conversion_target_file_label = QLabel(
            "Concatenation name:", self.ProcessSettings
        )
        conversion_target_file_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        conversion_target_file_label.move(column_two_x1, 100)
        self.conversion_target_file = QLineEdit(
            self.processing_input_dict["modify_files"]["Operator"]["rectify_video_fps"][
                "conversion_target_file"
            ],
            self.ProcessSettings,
        )
        self.conversion_target_file.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        constant_rate_factor_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        constant_rate_factor_label.move(column_two_x1, 130)
        self.constant_rate_factor = QLineEdit(
            f"{self.processing_input_dict['modify_files']['Operator']['rectify_video_fps']['constant_rate_factor']}",
            self.ProcessSettings,
        )
        self.constant_rate_factor.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        self.encoding_preset_list = sorted(
            ["veryfast", "faster", "fast", "medium", "slow", "slower", "veryslow"],
            key=lambda x: x == self.encoding_preset,
            reverse=True,
        )
        self.encoding_preset_cb = QComboBox(self.ProcessSettings)
        self.encoding_preset_cb.addItems(
            [
                str(encode_preset_item)
                for encode_preset_item in self.encoding_preset_list
            ]
        )
        self.encoding_preset_cb.setStyleSheet("QComboBox { width: 80px; }")
        self.encoding_preset_cb.activated.connect(
            partial(self._combo_box_encoding_preset, variable_id="encoding_preset")
        )
        self.encoding_preset_cb.move(column_two_x2, 160)

        delete_con_file_cb_label = QLabel(
            "Purge concatenated files:", self.ProcessSettings
        )
        delete_con_file_cb_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        ...
        self.delete_con_file_cb.activated.connect(
            partial(self._combo_box_prior_true, variable_id="delete_con_file_cb_bool")
        )
        self.delete_con_file_cb.move(column_two_x2, 190)

        sleap_cluster_cb_label = QLabel(
            "Prepare SLEAP cluster job:", self.ProcessSettings
        )
        sleap_cluster_cb_label.setFont(
            QFont(self.font_id, 11 + self.font_size_increase)
        )
        sleap_cluster_cb_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        ...
        self.sleap_cluster_cb.activated.connect(
            partial(self._combo_box_prior_false, variable_id="sleap_cluster_cb_bool")
        )
        self.sleap_cluster_cb.move(column_two_x2, 220)

        sleap_file_conversion_cb_label = QLabel(
            "Run SLP-H5 conversion:", self.ProcessSettings
        )
        sleap_file_conversion_cb_label.setFont(
            QFont(self.font_id, 11 + self.font_size_increase)
        )
        sleap_file_conversion_cb_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        ...
        self.sleap_file_conversion_cb.activated.connect(
            partial(
                self._combo_box_prior_false, variable_id="sleap_file_conversion_cb_bool"
            )
        )
        self.sleap_file_conversion_cb.move(column_two_x2, 250)

        anipose_calibration_cb_label = QLabel(
            "Run AP calibration:", self.ProcessSettings
        )
        anipose_calibration_cb_label.setFont(
            QFont(self.font_id, 11 + self.font_size_increase)
        )
        anipose_calibration_cb_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        ...
        self.anipose_calibration_cb.activated.connect(
            partial(
                self._combo_box_prior_false, variable_id="anipose_calibration_cb_bool"
            )
        )
        self.anipose_calibration_cb.move(column_two_x2, 280)

        board_provided_cb_label = QLabel(
            "ChArUco board provided:", self.ProcessSettings
        )
        board_provided_cb_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        ...
        self.board_provided_cb.activated.connect(
            partial(self._combo_box_prior_false, variable_id="board_provided_cb_bool")
        )
        self.board_provided_cb.move(column_two_x2, 310)

        anipose_triangulation_cb_label = QLabel(
            "Run AP triangulation:", self.ProcessSettings
        )
        anipose_triangulation_cb_label.setFont(
            QFont(self.font_id, 11 + self.font_size_increase)
        )
        anipose_triangulation_cb_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        ...
        self.anipose_triangulation_cb.activated.connect(
            partial(
                self._combo_box_prior_false, variable_id="anipose_triangulation_cb_bool"
            )
        )
        self.anipose_triangulation_cb.move(column_two_x2, 340)

        triangulate_arena_points_cb_label = QLabel(
            "Triangulate arena nodes:", self.ProcessSettings
        )
        triangulate_arena_points_cb_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        ...
        self.triangulate_arena_points_cb.activated.connect(
            partial(
                self._combo_box_prior_false,
                variable_id="triangulate_arena_points_cb_bool",
            )
        )
        ...
        display_progress_cb_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        ...
        self.display_progress_cb.activated.connect(
            partial(self._combo_box_prior_true, variable_id="display_progress_cb_bool")
        )
        self.display_progress_cb.move(column_two_x2, 400)

        frame_restriction_label = QLabel("Frame restriction:", self.ProcessSettings)
        frame_restriction_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        frame_restriction_label.move(column_two_x1, 430)
        frame_restriction_input = (
            ""
            if self.processing_input_dict["anipose_operations"]["ConvertTo3D"][
            ...
            is None
                ...
                [
                    str(x)
                    for x in self.processing_input_dict["anipose_operations"][
                        "ConvertTo3D"
                    ]["conduct_anipose_triangulation"]["frame_restriction"]
                ]
            )
        )
        self.frame_restriction = QLineEdit(
            frame_restriction_input, self.ProcessSettings
        )
        self.frame_restriction.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        self.excluded_views = QLineEdit(
            ",".join(
                [
                    str(x)
                    for x in self.processing_input_dict["anipose_operations"][
                        "ConvertTo3D"
                    ]["conduct_anipose_triangulation"]["excluded_views"]
                ]
            ),
            self.ProcessSettings,
        )
        ...
        self.ransac_cb.activated.connect(
            partial(self._combo_box_prior_false, variable_id="ransac_cb_bool")
        )
        self.ransac_cb.move(column_two_x2, 490)

        rigid_body_constraints_label = QLabel(
            "Rigid body constraints:", self.ProcessSettings
        )
        rigid_body_constraints_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        rigid_body_constraints_label.move(column_two_x1, 520)
        self.rigid_body_constraints = QLineEdit("", self.ProcessSettings)
        self.rigid_body_constraints.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        self.rigid_body_constraints.setStyleSheet("QLineEdit { width: 108px; }")
        self.rigid_body_constraints.move(column_two_x2, 520)

        weak_body_constraints_label = QLabel(
            "Weak body constraints:", self.ProcessSettings
        )
        weak_body_constraints_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        weak_body_constraints_label.move(column_two_x1, 550)
        self.weak_body_constraints = QLineEdit("", self.ProcessSettings)
        self.weak_body_constraints.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        self.smooth_scale = QLineEdit(
            f"{self.processing_input_dict['anipose_operations']['ConvertTo3D']['conduct_anipose_triangulation']['smooth_scale']}",
            self.ProcessSettings,
        )
        ...
        self.weight_rigid = QLineEdit(
            f"{self.processing_input_dict['anipose_operations']['ConvertTo3D']['conduct_anipose_triangulation']['weight_rigid']}",
            self.ProcessSettings,
        )
        ...
        self.weight_weak = QLineEdit(
            f"{self.processing_input_dict['anipose_operations']['ConvertTo3D']['conduct_anipose_triangulation']['weight_weak']}",
            self.ProcessSettings,
        )
        ...
        reprojection_error_threshold_label = QLabel(
            "Reproject error threshold:", self.ProcessSettings
        )
        reprojection_error_threshold_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        reprojection_error_threshold_label.move(column_two_x1, 670)
        self.reprojection_error_threshold = QLineEdit(
            f"{self.processing_input_dict['anipose_operations']['ConvertTo3D']['conduct_anipose_triangulation']['reprojection_error_threshold']}",
            self.ProcessSettings,
        )
        self.reprojection_error_threshold.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        regularization_function_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        regularization_function_label.move(column_two_x1, 700)
        self.regularization_function = QLineEdit(
            f"{self.processing_input_dict['anipose_operations']['ConvertTo3D']['conduct_anipose_triangulation']['regularization_function']}",
            self.ProcessSettings,
        )
        self.regularization_function.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        self.n_deriv_smooth = QLineEdit(
            f"{self.processing_input_dict['anipose_operations']['ConvertTo3D']['conduct_anipose_triangulation']['n_deriv_smooth']}",
            self.ProcessSettings,
        )
        ...
        translate_rotate_metric_label = QLabel(
            "Re-coordinate (ExCode!):", self.ProcessSettings
        )
        translate_rotate_metric_label.setFont(
            QFont(self.font_id, 11 + self.font_size_increase)
        )
        translate_rotate_metric_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        ...
        self.translate_rotate_metric_cb.activated.connect(
            partial(
                self._combo_box_prior_false,
                variable_id="translate_rotate_metric_cb_bool",
            )
        )
        self.translate_rotate_metric_cb.move(column_two_x2, 760)

        static_reference_len_label = QLabel(
            "Static reference (m):", self.ProcessSettings
        )
        static_reference_len_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        static_reference_len_label.move(column_two_x1, 790)
        self.static_reference_len = QLineEdit(
            f"{self.processing_input_dict['anipose_operations']['ConvertTo3D']['translate_rotate_metric']['static_reference_len']}",
            self.ProcessSettings,
        )
        self.static_reference_len.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        self.static_reference_len.setStyleSheet("QLineEdit { width: 108px; }")
        self.static_reference_len.move(column_two_x2, 790)

        save_transformed_data_cb_label = QLabel(
            "Save transformation type:", self.ProcessSettings
        )
        save_transformed_data_cb_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        ...
        self.save_transformed_data_cb.activated.connect(
            partial(
                self._combo_box_prior_transformed_tracking_data,
                variable_id="save_transformed_data",
            )
        )
        self.save_transformed_data_cb.move(column_two_x2, 820)

        delete_original_h5_cb_label = QLabel(
            "Delete original .h5:", self.ProcessSettings
        )
        delete_original_h5_cb_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        ...
        self.delete_original_h5_cb.activated.connect(
            partial(
                self._combo_box_prior_true, variable_id="delete_original_h5_cb_bool"
            )
        )
        ...
        conduct_multichannel_conversion_cb_label = QLabel(
            "Convert to single-ch files:", self.ProcessSettings
        )
        conduct_multichannel_conversion_cb_label.setFont(
            QFont(self.font_id, 11 + self.font_size_increase)
        )
        conduct_multichannel_conversion_cb_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        ...
        self.conduct_multichannel_conversion_cb.setStyleSheet(
            "QComboBox { width: 80px; }"
        )
        self.conduct_multichannel_conversion_cb.activated.connect(
            partial(
                self._combo_box_prior_false,
                variable_id="conduct_multichannel_conversion_cb_bool",
            )
        )
        ...
        crop_wav_cam_cb_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        ...
        self.crop_wav_cam_cb.activated.connect(
            partial(self._combo_box_prior_false, variable_id="crop_wav_cam_cb_bool")
        )
        self.crop_wav_cam_cb.move(column_three_x2, 70)

        device_receiving_input_cb_label = QLabel(
            "Trgbox-USGH device(s):", self.ProcessSettings
        )
        device_receiving_input_cb_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        device_receiving_input_cb_label.move(column_three_x1, 100)
        self.usgh_device_receiving_input_list = sorted(
            ["m", "s", "both"],
            key=lambda x: x
            == self.processing_input_dict["synchronize_files"]["Synchronizer"][
                "crop_wav_files_to_video"
            ]["device_receiving_input"],
            reverse=True,
        )
        ...
        self.device_receiving_input_cb.activated.connect(
            partial(
                self._combo_box_prior_audio_device_camera_input,
                variable_id="device_receiving_input",
            )
        )
        self.device_receiving_input_cb.move(column_three_x2, 100)

        ch_receiving_input_label = QLabel(
            "Trgbox-USGH ch (1-12):", self.ProcessSettings
        )
        ch_receiving_input_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        ch_receiving_input_label.move(column_three_x1, 130)
        self.ch_receiving_input = QLineEdit(
            f"{self.processing_input_dict['synchronize_files']['Synchronizer']['crop_wav_files_to_video']['triggerbox_ch_receiving_input']}",
            self.ProcessSettings,
        )
        self.ch_receiving_input.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        conduct_hpss_cb_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        ...
        self.conduct_hpss_cb.activated.connect(
            partial(self._combo_box_prior_false, variable_id="conduct_hpss_cb_bool")
        )
        ...
        self.stft_window_hop = QLineEdit(
            ",".join(
                [
                    str(x)
                    for x in self.processing_input_dict["modify_files"]["Operator"][
                        "hpss_audio"
        ...
        hpss_kernel_size_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        hpss_kernel_size_label.move(column_three_x1, 220)
        self.hpss_kernel_size = QLineEdit(
            ",".join(
                [
                    str(x)
                    for x in self.processing_input_dict["modify_files"]["Operator"][
        ...
        self.hpss_power = QLineEdit(
            f"{self.processing_input_dict['modify_files']['Operator']['hpss_audio']['hpss_power']}",
            self.ProcessSettings,
        )
        ...
        filter_audio_cb_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        ...
        self.filter_audio_cb.activated.connect(
            partial(self._combo_box_prior_false, variable_id="filter_audio_cb_bool")
        )
        self.filter_audio_cb.move(column_three_x2, 310)

        filter_freq_bounds_label = QLabel(
            "Filter freq bounds (Hz):", self.ProcessSettings
        )
        filter_freq_bounds_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        filter_freq_bounds_label.move(column_three_x1, 340)
        self.filter_freq_bounds = QLineEdit(
            ",".join(
                [
                    str(x)
                    for x in self.processing_input_dict["modify_files"]["Operator"][
        ...
        self.filter_freq_bounds.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        conc_audio_cb_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        ...
        self.conc_audio_cb.activated.connect(
            partial(self._combo_box_prior_false, variable_id="conc_audio_cb_bool")
        )
        ...
        das_inference_cb_label.setFont(
            QFont(self.font_id, 11 + self.font_size_increase)
        )
        das_inference_cb_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        ...
        self.das_inference_cb.activated.connect(
            partial(self._combo_box_prior_false, variable_id="das_inference_cb_bool")
        )
        self.das_inference_cb.move(column_three_x2, 460)

        segment_confidence_threshold_label = QLabel(
            "DAS confidence threshold:", self.ProcessSettings
        )
        segment_confidence_threshold_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        segment_confidence_threshold_label.move(column_three_x1, 490)
        self.segment_confidence_threshold = QLineEdit(
            f"{self.processing_input_dict['usv_inference']['FindMouseVocalizations']['das_command_line_inference']['segment_confidence_threshold']}",
            self.ProcessSettings,
        )
        self.segment_confidence_threshold.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        self.segment_minlen = QLineEdit(
            f"{self.processing_input_dict['usv_inference']['FindMouseVocalizations']['das_command_line_inference']['segment_minlen']}",
            self.ProcessSettings,
        )
        ...
        segment_fillgap_label = QLabel(
            "Fill gaps shorter than (s):", self.ProcessSettings
        )
        ...
        self.segment_fillgap = QLineEdit(
            f"{self.processing_input_dict['usv_inference']['FindMouseVocalizations']['das_command_line_inference']['segment_fillgap']}",
            self.ProcessSettings,
        )
        ...
        das_output_type_label = QLabel(
            "Inference output file type:", self.ProcessSettings
        )
        ...
        self.das_output_type = QLineEdit(
            f"{self.processing_input_dict['usv_inference']['FindMouseVocalizations']['das_command_line_inference']['output_file_type']}",
            self.ProcessSettings,
        )
        ...
        das_summary_cb_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        ...
        self.das_summary_cb.activated.connect(
            partial(self._combo_box_prior_false, variable_id="das_summary_cb_bool")
        )
        self.das_summary_cb.move(column_three_x2, 610)

        prepare_assign_usv_cb_label = QLabel(
            "Prepare USV assignment:", self.ProcessSettings
        )
        prepare_assign_usv_cb_label.setFont(
            QFont(self.font_id, 11 + self.font_size_increase)
        )
        prepare_assign_usv_cb_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        ...
        self.prepare_assign_usv_cb.activated.connect(
            partial(
                self._combo_box_prior_false, variable_id="prepare_assign_usv_cb_bool"
            )
        )
        ...
        assign_usv_cb_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        ...
        self.assign_usv_cb.activated.connect(
            partial(self._combo_box_prior_false, variable_id="assign_usv_cb_bool")
        )
        self.assign_usv_cb.move(column_three_x2, 670)

        av_sync_label = QLabel(
            "Synchronization between A/V files", self.ProcessSettings
        )
        ...
        conduct_sync_cb_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        ...
        self.conduct_sync_cb.activated.connect(
            partial(self._combo_box_prior_false, variable_id="conduct_sync_cb_bool")
        )
        self.conduct_sync_cb.move(column_three_x2, 740)

        phidget_extra_data_camera_label = QLabel(
            "Phidget(s) camera serial:", self.ProcessSettings
        )
        phidget_extra_data_camera_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        phidget_extra_data_camera_label.move(column_three_x1, 770)
        self.phidget_extra_data_camera = QLineEdit(
            f"{self.processing_input_dict['extract_phidget_data']['Gatherer']['prepare_data_for_analyses']['extra_data_camera']}",
            self.ProcessSettings,
        )
        self.phidget_extra_data_camera.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        a_ch_receiving_input_label = QLabel(
            "Arduino-USGH ch (1-12):", self.ProcessSettings
        )
        a_ch_receiving_input_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        a_ch_receiving_input_label.move(column_three_x1, 800)
        self.a_ch_receiving_input = QLineEdit(
            f"{self.processing_input_dict['synchronize_files']['Synchronizer']['find_audio_sync_trains']['sync_ch_receiving_input']}",
            self.ProcessSettings,
        )
        self.a_ch_receiving_input.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        self.a_ch_receiving_input.setStyleSheet("QLineEdit { width: 108px; }")
        self.a_ch_receiving_input.move(column_three_x2, 800)

        v_camera_serial_num_label = QLabel(
            "Sync camera serial num(s):", self.ProcessSettings
        )
        v_camera_serial_num_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        v_camera_serial_num_label.move(column_three_x1, 830)
        self.v_camera_serial_num = QLineEdit(
            ",".join(
                [
                    str(x)
                    for x in self.processing_input_dict["synchronize_files"][
                        "Synchronizer"
                    ]["find_video_sync_trains"]["sync_camera_serial_num"]
                ]
            ),
            self.ProcessSettings,
        )
        self.v_camera_serial_num.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        self.v_camera_serial_num.setStyleSheet("QLineEdit { width: 108px; }")
        self.v_camera_serial_num.move(column_three_x2, 830)

        self._create_buttons_process(
            seq=0,
            class_option=self.ProcessSettings,
            button_pos_y=record_four_y - 35,
            next_button_x_pos=record_four_x - 100,
        )
    ...
    def process_two(self) -> None:
        ...
        with open(
            (Path(__file__).parent / "_parameter_settings/processing_settings.json"),
            "w",
        ) as processing_settings_file:
            json.dump(self.processing_input_dict, processing_settings_file, indent=2)

        self.run_processing = Stylist(
            message_output=self._process_message,
            input_parameter_dict=self.processing_input_dict,
            root_directories=self.processing_input_dict["preprocess_data"][
                "root_directories"
            ],
            exp_settings_dict=self.exp_settings_dict,
        )
        ...
        self._create_buttons_process(
            seq=1,
            class_option=self.ConductProcess,
            button_pos_y=record_four_y - 35,
            next_button_x_pos=record_four_x - 100,
        )
    ...
    def analyze_one(self) -> None:
        ...
        analyses_dir_label = QLabel(
            "(*) Root directories for analyses", self.AnalysesSettings
        )
        ...
        self.analyses_dir_edit.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        self.analyses_dir_edit.move(10, 40)
        self.analyses_dir_edit.setFixedSize(350, 320)

        pc_usage_analyses_label = QLabel(
            "Notify e-mail(s) of PC usage:", self.AnalysesSettings
        )
        pc_usage_analyses_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        pc_usage_analyses_label.move(10, 365)
        self.pc_usage_analyses = QLineEdit("", self.AnalysesSettings)
        self.pc_usage_analyses.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        self.loaded_analyses_pc_list = sorted(
            self.analyses_input_dict["send_email"]["analyses_pc_list"],
            key=lambda x: x
            == self.analyses_input_dict["send_email"]["analyses_pc_choice"],
            reverse=True,
        )
        ...
        self.analyses_pc_cb.activated.connect(
            partial(
                self._combo_box_prior_analyses_pc_choice,
                variable_id="analyses_pc_choice",
            )
        )
        ...
        compute_behavioral_features_label = QLabel(
            "Compute 3D behavioral features:", self.AnalysesSettings
        )
        compute_behavioral_features_label.setFont(
            QFont(self.font_id, 11 + self.font_size_increase)
        )
        compute_behavioral_features_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        ...
        self.compute_behavioral_features_cb.activated.connect(
            partial(
                self._combo_box_prior_false,
                variable_id="compute_behavioral_features_cb_bool",
            )
        )
        self.compute_behavioral_features_cb.move(275, 465)

        calculate_neuronal_tuning_curves_label = QLabel(
            "Compute 3D feature tuning curves:", self.AnalysesSettings
        )
        calculate_neuronal_tuning_curves_label.setFont(
            QFont(self.font_id, 11 + self.font_size_increase)
        )
        calculate_neuronal_tuning_curves_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        ...
        self.calculate_neuronal_tuning_curves_cb.setStyleSheet(
            "QComboBox { width: 57px; }"
        )
        self.calculate_neuronal_tuning_curves_cb.activated.connect(
            partial(
                self._combo_box_prior_false,
                variable_id="calculate_neuronal_tuning_curves_cb_bool",
            )
        )
        self.calculate_neuronal_tuning_curves_cb.move(275, 495)

        create_usv_playback_wav_label = QLabel(
            "Create USV playback .WAV file:", self.AnalysesSettings
        )
        create_usv_playback_wav_label.setFont(
            QFont(self.font_id, 11 + self.font_size_increase)
        )
        create_usv_playback_wav_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        ...
        self.create_usv_playback_wav_cb.activated.connect(
            partial(
                self._combo_box_prior_false,
                variable_id="create_usv_playback_wav_cb_bool",
            )
        )
        self.create_usv_playback_wav_cb.move(275, 525)

        num_usv_files_label = QLabel(
            "Total number of USV playback files:", self.AnalysesSettings
        )
        num_usv_files_label.setFont(QFont(self.font_id, 12 + self.font_size_increase))
        num_usv_files_label.move(10, 555)
        self.num_usv_files = QLineEdit(
            f"{self.analyses_input_dict['create_usv_playback_wav']['num_usv_files']}",
            self.AnalysesSettings,
        )
        ...
        total_usv_number_label = QLabel(
            "Total number os USVs per file:", self.AnalysesSettings
        )
        total_usv_number_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        total_usv_number_label.move(10, 585)
        self.total_usv_number = QLineEdit(
            f"{self.analyses_input_dict['create_usv_playback_wav']['total_usv_number']}",
            self.AnalysesSettings,
        )
        ...
        ipi_duration_label = QLabel(
            "Fixed silence between USVs (s):", self.AnalysesSettings
        )
        ipi_duration_label.setFont(QFont(self.font_id, 12 + self.font_size_increase))
        ipi_duration_label.move(10, 615)
        self.ipi_duration = QLineEdit(
            f"{self.analyses_input_dict['create_usv_playback_wav']['ipi_duration']}",
            self.AnalysesSettings,
        )
        ...
        frequency_shift_audio_segment_label = QLabel(
            "Frequency-shift audio segment:", self.AnalysesSettings
        )
        frequency_shift_audio_segment_label.setFont(
            QFont(self.font_id, 11 + self.font_size_increase)
        )
        frequency_shift_audio_segment_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        ...
        self.frequency_shift_audio_segment_cb.setStyleSheet(
            "QComboBox { width: 57px; }"
        )
        self.frequency_shift_audio_segment_cb.activated.connect(
            partial(
                self._combo_box_prior_false,
                variable_id="frequency_shift_audio_segment_cb_bool",
            )
        )
        self.frequency_shift_audio_segment_cb.move(275, 645)

        frequency_shift_audio_dir_label = QLabel(
            ".WAV audio subdirectory of choice:", self.AnalysesSettings
        )
        frequency_shift_audio_dir_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        frequency_shift_audio_dir_label.move(10, 675)
        self.frequency_shift_audio_dir_list = sorted(
            ["cropped_to_video", "hpss", "hpss_filtered"],
            key=lambda x: x
            == self.analyses_input_dict["frequency_shift_audio_segment"][
                "fs_audio_dir"
            ],
            reverse=True,
        )
        ...
        self.frequency_shift_audio_dir_cb.activated.connect(
            partial(self._combo_box_fs_audio_dir, variable_id="fs_audio_dir")
        )
        self.frequency_shift_audio_dir_cb.move(275, 675)

        frequency_shift_device_id_label = QLabel(
            "Recording device identity (m|s):", self.AnalysesSettings
        )
        frequency_shift_device_id_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        frequency_shift_device_id_label.move(10, 705)
        self.frequency_shift_device_id_list = sorted(
            ["m", "s"],
            key=lambda x: x
            == self.analyses_input_dict["frequency_shift_audio_segment"][
                "fs_device_id"
        ...
        self.frequency_shift_device_id_cb.activated.connect(
            partial(self._combo_box_fs_device_id, variable_id="fs_device_id")
        )
        self.frequency_shift_device_id_cb.move(275, 705)

        frequency_shift_channel_id_label = QLabel(
            "Recording device channel (1-12):", self.AnalysesSettings
        )
        frequency_shift_channel_id_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        frequency_shift_channel_id_label.move(10, 735)
        self.frequency_shift_channel_id_list = sorted(
            list(range(1, 13)),
            key=lambda x: x
            == self.analyses_input_dict["frequency_shift_audio_segment"][
                "fs_channel_id"
        ...
        self.frequency_shift_channel_id_cb.addItems(
            [str(ch_id_item) for ch_id_item in self.frequency_shift_channel_id_list]
        )
        self.frequency_shift_channel_id_cb.setStyleSheet("QComboBox { width: 57px; }")
        self.frequency_shift_channel_id_cb.activated.connect(
            partial(self._combo_box_fs_channel_id, variable_id="fs_channel_id")
        )
        self.frequency_shift_channel_id_cb.move(275, 735)

        fs_sequence_start_label = QLabel(
            "Start of audio sequence (s):", self.AnalysesSettings
        )
        fs_sequence_start_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        fs_sequence_start_label.move(10, 765)
        self.fs_sequence_start = QLineEdit(
            f"{self.analyses_input_dict['frequency_shift_audio_segment']['fs_sequence_start']}",
            self.AnalysesSettings,
        )
        self.fs_sequence_start.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        self.fs_sequence_start.setStyleSheet("QLineEdit { width: 85px; }")
        self.fs_sequence_start.move(275, 765)

        fs_sequence_duration_label = QLabel(
            "Total duration of audio sequence (s):", self.AnalysesSettings
        )
        fs_sequence_duration_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        fs_sequence_duration_label.move(10, 795)
        self.fs_sequence_duration = QLineEdit(
            f"{self.analyses_input_dict['frequency_shift_audio_segment']['fs_sequence_duration']}",
            self.AnalysesSettings,
        )
        self.fs_sequence_duration.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        self.fs_sequence_duration.setStyleSheet("QLineEdit { width: 85px; }")
        self.fs_sequence_duration.move(275, 795)

        fs_octave_shift_label = QLabel(
            "Octave shift (direction and quantity):", self.AnalysesSettings
        )
        fs_octave_shift_label.setFont(QFont(self.font_id, 12 + self.font_size_increase))
        fs_octave_shift_label.move(10, 825)
        self.fs_octave_shift = QLineEdit(
            f"{self.analyses_input_dict['frequency_shift_audio_segment']['fs_octave_shift']}",
            self.AnalysesSettings,
        )
        ...
        volume_adjust_audio_segment_label = QLabel(
            "Volume-adjust audio segment:", self.AnalysesSettings
        )
        volume_adjust_audio_segment_label.setFont(
            QFont(self.font_id, 11 + self.font_size_increase)
        )
        ...
        self.volume_adjust_audio_segment_cb.activated.connect(
            partial(
                self._combo_box_prior_true,
                variable_id="volume_adjust_audio_segment_cb_bool",
            )
        )
        self.volume_adjust_audio_segment_cb.move(275, 855)

        self._create_buttons_analyze(
            seq=0,
            class_option=self.AnalysesSettings,
            button_pos_y=analyze_one_y - 35,
            next_button_x_pos=analyze_one_x - 100,
        )
    ...
    def analyze_two(self) -> None:
        ...
        with open(
            (Path(__file__).parent / "_parameter_settings/analyses_settings.json"), "w"
        ) as analyses_settings_file:
            json.dump(self.analyses_input_dict, analyses_settings_file, indent=2)

        self.run_analyses = Analyst(
            message_output=self._analyses_message,
            input_parameter_dict=self.analyses_input_dict,
            root_directories=self.analyses_input_dict["analyze_data"][
                "root_directories"
            ],
        )
        ...
        self._create_buttons_analyze(
            seq=1,
            class_option=self.ConductAnalyses,
            button_pos_y=analyze_two_y - 35,
            next_button_x_pos=analyze_two_x - 100,
        )
    ...
    def _save_analyses_labels_func(self) -> None:
        ...
        if os.name == "nt":
            self.analyses_dir_edit = self.analyses_dir_edit.toPlainText().replace(
                os.sep, "\\"
            )
        else:
        ...
        self.analyses_input_dict["analyze_data"]["root_directories"] = (
            self.analyses_dir_edit
        )

        self.analyses_input_dict["send_email"]["experimenter"] = f"{self.exp_id}"
        self.analyses_input_dict["send_email"]["analyses_pc_choice"] = str(
            self.analyses_pc_choice
        )
        ...
        self.analyses_input_dict["send_email"]["send_message"]["receivers"] = (
            self.pc_usage_analyses
        )

        self.analyses_input_dict["analyses_booleans"][
            "compute_behavioral_features_bool"
        ] = self.compute_behavioral_features_cb_bool
        self.compute_behavioral_features_cb_bool = False

        self.analyses_input_dict["analyses_booleans"][
            "compute_behavioral_tuning_bool"
        ] = self.calculate_neuronal_tuning_curves_cb_bool
        self.calculate_neuronal_tuning_curves_cb_bool = False

        self.analyses_input_dict["analyses_booleans"][
            "create_usv_playback_wav_bool"
        ] = self.create_usv_playback_wav_cb_bool
        self.create_usv_playback_wav_cb_bool = False

        self.analyses_input_dict["analyses_booleans"][
            "frequency_shift_audio_segment_bool"
        ] = self.frequency_shift_audio_segment_cb_bool
        self.frequency_shift_audio_segment_cb_bool = False

        self.analyses_input_dict["create_usv_playback_wav"]["num_usv_files"] = int(
            ast.literal_eval(self.num_usv_files.text())
        )
        self.analyses_input_dict["create_usv_playback_wav"]["total_usv_number"] = int(
            ast.literal_eval(self.total_usv_number.text())
        )
        self.analyses_input_dict["create_usv_playback_wav"]["ipi_duration"] = float(
            ast.literal_eval(self.ipi_duration.text())
        )

        self.analyses_input_dict["frequency_shift_audio_segment"]["fs_audio_dir"] = (
            self.fs_audio_dir
        )
        self.analyses_input_dict["frequency_shift_audio_segment"]["fs_device_id"] = (
            self.fs_device_id
        )
        self.analyses_input_dict["frequency_shift_audio_segment"]["fs_channel_id"] = (
            self.fs_channel_id
        )

        self.analyses_input_dict["frequency_shift_audio_segment"][
            "fs_sequence_start"
        ] = float(ast.literal_eval(self.fs_sequence_start.text()))
        self.analyses_input_dict["frequency_shift_audio_segment"][
            "fs_sequence_duration"
        ] = float(ast.literal_eval(self.fs_sequence_duration.text()))
        self.analyses_input_dict["frequency_shift_audio_segment"]["fs_octave_shift"] = (
            int(ast.literal_eval(self.fs_octave_shift.text()))
        )

        self.analyses_input_dict["frequency_shift_audio_segment"][
            "fs_volume_adjustment"
        ] = self.volume_adjust_audio_segment_cb_bool
    ...
    def visualize_one(self) -> None:
        ...
        visualizations_dir_label = QLabel(
            "(*) Root directories for visualizations", self.VisualizationsSettings
        )
        visualizations_dir_label.setFont(
            QFont(self.font_id, 13 + self.font_size_increase)
        )
        ...
        self.visualizations_dir_edit.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        self.visualizations_dir_edit.move(10, 40)
        self.visualizations_dir_edit.setFixedSize(350, 320)

        pc_usage_visualizations_label = QLabel(
            "Notify e-mail(s) of PC usage:", self.VisualizationsSettings
        )
        pc_usage_visualizations_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        pc_usage_visualizations_label.move(10, 365)
        self.pc_usage_visualizations = QLineEdit("", self.VisualizationsSettings)
        self.pc_usage_visualizations.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        self.pc_usage_visualizations.setStyleSheet("QLineEdit { width: 135px; }")
        self.pc_usage_visualizations.move(225, 365)

        visualizations_pc_label = QLabel(
            "Visualizations PC of choice:", self.VisualizationsSettings
        )
        visualizations_pc_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        visualizations_pc_label.move(10, 395)
        self.loaded_visualizations_pc_list = sorted(
            self.visualizations_input_dict["send_email"]["visualizations_pc_list"],
            key=lambda x: x
            == self.visualizations_input_dict["send_email"]["visualizations_pc_choice"],
            reverse=True,
        )
        ...
        self.visualizations_pc_cb.activated.connect(
            partial(
                self._combo_box_prior_visualizations_pc_choice,
                variable_id="visualizations_pc_choice",
            )
        )
        ...
        plot_behavioral_features_label = QLabel(
            "Plot 3D behavioral tuning curves:", self.VisualizationsSettings
        )
        plot_behavioral_features_label.setFont(
            QFont(self.font_id, 11 + self.font_size_increase)
        )
        plot_behavioral_features_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        ...
        self.plot_behavioral_features_cb.activated.connect(
            partial(
                self._combo_box_prior_false,
                variable_id="plot_behavioral_tuning_cb_bool",
            )
        )
        self.plot_behavioral_features_cb.move(275, 465)

        smoothing_sd_label = QLabel(
            "Ratemap smoothing sigma (bins):", self.VisualizationsSettings
        )
        smoothing_sd_label.setFont(QFont(self.font_id, 12 + self.font_size_increase))
        smoothing_sd_label.move(10, 495)
        self.smoothing_sd = QLineEdit(
            f"{self.visualizations_input_dict['neuronal_tuning_figures']['smoothing_sd']}",
        ...
        occ_threshold_label = QLabel(
            "Minimal occupancy allowed (s):", self.VisualizationsSettings
        )
        occ_threshold_label.setFont(QFont(self.font_id, 12 + self.font_size_increase))
        occ_threshold_label.move(10, 525)
        self.occ_threshold = QLineEdit(
            f"{self.visualizations_input_dict['neuronal_tuning_figures']['occ_threshold']}",
            self.VisualizationsSettings,
        )
        ...
        make_behavioral_video_label = QLabel(
            "Visualize 3D behavior (figure/video):", self.VisualizationsSettings
        )
        make_behavioral_video_label.setFont(
            QFont(self.font_id, 11 + self.font_size_increase)
        )
        make_behavioral_video_label.setStyleSheet(
            "QLabel { color: #F58025; font-weight: bold;}"
        )
        ...
        self.make_behavioral_video_cb.activated.connect(
            partial(
                self._combo_box_prior_false, variable_id="make_behavioral_video_cb_bool"
            )
        )
        self.make_behavioral_video_cb.move(vis_col_two_x2, 40)

        self.arena_root_directory_edit = QLineEdit(
            f"{self.visualizations_input_dict['make_behavioral_videos']['arena_directory']}",
            self.VisualizationsSettings,
        )
        self.arena_root_directory_edit.setPlaceholderText(
            "Arena tracking root directory"
        )
        self.arena_root_directory_edit.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        arena_root_directory_btn.setFont(
            QFont(self.font_id, 8 + self.font_size_increase)
        )
        arena_root_directory_btn.move(vis_col_two_x2, 70)
        arena_root_directory_btn.setStyleSheet(
            "QPushButton { min-width: 65px; min-height: 12px; max-width: 656px; max-height: 12px; }"
        )
        ...
        self.speaker_audio_file_edit = QLineEdit(
            f"{self.visualizations_input_dict['make_behavioral_videos']['speaker_audio_file']}",
            self.VisualizationsSettings,
        )
        self.speaker_audio_file_edit.setPlaceholderText("Speaker playback file")
        self.speaker_audio_file_edit.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        speaker_audio_file_btn.setStyleSheet(
            "QPushButton { min-width: 65px; min-height: 12px; max-width: 656px; max-height: 12px; }"
        )
        ...
        self.sequence_audio_file_edit.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        ...
        sequence_audio_file_btn.setFont(
            QFont(self.font_id, 8 + self.font_size_increase)
        )
        sequence_audio_file_btn.move(vis_col_two_x2, 130)
        sequence_audio_file_btn.setStyleSheet(
            "QPushButton { min-width: 65px; min-height: 12px; max-width: 656px; max-height: 12px; }"
        )
        ...
        visualization_type_cb_label = QLabel(
            "Create data animation (or else figure):", self.VisualizationsSettings
        )
        visualization_type_cb_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        ...
        self.visualization_type_cb.activated.connect(
            partial(
                self._combo_box_prior_false, variable_id="visualization_type_cb_bool"
            )
        )
        self.visualization_type_cb.move(vis_col_two_x2, 160)

        save_fig_format_cb_label = QLabel(
            "Save created figure to file in format:", self.VisualizationsSettings
        )
        save_fig_format_cb_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        save_fig_format_cb_label.move(vis_col_two_x1, 190)
        self.fig_format_list = sorted(
            ["eps", "pdf", "png", "svg"],
            key=lambda x: x
            == self.visualizations_input_dict["make_behavioral_videos"][
        ...
        self.save_fig_format_cb.activated.connect(
            partial(self._combo_box_figure_format, variable_id="fig_format")
        )
        self.save_fig_format_cb.move(vis_col_two_x2, 190)

        video_start_time_label = QLabel(
            "Animation/figure start time (s):", self.VisualizationsSettings
        )
        video_start_time_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        video_start_time_label.move(vis_col_two_x1, 220)
        self.video_start_time = QLineEdit(
            f"{self.visualizations_input_dict['make_behavioral_videos']['video_start_time']}",
            self.VisualizationsSettings,
        )
        ...
        video_duration_label = QLabel(
            "Total animation duration (s):", self.VisualizationsSettings
        )
        video_duration_label.setFont(QFont(self.font_id, 12 + self.font_size_increase))
        video_duration_label.move(vis_col_two_x1, 250)
        self.video_duration = QLineEdit(
            f"{self.visualizations_input_dict['make_behavioral_videos']['video_duration']}",
            self.VisualizationsSettings,
        )
        ...
        plot_theme_cb_label = QLabel(
            "Animation/figure background theme:", self.VisualizationsSettings
        )
        plot_theme_cb_label.setFont(QFont(self.font_id, 12 + self.font_size_increase))
        plot_theme_cb_label.move(vis_col_two_x1, 280)
        self.plot_theme_list = sorted(
            ["light", "dark"],
            key=lambda x: x
            == self.visualizations_input_dict["make_behavioral_videos"]["plot_theme"],
            reverse=True,
        )
        ...
        self.plot_theme_cb.activated.connect(
            partial(self._combo_box_plot_theme, variable_id="plot_theme")
        )
        self.plot_theme_cb.move(vis_col_two_x2, 280)

        view_angle_cb_label = QLabel(
            "Animation/figure viewing angle:", self.VisualizationsSettings
        )
        view_angle_cb_label.setFont(QFont(self.font_id, 12 + self.font_size_increase))
        view_angle_cb_label.move(vis_col_two_x1, 310)
        self.view_angle_list = sorted(
            ["top", "side"],
            key=lambda x: x
            == self.visualizations_input_dict["make_behavioral_videos"]["view_angle"],
            reverse=True,
        )
        ...
        self.view_angle_cb.activated.connect(
            partial(self._combo_box_view_angle, variable_id="view_angle")
        )
        self.view_angle_cb.move(vis_col_two_x2, 310)

        side_azimuth_start_label = QLabel(
            "Side view azimuth angle (°):", self.VisualizationsSettings
        )
        side_azimuth_start_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        side_azimuth_start_label.move(vis_col_two_x1, 340)
        self.side_azimuth_start = QLineEdit(
            f"{self.visualizations_input_dict['make_behavioral_videos']['side_azimuth_start']}",
            self.VisualizationsSettings,
        )
        self.side_azimuth_start.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        self.side_azimuth_start.setStyleSheet("QLineEdit { width: 85px; }")
        self.side_azimuth_start.move(vis_col_two_x2, 340)

        rotate_side_view_label = QLabel(
            "Rotate side view in animation:", self.VisualizationsSettings
        )
        rotate_side_view_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        ...
        self.rotate_side_view_cb.activated.connect(
            partial(self._combo_box_prior_false, variable_id="rotate_side_view_bool")
        )
        self.rotate_side_view_cb.move(vis_col_two_x2, 370)

        rotation_speed_label = QLabel(
            "Rotation speed of side view (°/s):", self.VisualizationsSettings
        )
        rotation_speed_label.setFont(QFont(self.font_id, 12 + self.font_size_increase))
        rotation_speed_label.move(vis_col_two_x1, 400)
        self.rotation_speed = QLineEdit(
            f"{self.visualizations_input_dict['make_behavioral_videos']['rotation_speed']}",
            self.VisualizationsSettings,
        )
        ...
        track_history_label = QLabel(
            "Plot history of single body node:", self.VisualizationsSettings
        )
        ...
        self.track_history_cb.activated.connect(
            partial(self._combo_box_prior_false, variable_id="history_cb_bool")
        )
        self.track_history_cb.move(vis_col_two_x2, 430)

        plot_playback_speaker_label = QLabel(
            "Plot tracking of playback speaker:", self.VisualizationsSettings
        )
        plot_playback_speaker_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        ...
        self.plot_playback_speaker_cb.activated.connect(
            partial(self._combo_box_prior_false, variable_id="speaker_cb_bool")
        )
        self.plot_playback_speaker_cb.move(vis_col_two_x2, 460)

        spectrogram_bool_label = QLabel(
            "Plot audio spectrogram (>30 kHz):", self.VisualizationsSettings
        )
        spectrogram_bool_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        ...
        self.spectrogram_bool_cb.activated.connect(
            partial(self._combo_box_prior_false, variable_id="spectrogram_cb_bool")
        )
        self.spectrogram_bool_cb.move(vis_col_two_x2, 490)

        spectrogram_ch_label = QLabel(
            "Audio spectrogram channel (0-23):", self.VisualizationsSettings
        )
        spectrogram_ch_label.setFont(QFont(self.font_id, 12 + self.font_size_increase))
        spectrogram_ch_label.move(vis_col_two_x1, 520)
        self.spectrogram_ch_list = sorted(
            list(range(24)),
            key=lambda x: x
            == self.visualizations_input_dict["make_behavioral_videos"][
                "spectrogram_ch"
        ...
        self.spectrogram_ch_cb.addItems(
            [str(spec_ch_item) for spec_ch_item in self.spectrogram_ch_list]
        )
        self.spectrogram_ch_cb.setStyleSheet("QComboBox { width: 57px; }")
        self.spectrogram_ch_cb.activated.connect(
            partial(self._combo_box_spectrogram_ch, variable_id="spectrogram_ch")
        )
        self.spectrogram_ch_cb.move(vis_col_two_x2, 520)

        raster_plot_bool_label = QLabel(
            "Plot neural cluster activity raster:", self.VisualizationsSettings
        )
        raster_plot_bool_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        ...
        self.raster_plot_bool_cb.activated.connect(
            partial(self._combo_box_prior_false, variable_id="raster_plot_cb_bool")
        )
        self.raster_plot_bool_cb.move(vis_col_two_x2, 550)

        raster_special_units_label = QLabel(
            "Emphasize specific unit(s) in raster plot:", self.VisualizationsSettings
        )
        raster_special_units_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        raster_special_units_label.move(vis_col_two_x1, 580)
        self.raster_special_units = QLineEdit(
            ",".join(
                map(
                    str,
                    self.visualizations_input_dict["make_behavioral_videos"][
        ...
        self.raster_special_units.setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        self.raster_special_units.setStyleSheet("QLineEdit { width: 85px; }")
        self.raster_special_units.move(vis_col_two_x2, 580)

        spike_sound_bool_label = QLabel(
            "Create a spike .WAV for special unit(s):", self.VisualizationsSettings
        )
        spike_sound_bool_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        ...
        self.spike_sound_bool_cb.activated.connect(
            partial(self._combo_box_prior_false, variable_id="spike_sound_cb_bool")
        )
        self.spike_sound_bool_cb.move(vis_col_two_x2, 610)

        beh_features_bool_label = QLabel(
            "Plot 3D behavioral/social features:", self.VisualizationsSettings
        )
        beh_features_bool_label.setFont(
            QFont(self.font_id, 12 + self.font_size_increase)
        )
        ...
        self.beh_features_bool_cb.activated.connect(
            partial(self._combo_box_prior_false, variable_id="beh_features_cb_bool")
        )
        self.beh_features_bool_cb.move(vis_col_two_x2, 640)

        self._create_buttons_visualize(
            seq=0,
            class_option=self.VisualizationsSettings,
            button_pos_y=visualize_one_y - 35,
            next_button_x_pos=visualize_one_x - 100,
        )
    ...
    def visualize_two(self) -> None:
        ...
        with open(
            ...
            "w",
        ) as visualizations_settings_file:
            json.dump(
                self.visualizations_input_dict, visualizations_settings_file, indent=2
            )

        self.run_visualizations = Visualizer(
            message_output=self._visualizations_message,
            input_parameter_dict=self.visualizations_input_dict,
            root_directories=self.visualizations_input_dict["visualize_data"][
                "root_directories"
            ],
        )
        ...
        self._create_buttons_visualize(
            seq=1,
            class_option=self.ConductVisualizations,
            button_pos_y=visualize_two_y - 35,
            next_button_x_pos=visualize_two_x - 100,
        )
    ...
    def _save_visualizations_labels_func(self) -> None:
        ...
        if os.name == "nt":
            self.visualizations_dir_edit = (
                self.visualizations_dir_edit.toPlainText().replace(os.sep, "\\")
            )
        else:
        ...
        self.visualizations_input_dict["visualize_data"]["root_directories"] = (
            self.visualizations_dir_edit
        )

        self.visualizations_input_dict["neuronal_tuning_figures"]["smoothing_sd"] = (
            float(ast.literal_eval(self.smoothing_sd.text()))
        )
        self.visualizations_input_dict["neuronal_tuning_figures"]["occ_threshold"] = (
            float(ast.literal_eval(self.occ_threshold.text()))
        )

        self.visualizations_input_dict["send_email"]["experimenter"] = f"{self.exp_id}"
        self.visualizations_input_dict["send_email"]["visualizations_pc_choice"] = str(
            self.visualizations_pc_choice
        )
        ...
        self.visualizations_input_dict["send_email"]["send_message"]["receivers"] = (
            self.pc_usage_visualizations
        )

        self.visualizations_input_dict["visualize_booleans"][
            "make_behavioral_tuning_figures_bool"
        ] = self.plot_behavioral_tuning_cb_bool
        self.plot_behavioral_tuning_cb_bool = False

        self.visualizations_input_dict["visualize_booleans"][
            "make_behavioral_videos_bool"
        ] = self.make_behavioral_video_cb_bool
        self.make_behavioral_video_cb_bool = False

        if not self.arena_root_directory_btn_clicked_flag:
            self.visualizations_input_dict["make_behavioral_videos"][
                "arena_directory"
            ] = self.arena_root_directory_edit.text()

        if not self.speaker_audio_file_btn_clicked_flag:
            self.visualizations_input_dict["make_behavioral_videos"][
                "speaker_audio_file"
            ] = self.speaker_audio_file_edit.text()

        if not self.sequence_audio_file_btn_clicked_flag:
            self.visualizations_input_dict["make_behavioral_videos"][
                "sequence_audio_file"
            ] = self.sequence_audio_file_edit.text()

        self.visualizations_input_dict["make_behavioral_videos"]["animate_bool"] = (
            self.visualization_type_cb_bool
        )
        self.visualization_type_cb_bool = False

        self.visualizations_input_dict["make_behavioral_videos"]["video_start_time"] = (
            float(ast.literal_eval(self.video_start_time.text()))
        )
        self.visualizations_input_dict["make_behavioral_videos"]["video_duration"] = (
            float(ast.literal_eval(self.video_duration.text()))
        )
        ...
        self.visualizations_input_dict["make_behavioral_videos"][
            "side_azimuth_start"
        ] = int(ast.literal_eval(self.side_azimuth_start.text()))

        self.visualizations_input_dict["make_behavioral_videos"][
            "rotate_side_view_bool"
        ] = self.rotate_side_view_bool
        self.rotate_side_view_bool = False

        self.visualizations_input_dict["make_behavioral_videos"]["rotation_speed"] = (
            int(ast.literal_eval(self.rotation_speed.text()))
        )

        self.visualizations_input_dict["make_behavioral_videos"]["history_bool"] = (
            self.history_cb_bool
        )
        self.history_cb_bool = False

        self.visualizations_input_dict["make_behavioral_videos"]["speaker_bool"] = (
            self.speaker_cb_bool
        )
        self.speaker_cb_bool = False

        self.visualizations_input_dict["make_behavioral_videos"]["spectrogram_bool"] = (
            self.spectrogram_cb_bool
        )
        self.spectrogram_cb_bool = False

        self.visualizations_input_dict["make_behavioral_videos"]["spectrogram_ch"] = (
            int(self.spectrogram_ch)
        )

        self.visualizations_input_dict["make_behavioral_videos"]["raster_plot_bool"] = (
            self.raster_plot_cb_bool
        )
        self.raster_plot_cb_bool = False

        self.visualizations_input_dict["make_behavioral_videos"][
            "raster_special_units"
        ] = self.raster_special_units.text().split(",")

        self.visualizations_input_dict["make_behavioral_videos"]["spike_sound_bool"] = (
            self.spike_sound_cb_bool
        )
        self.spike_sound_cb_bool = False

        self.visualizations_input_dict["make_behavioral_videos"][
            "beh_features_bool"
        ] = self.beh_features_cb_bool
    ...
    def _save_process_labels_func(self) -> None:
        ...
        qlabel_strings = [
            ...
            "ch_receiving_input",
            "a_ch_receiving_input",
            "pc_usage_process",
            "min_spike_num",
            "phidget_extra_data_camera",
            "hpss_power",
            "sleap_conda",
            "n_deriv_smooth",
            "das_conda",
        ...
        for one_elem_lst in lists_in_string:
            if type(self.__dict__[one_elem_lst]) != str:
                self.__dict__[one_elem_lst] = (
                    self.__dict__[one_elem_lst].text().split(",")
                )

        if os.name == "nt":
            self.processing_dir_edit = self.processing_dir_edit.toPlainText().replace(
                os.sep, "\\"
            )
        else:
        ...
        if os.name == "nt":
            self.exp_codes_edit = self.exp_codes_edit.toPlainText().replace(
                os.sep, "\\"
            )
        else:
        ...
        if not self.inference_root_dir_btn_clicked_flag:
            self.processing_input_dict["prepare_cluster_job"]["inference_root_dir"] = (
                self.inference_root_dir_edit.text()
            )

        if not self.centroid_model_btn_clicked_flag:
            self.processing_input_dict["prepare_cluster_job"]["centroid_model_path"] = (
                self.centroid_model_edit.text()
            )

        if not self.centered_instance_btn_btn_clicked_flag:
            self.processing_input_dict["prepare_cluster_job"][
                "centered_instance_model_path"
            ] = self.centered_instance_model_edit.text()

        if not self.calibration_file_loc_btn_clicked_flag:
            self.processing_input_dict["anipose_operations"]["ConvertTo3D"][
                "conduct_anipose_triangulation"
            ]["calibration_file_loc"] = self.calibration_file_loc_edit.text()
            self.processing_input_dict["anipose_operations"]["ConvertTo3D"][
                "translate_rotate_metric"
            ]["original_arena_file_loc"] = self.calibration_file_loc_edit.text()

        if not self.das_model_dir_btn_clicked_flag:
            self.processing_input_dict["usv_inference"]["FindMouseVocalizations"][
                "das_command_line_inference"
            ]["das_model_directory"] = self.das_model_dir_edit.text()

        if not self.vcl_model_dir_btn_clicked_flag:
            self.processing_input_dict["vocalocator"]["vcl_model_directory"] = (
                self.vcl_model_dir_edit.text()
            )

        self.processing_input_dict["synchronize_files"]["Synchronizer"][
            "crop_wav_files_to_video"
        ]["device_receiving_input"] = str(self.device_receiving_input)
        self.device_receiving_input = self.processing_input_dict["synchronize_files"][
            "Synchronizer"
        ]["crop_wav_files_to_video"]["device_receiving_input"]

        self.processing_input_dict["send_email"]["Messenger"][
            "processing_pc_choice"
        ] = str(self.processing_pc_choice)
        ...
        self.processing_input_dict["modify_files"]["Operator"]["rectify_video_fps"][
            "constant_rate_factor"
        ] = int(round(ast.literal_eval(self.constant_rate_factor)))
        self.processing_input_dict["modify_files"]["Operator"]["rectify_video_fps"][
            "encoding_preset"
        ] = str(self.encoding_preset)
        self.processing_input_dict["synchronize_files"]["Synchronizer"][
            "crop_wav_files_to_video"
        ]["triggerbox_ch_receiving_input"] = int(
            ast.literal_eval(self.ch_receiving_input)
        )
        self.processing_input_dict["modify_files"]["Operator"]["filter_audio_files"][
            "filter_freq_bounds"
        ] = [
            int(ast.literal_eval(freq_bound)) for freq_bound in self.filter_freq_bounds
        ]
        self.processing_input_dict["modify_files"]["Operator"]["hpss_audio"][
            "stft_window_length_hop_size"
        ] = [int(ast.literal_eval(stft_value)) for stft_value in self.stft_window_hop]
        self.processing_input_dict["modify_files"]["Operator"]["hpss_audio"][
            "kernel_size"
        ] = tuple(
            [
                int(ast.literal_eval(kernel_value))
                for kernel_value in self.hpss_kernel_size
            ]
        )
        self.processing_input_dict["modify_files"]["Operator"]["hpss_audio"][
            "hpss_power"
        ] = float(ast.literal_eval(self.hpss_power))
        self.processing_input_dict["modify_files"]["Operator"]["hpss_audio"][
            "margin"
        ] = tuple(
            [int(ast.literal_eval(margin_value)) for margin_value in self.hpss_margin]
        )
        self.processing_input_dict["modify_files"]["Operator"]["get_spike_times"][
            "min_spike_num"
        ] = int(ast.literal_eval(self.min_spike_num))
        self.processing_input_dict["synchronize_files"]["Synchronizer"][
            "find_audio_sync_trains"
        ]["sync_ch_receiving_input"] = int(ast.literal_eval(self.a_ch_receiving_input))
        self.processing_input_dict["extract_phidget_data"]["Gatherer"][
            "prepare_data_for_analyses"
        ]["extra_data_camera"] = self.phidget_extra_data_camera

        self.processing_input_dict["preprocess_data"]["root_directories"] = (
            self.processing_dir_edit
        )
        self.processing_input_dict["anipose_operations"]["ConvertTo3D"][
            "translate_rotate_metric"
        ]["experimental_codes"] = self.exp_codes_edit
        self.processing_input_dict["send_email"]["Messenger"]["send_message"][
            "receivers"
        ] = self.pc_usage_process
        self.processing_input_dict["synchronize_files"]["Synchronizer"][
            "find_video_sync_trains"
        ]["sync_camera_serial_num"] = self.v_camera_serial_num
        self.processing_input_dict["modify_files"]["Operator"]["filter_audio_files"][
            "filter_dirs"
        ] = self.filter_dirs
        self.processing_input_dict["modify_files"]["Operator"][
            "concatenate_audio_files"
        ]["concat_dirs"] = self.concat_dirs

        self.processing_input_dict["usv_inference"]["FindMouseVocalizations"][
            "das_command_line_inference"
        ]["das_conda_env_name"] = self.das_conda
        self.processing_input_dict["usv_inference"]["FindMouseVocalizations"][
            "das_command_line_inference"
        ]["model_name_base"] = self.das_model_base
        self.processing_input_dict["usv_inference"]["FindMouseVocalizations"][
            "das_command_line_inference"
        ]["output_file_type"] = self.das_output_type

        self.processing_input_dict["usv_inference"]["FindMouseVocalizations"][
            "das_command_line_inference"
        ]["segment_confidence_threshold"] = float(
            ast.literal_eval(self.segment_confidence_threshold)
        )
        self.processing_input_dict["usv_inference"]["FindMouseVocalizations"][
            "das_command_line_inference"
        ]["segment_minlen"] = float(ast.literal_eval(self.segment_minlen))
        self.processing_input_dict["usv_inference"]["FindMouseVocalizations"][
            "das_command_line_inference"
        ]["segment_fillgap"] = float(ast.literal_eval(self.segment_fillgap))

        self.processing_input_dict["anipose_operations"]["ConvertTo3D"][
            "sleap_file_conversion"
        ]["sleap_conda_env_name"] = self.sleap_conda
        if self.frame_restriction == [""]:
            ...
            self.processing_input_dict["anipose_operations"]["ConvertTo3D"][
                ...
                int(ast.literal_eval(fr_value)) for fr_value in self.frame_restriction
            ]
        ...
        if self.rigid_body_constraints == "":
            ...
            self.processing_input_dict["anipose_operations"]["ConvertTo3D"][
                ...
                ast.literal_eval(self.rigid_body_constraints)
            )
        if self.weak_body_constraints == "":
            ...
            self.processing_input_dict["anipose_operations"]["ConvertTo3D"][
                ...
                ast.literal_eval(self.weak_body_constraints)
            )

        self.processing_input_dict["anipose_operations"]["ConvertTo3D"][
            "conduct_anipose_triangulation"
        ]["smooth_scale"] = int(ast.literal_eval(self.smooth_scale))
        self.processing_input_dict["anipose_operations"]["ConvertTo3D"][
            "conduct_anipose_triangulation"
        ]["weight_rigid"] = int(ast.literal_eval(self.weight_rigid))
        self.processing_input_dict["anipose_operations"]["ConvertTo3D"][
            "conduct_anipose_triangulation"
        ]["weight_weak"] = int(ast.literal_eval(self.weight_weak))
        self.processing_input_dict["anipose_operations"]["ConvertTo3D"][
            "conduct_anipose_triangulation"
        ]["reprojection_error_threshold"] = int(
            ast.literal_eval(self.reprojection_error_threshold)
        )
        self.processing_input_dict["anipose_operations"]["ConvertTo3D"][
            "conduct_anipose_triangulation"
        ]["regularization_function"] = self.regularization_function
        self.processing_input_dict["anipose_operations"]["ConvertTo3D"][
            "conduct_anipose_triangulation"
        ]["n_deriv_smooth"] = int(ast.literal_eval(self.n_deriv_smooth))
        self.processing_input_dict["anipose_operations"]["ConvertTo3D"][
            "translate_rotate_metric"
        ]["static_reference_len"] = float(ast.literal_eval(self.static_reference_len))

        self.processing_input_dict["processing_booleans"][
            "conduct_video_concatenation"
        ] = self.conduct_video_concatenation_cb_bool
        self.conduct_video_concatenation_cb_bool = False
        self.processing_input_dict["processing_booleans"][
            "conduct_video_fps_change"
        ] = self.conduct_video_fps_change_cb_bool
        self.conduct_video_fps_change_cb_bool = False
        self.processing_input_dict["modify_files"]["Operator"]["rectify_video_fps"][
            "delete_old_file"
        ] = self.delete_con_file_cb_bool
        self.delete_con_file_cb_bool = True
        self.processing_input_dict["processing_booleans"][
            "conduct_audio_multichannel_to_single_ch"
        ] = self.conduct_multichannel_conversion_cb_bool
        self.conduct_multichannel_conversion_cb_bool = False
        self.processing_input_dict["processing_booleans"]["conduct_audio_cropping"] = (
            self.crop_wav_cam_cb_bool
        )
        self.crop_wav_cam_cb_bool = False
        self.processing_input_dict["processing_booleans"]["conduct_audio_to_mmap"] = (
            self.conc_audio_cb_bool
        )
        self.conc_audio_cb_bool = False
        self.processing_input_dict["processing_booleans"]["conduct_audio_filtering"] = (
            self.filter_audio_cb_bool
        )
        self.filter_audio_cb_bool = False
        self.processing_input_dict["processing_booleans"]["conduct_hpss"] = (
            self.conduct_hpss_cb_bool
        )
        self.conduct_hpss_cb_bool = False
        self.processing_input_dict["processing_booleans"][
            "conduct_audio_video_sync"
        ] = self.conduct_sync_cb_bool
        self.conduct_sync_cb_bool = False
        self.processing_input_dict["processing_booleans"][
            "conduct_ephys_video_sync"
        ] = self.conduct_nv_sync_cb_bool
        self.conduct_nv_sync_cb_bool = False
        self.processing_input_dict["processing_booleans"][
            "conduct_ephys_file_chaining"
        ] = self.conduct_ephys_file_chaining_cb_bool
        self.conduct_ephys_file_chaining_cb_bool = False
        self.processing_input_dict["processing_booleans"]["split_cluster_spikes"] = (
            self.split_cluster_spikes_cb_bool
        )
        self.split_cluster_spikes_cb_bool = False
        self.processing_input_dict["processing_booleans"]["prepare_sleap_cluster"] = (
            self.sleap_cluster_cb_bool
        )
        self.sleap_cluster_cb_bool = False
        self.processing_input_dict["processing_booleans"]["sleap_h5_conversion"] = (
            self.sleap_file_conversion_cb_bool
        )
        self.sleap_file_conversion_cb_bool = False
        self.processing_input_dict["processing_booleans"]["anipose_calibration"] = (
            self.anipose_calibration_cb_bool
        )
        self.anipose_calibration_cb_bool = False
        self.processing_input_dict["anipose_operations"]["ConvertTo3D"][
            "conduct_anipose_calibration"
        ]["board_provided_bool"] = self.board_provided_cb_bool
        self.board_provided_cb_bool = False
        self.processing_input_dict["processing_booleans"]["anipose_triangulation"] = (
            self.anipose_triangulation_cb_bool
        )
        self.anipose_triangulation_cb_bool = False
        self.processing_input_dict["anipose_operations"]["ConvertTo3D"][
            "conduct_anipose_triangulation"
        ]["triangulate_arena_points_bool"] = self.triangulate_arena_points_cb_bool
        self.triangulate_arena_points_cb_bool = False
        self.processing_input_dict["anipose_operations"]["ConvertTo3D"][
            "conduct_anipose_triangulation"
        ]["display_progress_bool"] = self.display_progress_cb_bool
        self.display_progress_cb_bool = True
        self.processing_input_dict["anipose_operations"]["ConvertTo3D"][
            "conduct_anipose_triangulation"
        ]["ransac_bool"] = self.ransac_cb_bool
        self.ransac_cb_bool = False
        self.processing_input_dict["processing_booleans"]["anipose_trm"] = (
            self.translate_rotate_metric_cb_bool
        )
        ...
        self.processing_input_dict["anipose_operations"]["ConvertTo3D"][
            "translate_rotate_metric"
        ]["delete_original_h5"] = self.delete_original_h5_cb_bool
        self.delete_original_h5_cb_bool = True
        self.processing_input_dict["processing_booleans"]["das_infer"] = (
            self.das_inference_cb_bool
        )
        self.das_inference_cb_bool = False
        self.processing_input_dict["processing_booleans"]["das_summarize"] = (
            self.das_summary_cb_bool
        )
        self.das_summary_cb_bool = False
        self.processing_input_dict["processing_booleans"][
            "prepare_assign_vocalizations"
        ] = self.prepare_assign_usv_cb_bool
        self.prepare_assign_usv_cb_bool = False
        self.processing_input_dict["processing_booleans"]["assign_vocalizations"] = (
            self.assign_usv_cb_bool
        )
    ...
    def _save_record_one_labels_func(self) -> None:
        ...
        if type(self.recording_files_destination_linux) != str:
            self.recording_files_destination_linux = (
                self.recording_files_destination_linux.text().split(",")
            )
        if type(self.recording_files_destination_windows) != str:
            self.recording_files_destination_windows = (
                self.recording_files_destination_windows.text().split(",")
            )
        ...
        self.exp_settings_dict["recording_files_destination_linux"] = (
            self.recording_files_destination_linux
        )
        self.exp_settings_dict["recording_files_destination_win"] = (
            self.recording_files_destination_windows
        )
        self.exp_settings_dict["video_session_duration"] = ast.literal_eval(
            self.video_session_duration
        )
        self.exp_settings_dict["calibration_duration"] = ast.literal_eval(
            self.calibration_session_duration
        )
        self.exp_settings_dict["ethernet_network"] = self.ethernet_network

        self.exp_settings_dict["conduct_tracking_calibration"] = (
            self.conduct_tracking_calibration_cb_bool
        )
        ...
        if not self.recorder_dir_btn_clicked_flag:
            self.exp_settings_dict["avisoft_recorder_exe"] = (
                self.recorder_settings_edit.text()
            )

        if not self.avisoft_base_dir_btn_clicked_flag:
            self.exp_settings_dict["avisoft_basedirectory"] = (
                self.avisoft_base_edit.text()
            )

        if not self.coolterm_base_dir_btn_clicked_flag:
            self.exp_settings_dict["coolterm_basedirectory"] = (
                self.coolterm_base_edit.text()
            )
            ...
            if variable in self.exp_settings_dict["audio"].keys():
                if variable == "used_mics" or variable == "cpu_affinity":
                    self.exp_settings_dict["audio"][f"{variable}"] = [
                        int(x) for x in getattr(self, variable).text().split(",")
                    ]
                elif variable == "cpu_priority":
                    self.exp_settings_dict["audio"][f"{variable}"] = getattr(
                        self, variable
                    ).text()
                else:
                    self.exp_settings_dict["audio"][f"{variable}"] = ast.literal_eval(
                        getattr(self, variable).text()
                    )
            elif variable in self.exp_settings_dict["audio"]["general"].keys():
                self.exp_settings_dict["audio"]["general"][f"{variable}"] = (
                    ast.literal_eval(getattr(self, variable).text())
                )
            elif variable in self.exp_settings_dict["audio"]["screen_position"].keys():
                self.exp_settings_dict["audio"]["screen_position"][f"{variable}"] = (
                    ast.literal_eval(getattr(self, variable).text())
                )
            elif variable in self.exp_settings_dict["audio"]["devices"].keys():
                self.exp_settings_dict["audio"]["devices"][f"{variable}"] = (
                    ast.literal_eval(getattr(self, variable).text())
                )
            elif variable in self.exp_settings_dict["audio"]["mics_config"].keys():
                if variable == "ditctime":
                    self.exp_settings_dict["audio"]["mics_config"][f"{variable}"] = (
                        getattr(self, variable).text()
                    )
                else:
                    self.exp_settings_dict["audio"]["mics_config"][f"{variable}"] = (
                ...
                self.exp_settings_dict["audio"]["monitor"][f"{variable}"] = (
                    ast.literal_eval(getattr(self, variable).text())
                )
            elif variable in self.exp_settings_dict["audio"]["call"].keys():
                self.exp_settings_dict["audio"]["call"][f"{variable}"] = (
                    ast.literal_eval(getattr(self, variable).text())
                )
    ...
    def _save_record_three_labels_func(self) -> None:
        ...
        self.exp_settings_dict["video"]["general"]["monitor_recording"] = (
            self.monitor_recording_cb_bool
        )
        self.exp_settings_dict["video"]["general"]["monitor_specific_camera"] = (
            self.monitor_specific_camera_cb_bool
        )
        self.exp_settings_dict["video"]["general"]["delete_post_copy"] = (
            self.delete_post_copy_cb_bool
        )

        self.exp_settings_dict["video"]["metadata"]["vacant_arena"] = (
            self.vacant_arena_cb_bool
        )
        self.exp_settings_dict["video"]["metadata"]["ambient_light"] = (
            self.ambient_light_cb_bool
        )
        self.exp_settings_dict["video"]["metadata"]["record_brain"] = (
            self.record_brain_cb_bool
        )
        self.exp_settings_dict["video"]["metadata"]["usv_playback"] = (
            self.usv_playback_cb_bool
        )
        self.exp_settings_dict["video"]["metadata"]["chemogenetics"] = (
            self.chemogenetics_cb_bool
        )
        self.exp_settings_dict["video"]["metadata"]["optogenetics"] = (
            self.optogenetics_cb_bool
        )
        self.exp_settings_dict["video"]["metadata"]["brain_lesion"] = (
            self.brain_lesion_cb_bool
        )
        self.exp_settings_dict["video"]["metadata"]["devocalization"] = (
            self.devocalization_cb_bool
        )
        self.exp_settings_dict["video"]["metadata"]["female_urine"] = (
            self.female_urine_cb_bool
        )
        self.exp_settings_dict["video"]["metadata"]["female_bedding"] = (
            self.female_bedding_cb_bool
        )

        self.exp_settings_dict["video"]["general"]["recording_frame_rate"] = (
            self.cameras_frame_rate.value()
        )
        self.exp_settings_dict["video"]["general"]["calibration_frame_rate"] = (
            self.calibration_frame_rate.value()
        )

        self.exp_settings_dict["video"]["cameras_config"]["21372316"][
            "exposure_time"
        ] = self.exposure_time_21372316.value()
        self.exp_settings_dict["video"]["cameras_config"]["21372316"]["gain"] = (
            self.gain_21372316.value()
        )
        self.exp_settings_dict["video"]["cameras_config"]["21372315"][
            "exposure_time"
        ] = self.exposure_time_21372315.value()
        self.exp_settings_dict["video"]["cameras_config"]["21372315"]["gain"] = (
            self.gain_21372315.value()
        )
        self.exp_settings_dict["video"]["cameras_config"]["21369048"][
            "exposure_time"
        ] = self.exposure_time_21369048.value()
        self.exp_settings_dict["video"]["cameras_config"]["21369048"]["gain"] = (
            self.gain_21369048.value()
        )
        self.exp_settings_dict["video"]["cameras_config"]["22085397"][
            "exposure_time"
        ] = self.exposure_time_22085397.value()
        self.exp_settings_dict["video"]["cameras_config"]["22085397"]["gain"] = (
            self.gain_22085397.value()
        )
        self.exp_settings_dict["video"]["cameras_config"]["21241563"][
            "exposure_time"
        ] = self.exposure_time_21241563.value()
        self.exp_settings_dict["video"]["cameras_config"]["21241563"]["gain"] = (
            self.gain_21241563.value()
        )

        for variable in video_dict_keys:
            if variable != "expected_cameras":
                if variable == "recording_codec":
                    self.exp_settings_dict["video"]["general"][variable] = str(
                        getattr(self, variable)
                    )
                elif variable == "browser" or variable == "specific_camera_serial":
                    self.exp_settings_dict["video"]["general"][variable] = getattr(
                        self, variable
                    ).text()
                elif variable == "notes_entry":
                    self.exp_settings_dict["video"]["metadata"][variable[:-6]] = (
                        getattr(self, variable).toPlainText()
                    )
                else:
                    self.exp_settings_dict["video"]["metadata"][variable[:-6]] = (
                        getattr(self, variable).text()
                ...
                self.exp_settings_dict["video"]["general"][variable] = (
                    self.expected_cameras.split(",")
                )
    ...
    def _save_variables_based_on_exp_id(self) -> None:
        ...
        das_model_dir = configure_path(
            self.processing_input_dict["usv_inference"]["FindMouseVocalizations"][
                "das_command_line_inference"
            ]["das_model_directory"]
        )
        sleap_inference_dir = configure_path(
            self.processing_input_dict["prepare_cluster_job"]["inference_root_dir"]
        )
        vcl_model_dir = configure_path(
            self.processing_input_dict["vocalocator"]["vcl_model_directory"]
        )

        self.das_model_dir_global = replace_name_in_path(
            experimenter_list=self.exp_settings_dict["experimenter_list"],
            recording_files_destinations=[das_model_dir],
            exp_id=self.exp_id,
        )

        self.sleap_inference_dir_global = replace_name_in_path(
            experimenter_list=self.exp_settings_dict["experimenter_list"],
            recording_files_destinations=[sleap_inference_dir],
            exp_id=self.exp_id,
        )

        self.vcl_model_dir_global = replace_name_in_path(
            experimenter_list=self.exp_settings_dict["experimenter_list"],
        ...
        self.destination_linux_global = replace_name_in_path(
            experimenter_list=self.exp_settings_dict["experimenter_list"],
            recording_files_destinations=self.exp_settings_dict[
                "recording_files_destination_linux"
            ],
            exp_id=self.exp_id,
        )

        self.destination_win_global = replace_name_in_path(
            experimenter_list=self.exp_settings_dict["experimenter_list"],
            recording_files_destinations=self.exp_settings_dict[
                "recording_files_destination_win"
            ],
            exp_id=self.exp_id,
        )

        self.processing_input_dict["send_email"]["Messenger"]["experimenter"] = (
            f"{self.exp_id}"
        )
        self.analyses_input_dict["send_email"]["experimenter"] = f"{self.exp_id}"

    def _combo_box_fs_channel_id(self, index: int, variable_id: str = None) -> None:
        """
    ...
    def _combo_box_fs_device_id(self, index: int, variable_id: str = None) -> None:
        """
    ...
    def _combo_box_fs_audio_dir(self, index: int, variable_id: str = None) -> None:
        """
    ...
    def _combo_box_spectrogram_ch(self, index: int, variable_id: str = None) -> None:
        """
    ...
    def _combo_box_view_angle(self, index: int, variable_id: str = None) -> None:
        """
    ...
    def _combo_box_figure_format(self, index: int, variable_id: str = None) -> None:
        """
    ...
    def _combo_box_plot_theme(self, index: int, variable_id: str = None) -> None:
        """
    ...
    def _combo_box_prior_transformed_tracking_data(
        self, index: int, variable_id: str = None
    ) -> None:
        """
    ...
    def _combo_box_encoding_preset(self, index: int, variable_id: str = None) -> None:
        """
    ...
    def _combo_box_prior_name(self, index: int, variable_id: str = None) -> None:
        """
    ...
    def _combo_box_prior_codec(self, index: int, variable_id: str = None) -> None:
        """
    ...
    def _combo_box_prior_processing_pc_choice(
        self, index: int, variable_id: str = None
    ) -> None:
        """
    ...
    def _combo_box_prior_analyses_pc_choice(
        self, index: int, variable_id: str = None
    ) -> None:
        """
    ...
    def _combo_box_prior_visualizations_pc_choice(
        self, index: int, variable_id: str = None
    ) -> None:
        """
    ...
    def _combo_box_prior_audio_device_camera_input(
        self, index: int, variable_id: str = None
    ) -> None:
        """
    ...
    def _combo_box_prior_true(self, index: int, variable_id: str = None) -> None:
        """
    ...
    def _combo_box_prior_false(self, index: int, variable_id: str = None) -> None:
        """
    ...
    def _update_exposure_time_label(self, value: int, variable_id: str = None) -> None:
        ...
        self.__dict__[variable_id].setText(f"exp time ({value!s} μs):")
    ...
    def _update_gain_label(self, value: int, variable_id: str = None) -> None:
        ...
        self.__dict__[variable_id].setText(f"digital gain ({value!s} dB):")
    ...
    def _update_fr_label(self, value: int) -> None:
        ...
        self.fr_label.setText(f"Recording ({value!s} fps):")
    ...
    def _update_cal_fr_label(self, value: int) -> None:
        ...
        self.cal_fr_label.setText(f"Calibration ({value!s} fps):")

    def _create_sliders_general(
        self, camera_id: str = None, camera_color: str = None, y_start: int = None
    ) -> None:
        """
        ...
        specific_camera_label = QLabel(
            f"Camera {camera_id} ({camera_color})", self.VideoSettings
        )
        ...
        self.__dict__[f"exposure_time_{camera_id}_label"] = QLabel(
            "exp time (2500 μs)", self.VideoSettings
        )
        self.__dict__[f"exposure_time_{camera_id}_label"].setFixedWidth(150)
        self.__dict__[f"exposure_time_{camera_id}_label"].setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        self.__dict__[f"exposure_time_{camera_id}_label"].move(25, y_start + 30)
        self.__dict__[f"exposure_time_{camera_id}"] = QSlider(
            Qt.Orientation.Horizontal, self.VideoSettings
        )
        self.__dict__[f"exposure_time_{camera_id}"].setFixedWidth(150)
        self.__dict__[f"exposure_time_{camera_id}"].setRange(500, 30000)
        self.__dict__[f"exposure_time_{camera_id}"].setValue(
            self.__dict__[f"{camera_id}_et"]
        )
        self.__dict__[f"exposure_time_{camera_id}"].move(5, y_start + 60)
        self.__dict__[f"exposure_time_{camera_id}"].valueChanged.connect(
            partial(
                self._update_exposure_time_label,
                variable_id=f"exposure_time_{camera_id}_label",
            )
        )

        self.__dict__[f"gain_{camera_id}_label"] = QLabel(
            "digital gain (0 dB)", self.VideoSettings
        )
        self.__dict__[f"gain_{camera_id}_label"].setFixedWidth(150)
        self.__dict__[f"gain_{camera_id}_label"].setFont(
            QFont(self.font_id, 10 + self.font_size_increase)
        )
        self.__dict__[f"gain_{camera_id}_label"].move(200, y_start + 30)
        self.__dict__[f"gain_{camera_id}"] = QSlider(
            Qt.Orientation.Horizontal, self.VideoSettings
        )
        ...
        self.__dict__[f"gain_{camera_id}"].valueChanged.connect(
            partial(self._update_gain_label, variable_id=f"gain_{camera_id}_label")
        )
    ...
    def _create_buttons_main(self) -> None:
        ...
        self.button_map = {
            "Process": QPushButton(QIcon(process_icon), "Process", self.Main),
        ...
        self.button_map["Record"].setFont(
            QFont(self.font_id, 8 + self.font_size_increase)
        )
        ...
        self.button_map["Process"].setFont(
            QFont(self.font_id, 8 + self.font_size_increase)
        )
    ...
    def _create_buttons_record(
        ...
        if len(next_win_connect) > 0:
            ...
            if self.exp_settings_dict["conduct_tracking_calibration"]:
                self.button_map["Calibrate"] = QPushButton(
                    QIcon(calibrate_icon), "Calibrate", class_option
                )
                self.button_map["Calibrate"].move(next_button_x_pos - 95, button_pos_y)
                self.button_map["Calibrate"].setFont(
                    QFont(self.font_id, 8 + self.font_size_increase)
                )
                self.button_map["Calibrate"].clicked.connect(
                    self._disable_other_buttons
                )
                self.button_map["Calibrate"].clicked.connect(self._start_calibration)
                self.button_map["Calibrate"].clicked.connect(
                    self._enable_other_buttons_post_cal
                )

            self.button_map["Record"] = QPushButton(
                QIcon(record_icon), "Record", class_option
            )
            ...
            self.button_map["Record"].clicked.connect(
                self._enable_other_buttons_post_rec
            )

    def _create_buttons_process(
        self,
        seq: int = None,
        class_option: str = None,
        button_pos_y: int = None,
        ...
        if seq == 0:
            ...
            self.button_map["Process"] = QPushButton(
                QIcon(process_icon), "Process", class_option
            )
    ...
    def _create_buttons_analyze(
        ...
        if seq == 0:
            ...
            self.button_map["Analyze"] = QPushButton(
                QIcon(analyze_icon), "Analyze", class_option
            )
    ...
    def _create_buttons_visualize(
        ...
        if seq == 0:
            previous_win = self.main_window
            next_window_connect = [
                self._save_visualizations_labels_func,
                self.visualize_two,
            ]
        else:
        ...
        if seq == 0:
            ...
            self.button_map["Visualize"] = QPushButton(
                QIcon(visualize_icon), "Visualize", class_option
            )
            self.button_map["Visualize"].move(next_button_x_pos, button_pos_y)
            self.button_map["Visualize"].setFont(
                QFont(self.font_id, 8 + self.font_size_increase)
            )
            self.button_map["Visualize"].clicked.connect(
                self._disable_visualize_buttons
            )
    ...
    def _sequence_playback_file_dialog(self) -> None:
        ...
        sequence_audio_file_name = QFileDialog.getOpenFileNames(
            self, "Select audible USV sequence .WAV file", "", "Wave Files (*.wav)"
        )
        if sequence_audio_file_name:
            ...
            if os.name == "nt":
                self.visualizations_input_dict["make_behavioral_videos"][
                    "sequence_audio_file"
                ] = str(sequence_audio_file_name_path).replace(os.sep, "\\")
            else:
                self.visualizations_input_dict["make_behavioral_videos"][
                    "sequence_audio_file"
                ] = str(sequence_audio_file_name_path)
        else:
            self.visualizations_input_dict["make_behavioral_videos"][
                "sequence_audio_file"
            ] = self.sequence_audio_file_edit.text()
    ...
    def _speaker_playback_file_dialog(self) -> None:
        ...
        speaker_audio_file_name = QFileDialog.getOpenFileNames(
            self, "Select playback speaker .WAV file", "", "Wave Files (*.wav)"
        )
        if speaker_audio_file_name:
            ...
            if os.name == "nt":
                self.visualizations_input_dict["make_behavioral_videos"][
                    "speaker_audio_file"
                ] = str(speaker_audio_file_name_path).replace(os.sep, "\\")
            else:
                self.visualizations_input_dict["make_behavioral_videos"][
                    "speaker_audio_file"
                ] = str(speaker_audio_file_name_path)
        else:
            self.visualizations_input_dict["make_behavioral_videos"][
                "speaker_audio_file"
            ] = self.speaker_audio_file_edit.text()
    ...
    def _open_arena_tracking_dialog(self) -> None:
        ...
        arena_tracking_dir_name = QFileDialog.getExistingDirectory(
            self, "Select arena tracking root directory", ""
        )
        if arena_tracking_dir_name:
            ...
            if os.name == "nt":
                self.visualizations_input_dict["make_behavioral_videos"][
                    "arena_directory"
                ] = str(arena_tracking_dir_name_path).replace(os.sep, "\\")
            else:
                self.visualizations_input_dict["make_behavioral_videos"][
                    "arena_directory"
                ] = str(arena_tracking_dir_name_path)
        else:
            self.visualizations_input_dict["make_behavioral_videos"][
                "arena_directory"
            ] = self.arena_root_directory_edit.text()
    ...
    def _open_centroid_dialog(self) -> None:
        ...
        centroid_dir_name = QFileDialog.getExistingDirectory(
            self, "Select SLEAP centroid model directory", ""
        )
        if centroid_dir_name:
            ...
            if os.name == "nt":
                self.processing_input_dict["prepare_cluster_job"][
                    "centroid_model_path"
                ] = str(centroid_dir_name_path).replace(os.sep, "\\") + "\\"
            else:
                self.processing_input_dict["prepare_cluster_job"][
                    "centroid_model_path"
                ] = str(centroid_dir_name_path)
        else:
            self.processing_input_dict["prepare_cluster_job"]["centroid_model_path"] = (
                self.centroid_model_edit.text()
            )
    ...
    def _open_centered_instance_dialog(self) -> None:
        ...
        centered_instance_dir_name = QFileDialog.getExistingDirectory(
            self, "Select SLEAP centered instance model directory", ""
        )
        if centered_instance_dir_name:
            centered_instance_dir_name_path = Path(centered_instance_dir_name)
            self.centered_instance_model_edit.setText(
                str(centered_instance_dir_name_path)
            )
            if os.name == "nt":
                self.processing_input_dict["prepare_cluster_job"][
                    "centered_instance_model_path"
                ] = str(centered_instance_dir_name_path).replace(os.sep, "\\") + "\\"
            else:
                self.processing_input_dict["prepare_cluster_job"][
                    "centered_instance_model_path"
                ] = str(centered_instance_dir_name_path)
    ...
    def _open_inference_root_dialog(self) -> None:
        ...
        inference_root_dir_name = QFileDialog.getExistingDirectory(
            self, "Select SLEAP inference directory", ""
        )
        if inference_root_dir_name:
            ...
            if os.name == "nt":
                self.processing_input_dict["prepare_cluster_job"][
                    "inference_root_dir"
                ] = str(inference_root_dir_name_path).replace(os.sep, "\\") + "\\"
            else:
                self.processing_input_dict["prepare_cluster_job"][
                    "inference_root_dir"
                ] = str(inference_root_dir_name_path)
        else:
            self.processing_input_dict["prepare_cluster_job"]["inference_root_dir"] = (
                self.inference_root_dir_edit.text()
            )
    ...
    def _open_recorder_dialog(self) -> None:
        ...
        if recorder_dir_name:
            ...
            if os.name == "nt":
                self.exp_settings_dict["avisoft_recorder_exe"] = str(
                    recorder_dir_name_path
                ).replace(os.sep, "\\")
            else:
                self.exp_settings_dict["avisoft_recorder_exe"] = str(
                    recorder_dir_name_path
    ...
    def _open_avisoft_dialog(self) -> None:
        ...
        avisoft_dir_name = QFileDialog.getExistingDirectory(
            self, "Select Avisoft base directory", f"{self.avisoft_base_dir_global}"
        )
        if avisoft_dir_name:
            ...
            if os.name == "nt":
                self.exp_settings_dict["avisoft_basedirectory"] = (
                    str(avisoft_dir_name_path).replace(os.sep, "\\") + "\\"
                )
            else:
                self.exp_settings_dict["avisoft_basedirectory"] = str(
                    avisoft_dir_name_path
                )
        else:
            self.exp_settings_dict["avisoft_basedirectory"] = (
    ...
    def _open_coolterm_dialog(self) -> None:
        ...
        coolterm_dir_name = QFileDialog.getExistingDirectory(
            self, "Select Coolterm base directory", f"{self.coolterm_base_dir_global}"
        )
        if coolterm_dir_name:
            ...
            if os.name == "nt":
                self.exp_settings_dict["coolterm_basedirectory"] = str(
                    coolterm_dir_name_path
                ).replace(os.sep, "\\")
            else:
                self.exp_settings_dict["coolterm_basedirectory"] = str(
                    coolterm_dir_name_path
    ...
    def _open_anipose_calibration_dialog(self) -> None:
        ...
        anipose_cal_dir_name = QFileDialog.getExistingDirectory(
            self, "Select Anipose calibration root directory", ""
        )
        if anipose_cal_dir_name:
            ...
            if os.name == "nt":
                self.processing_input_dict["anipose_operations"]["ConvertTo3D"][
                    "conduct_anipose_triangulation"
                ]["calibration_file_loc"] = str(anipose_cal_dir_name_path).replace(
                    os.sep, "\\"
                )
                self.processing_input_dict["anipose_operations"]["ConvertTo3D"][
                    "translate_rotate_metric"
                ]["original_arena_file_loc"] = str(anipose_cal_dir_name_path).replace(
                    os.sep, "\\"
                )
            else:
                self.processing_input_dict["anipose_operations"]["ConvertTo3D"][
                    "conduct_anipose_triangulation"
                ]["calibration_file_loc"] = str(anipose_cal_dir_name_path)
                self.processing_input_dict["anipose_operations"]["ConvertTo3D"][
                    "translate_rotate_metric"
                ]["original_arena_file_loc"] = str(anipose_cal_dir_name_path)
        else:
            self.processing_input_dict["anipose_operations"]["ConvertTo3D"][
                "conduct_anipose_triangulation"
            ]["calibration_file_loc"] = self.calibration_file_loc_edit.text()
    ...
    def _open_vcl_model_dialog(self) -> None:
        ...
        vcl_model_dir_name = QFileDialog.getExistingDirectory(
            self, "Select Vocalocator model directory", ""
        )
        if vcl_model_dir_name:
            ...
            if os.name == "nt":
                self.processing_input_dict["vocalocator"]["vcl_model_directory"] = str(
                    vcl_model_dir_name_path
                ).replace(os.sep, "\\")
            else:
                self.processing_input_dict["vocalocator"]["vcl_model_directory"] = str(
                    vcl_model_dir_name_path
    ...
    def _open_das_model_dialog(self) -> None:
        ...
        das_model_dir_name = QFileDialog.getExistingDirectory(
            self, "Select DAS model directory", ""
        )
        if das_model_dir_name:
            ...
            if os.name == "nt":
                self.processing_input_dict["usv_inference"]["FindMouseVocalizations"][
                    "das_command_line_inference"
                ]["das_model_directory"] = str(das_model_dir_name_path).replace(
                    os.sep, "\\"
                )
            else:
                self.processing_input_dict["usv_inference"]["FindMouseVocalizations"][
                    "das_command_line_inference"
                ]["das_model_directory"] = str(das_model_dir_name_path)
        else:
            self.processing_input_dict["usv_inference"]["FindMouseVocalizations"][
                "das_command_line_inference"
            ]["das_model_directory"] = self.das_model_dir_edit.text()
...
def main() -> None:
    ...
    with open(Path(__file__).parent / "_config/gui_style_sheet.css") as file:
    ...
    initial_values_dict = {
        "exp_id": _toml["video"]["metadata"]["experimenter"],
        "conduct_audio_cb_bool": _toml["conduct_audio_recording"],
        "conduct_tracking_calibration_cb_bool": _toml["conduct_tracking_calibration"],
        "disable_ethernet_cb_bool": _toml["disable_ethernet"],
        "monitor_recording_cb_bool": _toml["video"]["general"]["monitor_recording"],
        "monitor_specific_camera_cb_bool": _toml["video"]["general"][
            "monitor_specific_camera"
        ],
        "delete_post_copy_cb_bool": _toml["video"]["general"]["delete_post_copy"],
        "vacant_arena_cb_bool": _toml["video"]["metadata"]["vacant_arena"],
        "ambient_light_cb_bool": _toml["video"]["metadata"]["ambient_light"],
        "record_brain_cb_bool": _toml["video"]["metadata"]["record_brain"],
        "usv_playback_cb_bool": _toml["video"]["metadata"]["usv_playback"],
        "chemogenetics_cb_bool": _toml["video"]["metadata"]["chemogenetics"],
        "optogenetics_cb_bool": _toml["video"]["metadata"]["optogenetics"],
        "brain_lesion_cb_bool": _toml["video"]["metadata"]["brain_lesion"],
        "devocalization_cb_bool": _toml["video"]["metadata"]["devocalization"],
        "female_urine_cb_bool": _toml["video"]["metadata"]["female_urine"],
        "female_bedding_cb_bool": _toml["video"]["metadata"]["female_bedding"],
        "recording_codec": _toml["video"]["general"]["recording_codec"],
        "21372315_et": _toml["video"]["cameras_config"]["21372315"]["exposure_time"],
        "21372315_dg": _toml["video"]["cameras_config"]["21372315"]["gain"],
        "21372316_et": _toml["video"]["cameras_config"]["21372316"]["exposure_time"],
        "21372316_dg": _toml["video"]["cameras_config"]["21372316"]["gain"],
        "21369048_et": _toml["video"]["cameras_config"]["21369048"]["exposure_time"],
        "21369048_dg": _toml["video"]["cameras_config"]["21369048"]["gain"],
        "22085397_et": _toml["video"]["cameras_config"]["22085397"]["exposure_time"],
        "22085397_dg": _toml["video"]["cameras_config"]["22085397"]["gain"],
        "21241563_et": _toml["video"]["cameras_config"]["21241563"]["exposure_time"],
        "21241563_dg": _toml["video"]["cameras_config"]["21241563"]["gain"],
        "inference_root_dir_btn_clicked_flag": False,
        "centroid_model_btn_clicked_flag": False,
        "centered_instance_btn_btn_clicked_flag": False,
        "calibration_file_loc_btn_clicked_flag": False,
        "das_model_dir_btn_clicked_flag": False,
        "recorder_dir_btn_clicked_flag": False,
        "avisoft_base_dir_btn_clicked_flag": False,
        "coolterm_base_dir_btn_clicked_flag": False,
        "device_receiving_input": processing_input_dict["synchronize_files"][
            "Synchronizer"
        ]["crop_wav_files_to_video"]["device_receiving_input"],
        "save_transformed_data": processing_input_dict["anipose_operations"][
            "ConvertTo3D"
        ]["translate_rotate_metric"]["save_transformed_data"],
        "conduct_video_concatenation_cb_bool": False,
        "conduct_video_fps_change_cb_bool": False,
        "conduct_multichannel_conversion_cb_bool": False,
        "crop_wav_cam_cb_bool": False,
        "conc_audio_cb_bool": False,
        "filter_audio_cb_bool": False,
        "conduct_sync_cb_bool": False,
        "conduct_hpss_cb_bool": False,
        "conduct_ephys_file_chaining_cb_bool": False,
        "conduct_nv_sync_cb_bool": False,
        "split_cluster_spikes_cb_bool": False,
        "anipose_calibration_cb_bool": False,
        "sleap_file_conversion_cb_bool": False,
        "anipose_triangulation_cb_bool": False,
        "translate_rotate_metric_cb_bool": False,
        "sleap_cluster_cb_bool": False,
        "das_inference_cb_bool": False,
        "das_summary_cb_bool": False,
        "assign_usv_cb_bool": False,
        "prepare_assign_usv_cb_bool": False,
        "delete_con_file_cb_bool": True,
        "board_provided_cb_bool": False,
        "triangulate_arena_points_cb_bool": False,
        "display_progress_cb_bool": True,
        "ransac_cb_bool": False,
        "delete_original_h5_cb_bool": True,
        "compute_behavioral_features_cb_bool": False,
        "plot_behavioral_tuning_cb_bool": False,
        "make_behavioral_video_cb_bool": False,
        "visualization_type_cb_bool": False,
        "plot_theme": visualizations_input_dict["make_behavioral_videos"]["plot_theme"],
        "fig_format": visualizations_input_dict["make_behavioral_videos"][
            "general_figure_specs"
        ]["fig_format"],
        "view_angle": visualizations_input_dict["make_behavioral_videos"]["view_angle"],
        "rotate_side_view_bool": False,
        "history_cb_bool": False,
        "speaker_cb_bool": False,
        "spectrogram_cb_bool": False,
        "spectrogram_ch": visualizations_input_dict["make_behavioral_videos"][
            "spectrogram_ch"
        ],
        "raster_plot_cb_bool": False,
        "spike_sound_cb_bool": False,
        "beh_features_cb_bool": False,
        "calculate_neuronal_tuning_curves_cb_bool": False,
        "create_usv_playback_wav_cb_bool": False,
        "frequency_shift_audio_segment_cb_bool": False,
        "fs_audio_dir": analyses_input_dict["frequency_shift_audio_segment"][
            "fs_audio_dir"
        ],
        "fs_device_id": analyses_input_dict["frequency_shift_audio_segment"][
            "fs_device_id"
        ],
        "fs_channel_id": analyses_input_dict["frequency_shift_audio_segment"][
            "fs_channel_id"
        ],
        "volume_adjust_audio_segment_cb_bool": True,
        "visualizations_pc_choice": visualizations_input_dict["send_email"][
            "visualizations_pc_choice"
        ],
        "analyses_pc_choice": analyses_input_dict["send_email"]["analyses_pc_choice"],
        "processing_pc_choice": processing_input_dict["send_email"]["Messenger"][
            "processing_pc_choice"
        ],
        "encoding_preset": processing_input_dict["modify_files"]["Operator"][
            "rectify_video_fps"
        ]["encoding_preset"],
    }
