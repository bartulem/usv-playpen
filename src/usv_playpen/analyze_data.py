...
class Analyst:
    def __init__(
        ...
        if root_directories is None:
            with open(
                ...
                self.root_directories = json.load(json_file)["analyze_data"][
                    "root_directories"
                ]
        else:
    ...
    def analyze_data(self) -> None:
        ...
        Messenger(
            ...
            subject=f"{self.input_parameter_dict['send_email']['analyses_pc_choice']} PC is busy, do NOT attempt to remote in!",
        ...
        if self.input_parameter_dict["analyses_booleans"][
            "create_usv_playback_wav_bool"
        ]:
            AudioGenerator(
                exp_id=self.input_parameter_dict["send_email"]["experimenter"],
                create_playback_settings_dict=self.input_parameter_dict[
        ...
        for one_directory in self.root_directories:
            try:
                self.message_output(
                    f"Analyzing data in {one_directory} started at: "
                    f"{datetime.now().hour:02d}:{datetime.now().minute:02d}.{datetime.now().second:02d}."
                )
                ...
                if self.input_parameter_dict["analyses_booleans"][
                    ...
                    FeatureZoo(
                        ...
                        behavioral_parameters_dict=self.input_parameter_dict[
                            "compute_behavioral_features"
                        ],
                        message_output=self.message_output,
                    ).save_behavioral_features_to_file()
                ...
                if self.input_parameter_dict["analyses_booleans"][
                    ...
                    NeuronalTuning(
                        root_directory=one_directory,
                        tuning_parameters_dict=self.input_parameter_dict[
                ...
                if self.input_parameter_dict["analyses_booleans"][
                    ...
                    AudioGenerator(
                        root_directory=one_directory,
                        freq_shift_settings_dict=self.input_parameter_dict[
                ...
                self.message_output(
                    f"Analyzing data in {one_directory} finished at: "
                    f"{datetime.now().hour:02d}:{datetime.now().minute:02d}.{datetime.now().second:02d}."
                )
        ...
        Messenger(
            ...
            subject=f"{self.input_parameter_dict['send_email']['analyses_pc_choice']} PC is available again, analyses have been completed",
...
@click.option(
    "--exp-id", "experimenter", type=str, required=True, help="Experimenter ID."
)
@click.option(
    "--num-usv-files",
    "num_usv_files",
    ...
    default=None,
    required=False,
    help="Number of WAV files to create.",
)
@click.option(
    "--total-usv-number",
    "total_usv_number",
    ...
    default=None,
    required=False,
    help="Total number of USVs to distribute across files.",
)
@click.option(
    "--ipi-duration",
    "ipi_duration",
    ...
    default=None,
    required=False,
    help="Inter-USV-interval duration (in s).",
)
@click.option(
    "--wav-sampling-rate",
    "wav_sampling_rate",
    ...
    default=None,
    required=False,
    help="Sampling rate for the output WAV file (in Hz).",
)
@click.option(
    "--playback-snippets-dir",
    "playback_snippets_dir",
    type=str,
    default=None,
    required=False,
    help="Directory of USV playback snippets.",
)
@click.pass_context
def generate_usv_playback_cli(ctx, exp_id, **kwargs) -> None:
    ...
    if not provided_params:
        ...
        analyses_settings_parameter_dict = modify_settings_json_for_cli(
            ctx=ctx, provided_params=provided_params, settings_dict="analyses_settings"
        )
        AudioGenerator(
            exp_id=exp_id,
            create_playback_settings_dict=analyses_settings_parameter_dict,
            message_output=print,
        ).create_usv_playback_wav(spock_cluster_bool=True)
...
@click.option(
    ...
    default=None,
    required=False,
    help="Spike-behavior offsets to consider (in s).",
)
@click.option(
    "--n-shuffles",
    "n_shuffles",
...
@click.option(
    ...
    default=None,
    required=False,
    help="Total number of bins for 1D tuning curves.",
)
@click.option(
    "--n-spatial-bins",
    "n_spatial_bins",
...
@click.option(
    ...
    default=None,
    required=False,
    help="Spatial extent of the arena (in cm).",
)
@click.pass_context
...
@click.option(
    ...
    default=None,
    required=False,
    help="Number of bins for derivative calculation.",
)
@click.pass_context
