...
@click.option('--corr-cutoff', 'noise_corr_cutoff_min', type=float, default=None, required=False, help='Absolute detected-channel correlation cutoff (drop below).')
@click.option('--coherence-cutoff', 'coherence_cutoff_min', type=float, default=None, required=False, help='Absolute top-K spatial-coherence cutoff (drop below).')
@click.option('--coherence-channel-count', 'coherence_channel_count', type=int, default=None, required=False, help='Number of loudest in-band channels the spatial coherence is computed over.')
...
@click.option('--max-usv-duration/--no-max-usv-duration', 'max_usv_duration_bool', default=None, required=False, help='Reject merged intervals longer than --max-usv-duration-s before the noise checks run (default: enabled).')
@click.option('--max-usv-duration-s', 'max_usv_duration_s', type=float, default=None, required=False, help='Longest a merged interval may be (s) before it is rejected as noise; no mouse USV lasts this long.')
@click.option('--consensus-remerge/--no-consensus-remerge', 'consensus_remerge_bool', default=None, required=False, help='Re-merge intervals whose extent is set by one channel contradicting the majority (default: enabled). Pass --no-consensus-remerge to take the union exactly as detected.')
@click.option('--consensus-remerge-min-duration-s', 'consensus_remerge_min_duration_s', type=float, default=None, required=False, help='Consensus re-merge: only intervals longer than this (s) are examined.')
@click.option('--consensus-remerge-span-factor', 'consensus_remerge_span_factor', type=float, default=None, required=False, help='Consensus re-merge: a channel dominates when its longest segment exceeds this multiple of the median longest-segment across contributing channels.')
@click.option('--consensus-remerge-max-dissenting-channels', 'consensus_remerge_max_dissenting_channels', type=int, default=None, required=False, help='Consensus re-merge: fire only when at most this many channels dominate.')
@click.option('--consensus-remerge-min-agreeing-channels', 'consensus_remerge_min_agreeing_channels', type=int, default=None, required=False, help='Consensus re-merge: fire only when at least this many channels remain once dissenting channels are set aside.')
@click.option('--consensus-remerge-min-gap-s', 'consensus_remerge_min_gap_s', type=float, default=None, required=False, help="Consensus re-merge: narrowest gap (s) a cut may sit in; below the segmenter's own --segment-fillgap the gap was manufactured by the union rather than found in the audio.")
@click.option('--consensus-remerge-rescue-dissenting-channels', 'consensus_remerge_rescue_dissenting_channels', type=int, default=None, required=False, help='Consensus re-merge: dissent limit used only for an interval about to be rejected for exceeding --max-usv-duration-s.')
@click.option('--consensus-remerge-max-depth', 'consensus_remerge_max_depth', type=int, default=None, required=False, help='Consensus re-merge: how many times a corrected interval may itself be re-examined; 0 reproduces the single-pass behaviour.')
@click.option('--seam-repair/--no-seam-repair', 'seam_repair_bool', default=None, required=False, help='Run the post-summary seam check-and-repair (default: disabled). Only meaningful for annotations from the legacy non-overlapping model (stride 8128); pass --seam-repair to enable.')
@click.option('--seam-repair-legacy-stride-samples', 'seam_repair_legacy_stride_samples', type=int, default=None, required=False, help='Seam repair: window-stitching stride (samples) of the legacy DAS model whose seam artifact is checked for.')
@click.option('--seam-repair-max-rung', 'seam_repair_max_rung', type=int, default=None, required=False, help='Seam repair: highest stride multiple tested for the seam-ladder fingerprint.')
@click.option('--seam-repair-ladder-tolerance-samples', 'seam_repair_ladder_tolerance_samples', type=int, default=None, required=False, help='Seam repair: maximum deviation (samples) from the exact ladder fingerprint in raw annotations.')
@click.option('--seam-repair-width-tolerance-below-ms', 'seam_repair_width_tolerance_below_ms', type=float, default=None, required=False, help='Seam repair: merged-gap width gate reach below each stride multiple (ms).')
@click.option('--seam-repair-width-tolerance-above-ms', 'seam_repair_width_tolerance_above_ms', type=float, default=None, required=False, help='Seam repair: merged-gap width gate reach above each stride multiple (ms).')
@click.option('--seam-repair-raw-stop-tolerance-s', 'seam_repair_raw_stop_tolerance_s', type=float, default=None, required=False, help='Seam repair: maximum distance (s) between a merged stop and a raw ladder-gap stop for corroboration.')
@click.option('--seam-repair-snippet-margin-s', 'seam_repair_snippet_margin_s', type=float, default=None, required=False, help='Seam repair: audio margin (s) excised around each flagged pair for re-detection.')
@click.option('--seam-repair-max-boundary-shift-s', 'seam_repair_max_boundary_shift_s', type=float, default=None, required=False, help='Seam repair: maximum outward correction (s) permitted per call edge.')
@click.pass_context
