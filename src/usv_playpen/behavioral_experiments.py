...
class ExperimentController:
    ...
    def get_connection_params(self) -> tuple:
        ...
        if not os.path.exists(
            os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "_config/motif_config.ini"
            )
        ):
    ...
    def get_custom_dir_names(self, now: float = None) -> tuple:
        ...
        for lin_directory in self.exp_settings_dict[
    ...
    def check_camera_vitals(self, camera_fr: int | float = None) -> None:
        ...
        try:
            ...
            self.message_output(
                "Motif not running or reachable. Check hardware and connections."
            )
            smart_wait(app_context_bool=self.app_context_bool, seconds=10)
            sys.exit()
        else:
            self.camera_serial_num = [
                camera_dict["serial"] for camera_dict in available_cameras
            ]
            self.message_output(
                f"The system is running Motif v{api.call('version')['software']} "
                f"and {len(available_cameras)} camera(s) is/are online: {self.camera_serial_num}"
            )

            if (
                len(self.camera_serial_num)
                != self.exp_settings_dict["video"]["general"]["expected_camera_num"]
            ):
                self.message_output(
                    f"The number of connected cameras ({len(self.camera_serial_num)}) does not match the expected "
            ...
            # configure cameras
            for serial_num in self.exp_settings_dict["video"]["general"][
                "expected_cameras"
            ]:
                api.call(
                    f"camera/{serial_num}/configure",
                    ExposureTime=self.exp_settings_dict["video"]["cameras_config"][
                        serial_num
                    ]["exposure_time"],
                    Gain=self.exp_settings_dict["video"]["cameras_config"][serial_num][
                        "gain"
                    ],
                )
            ...
            if len(self.exp_settings_dict["video"]["general"]["expected_cameras"]) == 1:
                api.call(
                    f"camera/{self.exp_settings_dict['video']['general']['expected_cameras'][0]}/configure",
                    AcquisitionFrameRate=camera_fr,
                )
                self.message_output(
                    f"The camera frame rate is set to {camera_fr} fps for {self.exp_settings_dict['video']['general']['expected_cameras'][0]}."
                )
            else:
                api.call("cameras/configure", MotifMulticamFrameRate=camera_fr)
                self.message_output(
                    f"The camera frame rate is set to {camera_fr} fps for all available cameras."
                )

            # monitor recording via browser
            if self.exp_settings_dict["video"]["general"]["monitor_recording"]:
                if self.exp_settings_dict["video"]["general"][
                    "monitor_specific_camera"
                ]:
                    meta = api.call(
                        f"camera/{self.exp_settings_dict['video']['general']['specific_camera_serial']}"
                    )
                    webbrowser.open(meta["camera_info"]["stream"]["preview"]["url"])
                else:
                    webbrowser.open(
                        self.exp_settings_dict["video"]["general"]["monitor_url"]
                    )
    ...
    def conduct_tracking_calibration(self) -> None:
        ...
        self.message_output(
            f"Video calibration started at {datetime.datetime.now().hour:02d}:{datetime.datetime.now().minute:02d}.{datetime.datetime.now().second:02d}"
        )

        self.check_camera_vitals(
            camera_fr=self.exp_settings_dict["video"]["general"][
        ...
        smart_wait(
            app_context_bool=self.app_context_bool,
            seconds=(self.exp_settings_dict["calibration_duration"] * 60) + 5,
        )

        self.message_output(
            f"Video calibration completed at {datetime.datetime.now().hour:02d}:{datetime.datetime.now().minute:02d}.{datetime.datetime.now().second:02d}"
        )
    ...
    def modify_audio_file(self) -> None:
        ...
        self.config_1.read(
            os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "_config/avisoft_config.ini"
            )
        )

        if (
            str(self.exp_settings_dict["avisoft_basedirectory"])
            != self.config_1["Configuration"]["basedirectory"]
        ):
            self.config_1["Configuration"]["basedirectory"] = self.exp_settings_dict[
        ...
        for mic_num in range(self.exp_settings_dict["audio"]["total_mic_number"]):
            if mic_num in self.exp_settings_dict["audio"]["used_mics"]:
                if self.config_1["Configuration"][f"active{mic_num}"] != str(1):
                    ...
                    changes += 1
            elif self.config_1["Configuration"][f"active{mic_num}"] != str(0):
                self.config_1["Configuration"][f"active{mic_num}"] = str(0)
                changes += 1

        if self.exp_settings_dict["audio"]["general"]["total"] == 0:
            if not math.isclose(
                (self.exp_settings_dict["video_session_duration"] + 0.41),
                float(self.config_1["MaxFileSize"]["minutes"]),
            ):
                self.config_1["MaxFileSize"]["minutes"] = (
                    f"{(self.exp_settings_dict['video_session_duration'] + 0.41)}"
                )
                changes += 1
        else:
            max_file_size = (
                self.exp_settings_dict["video_session_duration"] + 0.36
            ) / (self.exp_settings_dict["video_session_duration"] / 5)
            if not math.isclose(
                float(self.config_1["MaxFileSize"]["minutes"]), max_file_size
            ):
                self.config_1["MaxFileSize"]["minutes"] = str(max_file_size)
                changes += 1

        if (
            self.config_1["Configuration"]["configfilename"]
        ...
        for general_key in self.exp_settings_dict["audio"]["general"].keys():
            if (
                str(self.exp_settings_dict["audio"]["general"][general_key])
                != self.config_1["Configuration"][general_key]
            ):
                self.config_1["Configuration"][general_key] = str(
                    self.exp_settings_dict["audio"]["general"][general_key]
                )
                changes += 1
        ...
        if self.exp_settings_dict["audio"]["mics_config"]["triggertype"] == 41:
            timer_duration = self.exp_settings_dict["video_session_duration"] + 0.36
            if not math.isclose(
                (timer_duration * 60), float(self.config_1["Configuration"]["timer"])
            ):
        ...
        for screen_pos_key in self.exp_settings_dict["audio"]["screen_position"].keys():
            if (
                str(self.exp_settings_dict["audio"]["screen_position"][screen_pos_key])
                != self.config_1["MainWindowPos"][screen_pos_key]
            ):
                self.config_1["MainWindowPos"][screen_pos_key] = str(
                    self.exp_settings_dict["audio"]["screen_position"][screen_pos_key]
                )
                changes += 1
        ...
        for device_num in range(self.exp_settings_dict["audio"]["total_device_num"]):
            for device_setting_key in self.exp_settings_dict["audio"]["devices"].keys():
                if device_setting_key != "usghflags" or device_num == 0:
                    if not math.isclose(
                        self.exp_settings_dict["audio"]["devices"][device_setting_key],
                        float(
                            self.config_1["Configuration"][
                                f"{device_setting_key}{device_num}"
                            ]
                        ),
                    ):
                        self.config_1["Configuration"][
                            ...
                            self.exp_settings_dict["audio"]["devices"][
                                device_setting_key
                            ]
                        )
                        changes += 1
        ...
        for mic_num in range(self.exp_settings_dict["audio"]["total_mic_number"]):
            for mic_spec_key in self.exp_settings_dict["audio"]["mics_config"].keys():
                if mic_spec_key in ["name", "deviceid", "id", "channel", "ditctime"]:
                    if (
                        mic_num in self.exp_settings_dict["audio"]["used_mics"]
                        and f"{mic_spec_key}{mic_num}"
                        not in self.config_1["Configuration"].keys()
                    ) or (
                        mic_num in self.exp_settings_dict["audio"]["used_mics"]
                        and str(
                            self.exp_settings_dict["audio"]["mics_config"][mic_spec_key]
                        )
                        != self.config_1["Configuration"][f"{mic_spec_key}{mic_num}"]
                    ):
                        if mic_spec_key == "name":
                            if (
                                self.config_1["Configuration"][
                                    f"{mic_spec_key}{mic_num}"
                                ]
                                != f"ch{mic_num + 1}"
                            ):
                                self.config_1["Configuration"][
                            ...
                            if mic_num < 12:
                                ...
                                changes += 1
                        elif self.config_1["Configuration"][f"{mic_spec_key}"] != str(
                            self.exp_settings_dict["audio"]["mics_config"][mic_spec_key]
                        ):
                            self.config_1["Configuration"][f"{mic_spec_key}"] = str(
                                self.exp_settings_dict["audio"]["mics_config"][
                                    mic_spec_key
                                ]
                            )
                            changes += 1
                    ...
                    self.config_1["Configuration"][f"{mic_spec_key}{mic_num}"] = str(
                        self.exp_settings_dict["audio"]["mics_config"][mic_spec_key]
                    )
                    changes += 1
        ...
        for mic_num in range(self.exp_settings_dict["audio"]["total_mic_number"]):
            for monitor_key in self.exp_settings_dict["audio"]["monitor"].keys():
                if (
                    mic_num in self.exp_settings_dict["audio"]["used_mics"]
                    and f"{monitor_key}{mic_num}" not in self.config_1["Monitor"].keys()
                ) or (
                    mic_num in self.exp_settings_dict["audio"]["used_mics"]
                    and not math.isclose(
                        self.exp_settings_dict["audio"]["monitor"][monitor_key],
                        float(self.config_1["Monitor"][f"{monitor_key}{mic_num}"]),
                    )
                ):
                    self.config_1["Monitor"][f"{monitor_key}{mic_num}"] = str(
                        self.exp_settings_dict["audio"]["monitor"][monitor_key]
                    )
                    changes += 1
        ...
        for mic_num in range(self.exp_settings_dict["audio"]["total_mic_number"]):
            for call_key in self.exp_settings_dict["audio"]["call"].keys():
                if (
                    mic_num in self.exp_settings_dict["audio"]["used_mics"]
                    and f"{call_key}{mic_num}" not in self.config_1["Call"].keys()
                ) or (
                    mic_num in self.exp_settings_dict["audio"]["used_mics"]
                    and not math.isclose(
                        self.exp_settings_dict["audio"]["call"][call_key],
                        float(self.config_1["Call"][f"{call_key}{mic_num}"]),
                    )
                ):
                    self.config_1["Call"][f"{call_key}{mic_num}"] = str(
                        self.exp_settings_dict["audio"]["call"][call_key]
                    )
                    changes += 1
    ...
    def conduct_behavioral_recording(self) -> None:
        ...
        Messenger(
            ...
            subject="Audio PC in 165B is busy, do NOT attempt to remote in!",
        ...
        if not os.path.isfile(
            ...
            shutil.copy(
                src=os.path.join(
                    os.path.dirname(os.path.abspath(__file__)),
                    "_config/coolterm_config.ini",
                ),
                dst=f"{self.exp_settings_dict['coolterm_basedirectory']}{os.sep}Connection_settings{os.sep}coolterm_config.stc",
            )
        subprocess.Popen(
            args=f'''cmd /c "start /MIN {self.exp_settings_dict["coolterm_basedirectory"]}{os.sep}Connection_settings{os.sep}coolterm_config.stc"''',
            stdout=subprocess.PIPE,
        )

        self.check_camera_vitals(
            camera_fr=self.exp_settings_dict["video"]["general"]["recording_frame_rate"]
        )
        ...
        start_hour_min_sec, total_dir_name_linux, total_dir_name_windows = (
            self.get_custom_dir_names(now=self.api.call("schedule")["now"])
        )

        # start recording audio
        if self.exp_settings_dict["conduct_audio_recording"]:
            self.message_output(
                f"Audio recording in progress since {start_hour_min_sec}, it will last {round(self.exp_settings_dict['video_session_duration'] + 0.36, 2)} minute(s). Please be patient."
            )

            avisoft_recorder_program_name = self.exp_settings_dict[
                "avisoft_recorder_program_name"
            ]
            ...
            if os.path.exists(
                f"{self.exp_settings_dict['avisoft_basedirectory']}{os.sep}Configurations{os.sep}RECORDER_USGH{os.sep}avisoft_config.ini"
            ):
                subprocess.Popen(
                    args=f'''cmd /c "start /{cpu_priority} /affinity {cpu_affinity_mask} {avisoft_recorder_program_name} /CFG=avisoft_config.ini /AUT"''',
                    stdout=subprocess.PIPE,
                ...
                cmd_result = subprocess.run(
                    args=f'''tasklist /FI "IMAGENAME eq {avisoft_recorder_program_name}"''',
                    capture_output=True,
                    text=True,
                    check=False,
                    shell=True,
                ...
                if (
                    ...
                    and avisoft_recorder_program_name.lower()
                    in cmd_result.stdout.lower()
                ):
                    # check if Avisoft Recorder is frozen and exit GUI if so
                    cmd_result_frozen = subprocess.run(
                        args=f"""tasklist /NH /FI "IMAGENAME eq {avisoft_recorder_program_name}" /FI "STATUS eq Not Responding""",
                        capture_output=True,
                        text=True,
                        check=False,
                        shell=True,
                    ...
                    if not (
                        cmd_result_frozen.returncode == 0
                        and avisoft_recorder_program_name.lower()
                        in cmd_result_frozen.stdout.lower()
                    ):
                        is_running = True
                    else:
                        is_frozen = True
                if not is_running:
                    subprocess.Popen(
                        '''cmd /c "taskkill /IM CoolTerm.exe /T /F 1>nul 2>&1"'''
                    ).wait()
                    if is_frozen:
                        subprocess.Popen(
                            f'''cmd /c "taskkill /IM {avisoft_recorder_program_name} /T /F 1>nul 2>&1"'''
                        ).wait()
                    print(
                        "Aborted experiment as Avisoft Recorder was not running or was frozen."
                    )
                    sys.exit(1)
        ...
        if len(self.exp_settings_dict["video"]["general"]["expected_cameras"]) == 1:
            self.message_output(
                f"You chose to conduct the recording with one camera only (camera serial num: {self.exp_settings_dict['video']['general']['expected_cameras'][0]})."
            )
            self.api.call(
                f"camera/{self.exp_settings_dict['video']['general']['expected_cameras'][0]}/recording/start",
        ...
        self.message_output(
            f"Video recording in progress since {datetime.datetime.now().hour:02d}:{datetime.datetime.now().minute:02d}.{datetime.datetime.now().second:02d}, it will last {round(self.exp_settings_dict['video_session_duration'], 2)} minute(s). Please be patient."
        )

        if self.exp_settings_dict["disable_ethernet"]:
            subprocess.Popen(
                args=f'''cmd /c netsh interface set interface "{self.exp_settings_dict["ethernet_network"]}" disable''',
                stdout=subprocess.DEVNULL,
                stderr=subprocess.STDOUT,
            ).wait()
            self.message_output(
                f"Ethernet DISCONNECTED at {datetime.datetime.now().hour:02d}:{datetime.datetime.now().minute:02d}.{datetime.datetime.now().second:02d}."
            )
        ...
        smart_wait(
            app_context_bool=self.app_context_bool,
            seconds=25 + (self.exp_settings_dict["video_session_duration"] * 60),
        )

        self.message_output(
            f"Recording fully completed at {datetime.datetime.now().hour:02d}:{datetime.datetime.now().minute:02d}.{datetime.datetime.now().second:02d}."
        )
        ...
        if self.exp_settings_dict["disable_ethernet"]:
            subprocess.Popen(
                args=f'''cmd /c netsh interface set interface "{self.exp_settings_dict["ethernet_network"]}" enable''',
                stdout=subprocess.DEVNULL,
                stderr=subprocess.STDOUT,
            ).wait()
            self.message_output(
                f"Ethernet RECONNECTED at {datetime.datetime.now().hour:02d}:{datetime.datetime.now().minute:02d}.{datetime.datetime.now().second:02d}."
            )
            smart_wait(app_context_bool=self.app_context_bool, seconds=20)

        pathlib.Path(f"{total_dir_name_windows[0]}{os.sep}video").mkdir(
            parents=True, exist_ok=True
        )
        pathlib.Path(f"{total_dir_name_windows[0]}{os.sep}sync").mkdir(
            parents=True, exist_ok=True
        )
        if self.exp_settings_dict["conduct_audio_recording"]:
            if self.exp_settings_dict["audio"]["general"]["total"] == 0:
                pathlib.Path(
                    f"{total_dir_name_windows[0]}{os.sep}audio{os.sep}original"
                ...
                pathlib.Path(
                    f"{total_dir_name_windows[0]}{os.sep}audio{os.sep}original_mc"
                ).mkdir(parents=True, exist_ok=True)

        self.message_output(
            f"Transferring audio/video files started at: {datetime.datetime.now().hour:02d}:{datetime.datetime.now().minute:02d}.{datetime.datetime.now().second:02d}"
        )
        ...
        if self.exp_settings_dict["conduct_audio_recording"]:
            ...
            if self.exp_settings_dict["audio"]["general"]["total"] == 0:
                for mic_idx in self.exp_settings_dict["audio"]["used_mics"]:
                    last_modified_audio_file = max(
                        glob.glob(
                            f"{self.exp_settings_dict['avisoft_basedirectory']}{os.sep}ch{mic_idx + 1}{os.sep}*.wav"
                        ),
                        key=os.path.getctime,
                    )
                    single_audio_copy_subp = subprocess.Popen(
                        args=f'''cmd /c move "{last_modified_audio_file.split(os.sep)[-1]}" "{total_dir_name_windows[0]}{os.sep}audio{os.sep}original{os.sep}ch{mic_idx + 1}_{last_modified_audio_file.split(os.sep)[-1]}"''',
                ...
                for mic_pos_idx, mic_idx in enumerate([0, 12]):
                    ...
                    for aud_file in audio_file_list:
                        multi_audio_copy_subp = subprocess.Popen(
                            args=f'''cmd /c move "{aud_file.split(os.sep)[-1]}" "{total_dir_name_windows[0]}{os.sep}audio{os.sep}original_mc{os.sep}{device_id[mic_pos_idx]}_{aud_file.split(os.sep)[-1]}"''',
                            cwd=f"{self.exp_settings_dict['avisoft_basedirectory']}{os.sep}ch{mic_idx + 1}",
                            stdout=subprocess.DEVNULL,
                            stderr=subprocess.STDOUT,
                            shell=False,
            ...
            while True:
                status_poll = [
                    query_subp.poll() for query_subp in audio_copy_subprocesses
                ]
        ...
        last_modified_sync_file = max(
            glob.glob(
                f"{self.exp_settings_dict['coolterm_basedirectory']}{os.sep}Data{os.sep}*.txt"
            ),
            key=os.path.getctime,
        )
        shutil.move(
            src=last_modified_sync_file,
        ...
        if len(total_dir_name_windows) > 1:
            for win_dir_idx, win_dir in enumerate(total_dir_name_windows[1:]):
                subprocess.Popen(
                    args=f'''cmd /c robocopy "{total_dir_name_windows[0]}" "{win_dir}" /MIR''',
        ...
        self.message_output(
            f"Transferring audio/video files finished at: {datetime.datetime.now().hour:02d}:{datetime.datetime.now().minute:02d}.{datetime.datetime.now().second:02d}"
        )
        ...
        Messenger(
            ...
            subject="Audio PC in 165B is available again, recording has been completed.",
...
@click.option(
    ...
    "overrides",
    multiple=True,
    help="Override a setting, e.g., --set calibration_duration=10",
)
def conduct_calibration_cli(overrides):
    ...
    with open(
        pathlib.Path(__file__).parent / "_config/behavioral_experiments_settings.toml"
    ) as f:
        exp_settings_dict = toml.load(f)

    if len(overrides) > 0:
        exp_settings_dict = override_toml_values(
            overrides=overrides, exp_settings_dict=exp_settings_dict
        )

    ExperimentController(
        exp_settings_dict=exp_settings_dict
    ).conduct_tracking_calibration()
...
@click.option(
    ...
    "overrides",
    multiple=True,
    help='Override a setting, e.g., --set video.metadata.notes="Test run"',
)
