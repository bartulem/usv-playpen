...
class PrepareClusterJob:
    def __init__(
        ...
        if root_directory is None:
            with open(
                ...
                self.root_directory = json.load(json_file)["prepare_cluster_job"][
                    "root_directory"
                ]
        else:
    ...
    def video_list_to_txt(self) -> None:
        ...
        if platform.system() == "Windows":
            spock_converted_first_model_path = (
                self.input_parameter_dict["centroid_model_path"]
                .replace("\\", "/")
                .replace("F:", "/mnt/cup/labs/falkner")
            )
            if self.input_parameter_dict["centered_instance_model_path"] == "":
                spock_converted_second_model_path = ""
            else:
                spock_converted_second_model_path = (
                    self.input_parameter_dict["centered_instance_model_path"]
                    .replace("\\", "/")
                    .replace("F:", "/mnt/cup/labs/falkner")
                )
        elif platform.system() == "Linux":
            spock_converted_first_model_path = self.input_parameter_dict[
                "centroid_model_path"
            ].replace("mnt", "mnt/cup/labs")
            if self.input_parameter_dict["centered_instance_model_path"] == "":
                spock_converted_second_model_path = ""
            else:
                spock_converted_second_model_path = self.input_parameter_dict[
                    "centered_instance_model_path"
                ].replace("mnt", "mnt/cup/labs")
        else:
            spock_converted_first_model_path = self.input_parameter_dict[
                "centroid_model_path"
            ].replace("Volumes", "mnt/cup/labs")
            if self.input_parameter_dict["centered_instance_model_path"] == "":
                spock_converted_second_model_path = ""
            else:
                spock_converted_second_model_path = self.input_parameter_dict[
        ...
        with open(
            f"{self.input_parameter_dict['inference_root_dir']}{os.sep}job_list.txt",
            mode="w",
        ) as job_list_file:
            for root_dir in self.root_directory:
                if platform.system() == "Windows":
                    spock_converted_root_dir = root_dir.replace("\\", "/").replace(
                        "F:", "/mnt/cup/labs/falkner"
                    )
                elif platform.system() == "Linux":
                    spock_converted_root_dir = root_dir.replace("mnt", "mnt/cup/labs")
                else:
                    spock_converted_root_dir = root_dir.replace(
                        "Volumes", "mnt/cup/labs"
                    )

                for video_processed_dir in os.listdir(f"{root_dir}{os.sep}video"):
                    if video_processed_dir.isnumeric():
                        for video_dir in os.listdir(
                            f"{root_dir}{os.sep}video{os.sep}{video_processed_dir}"
                        ):
                            if (
                                os.path.isdir(
                                    f"{root_dir}{os.sep}video{os.sep}{video_processed_dir}{os.sep}{video_dir}"
                                )
                                and video_dir
                                in self.input_parameter_dict["camera_names"]
                            ):
                                for video_dir_item in os.listdir(
                                    f"{root_dir}{os.sep}video{os.sep}{video_processed_dir}{os.sep}{video_dir}"
                                ):
                                    if video_dir_item.endswith(".mp4"):
                                        ...
                                        if spock_converted_second_model_path == "":
                                            job_list_file.write(
                                                f"{spock_converted_first_model_path} {video_path} {slp_result_path}\n"
                                            )
                                        else:
                                            job_list_file.write(
                                                f"{spock_converted_first_model_path} {spock_converted_second_model_path} {video_path} {slp_result_path}\n"
                                            )
