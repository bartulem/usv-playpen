...
def _write_embedding_tree(tmp_path, rng, n_points: int = 60,
                          n_categories: int = 3, subdir: str = "Lab/Data",
                          session_key: str = "20230615_ABC_session_1",
                          n_label_zero: int = 6,
                          include_mmap: bool = False,
                          sample_rate: int = 250000,
                          mmap_seconds: float = 0.05) -> tuple:
    """
    Build a self-contained synthetic discovery tree for
    ``plot_vocalization_embedding_space`` under ``tmp_path``.

    The function discovers data by, for each ``session_key`` in the UMAP
    HDF5 file, taking ``search_term = session_key[:15]``, globbing
    ``{find_base_path()}/{subdir}/*{search_term}`` for a *directory*, then
    looking for a ``**/*_usv_summary.csv`` inside it (so tests monkeypatch
    ``find_base_path`` to point at ``tmp_path``). This helper therefore writes:

    * a UMAP ``.h5`` keyed by ``session_key`` -> ``(n_points, 2)`` float
      coordinates,
    * a cluster-category ``.h5`` keyed by ``session_key`` -> ``(n_points,)``
      integer labels (the first ``n_label_zero`` entries set to ``0`` so the
      ``valid_mask = labels != 0`` noise-rejection branch is exercised),
    * a session directory at
      ``{tmp_path}/{subdir}/{session_key[:15]}_recording/`` whose name ends
      with ``search_term`` so the directory glob matches, and
    * a ``*_usv_summary.csv`` (written with polars) inside that directory
      with exactly ``n_points`` rows and the ``usv_supercategory`` / ``start``
      / ``stop`` columns the function reads.

    When ``include_mmap`` is True a tiny 24-channel ``int16`` ``.mmap`` audio
    file is also written next to the CSV, named so the function's
    ``*_24_int16.mmap`` glob matches and its ``_<rate>_<n_samples>_24_int16``
    suffix parses into a valid sampling-rate / sample-count / channel-count
    triple. This makes the regional spectrogram-inset path reachable.

    Parameters
    ----------
    tmp_path : pathlib.Path
        Pytest temporary directory (everything is written beneath it).
    rng : numpy.random.Generator
        Seeded generator for reproducible synthetic coordinates / labels.
    n_points : int, default 60
        Number of USV rows (coords == labels == CSV length).
    n_categories : int, default 3
        Number of distinct supercategory values among the valid points (so
        the boundary-contour math has multiple territories).
    subdir : str, default "Lab/Data"
        Discovery subdirectory (relative to the monkeypatched
        ``find_base_path()`` root) to create and pass back as the single
        ``target_subdirs`` entry.
    session_key : str, default "20230615_ABC_session_1"
        HDF5 session key; its first 15 characters drive directory discovery.
    n_label_zero : int, default 6
        Number of leading label-``0`` (noise) rows, dropped by the function's
        ``valid_mask``.
    include_mmap : bool, default False
        When True, also write a minimal 24-channel ``int16`` ``.mmap`` audio
        file so the regional-inset path can load audio.
    sample_rate : int, default 250000
        Sampling rate encoded into the ``.mmap`` filename (and used to size
        the start/stop sample windows in the CSV) when ``include_mmap``.
    mmap_seconds : float, default 0.05
        Duration (seconds) of the synthetic ``.mmap`` recording; the per-USV
        ``start``/``stop`` windows are placed inside this span.

    Returns
    -------
    tuple of (str, str, str)
        ``(umap_h5_path, category_h5_path, subdir)`` as strings, ready to
        feed straight into ``plot_vocalization_embedding_space``.
    """

    search_term = session_key[:15]
    session_dir = tmp_path / subdir / f"recording_{search_term}"
    session_dir.mkdir(parents=True, exist_ok=True)

    coords = rng.uniform(-10.0, 10.0, size=(n_points, 2)).astype(np.float64)
    labels = rng.integers(1, n_categories + 1, size=n_points).astype(np.int64)
    labels[:n_label_zero] = 0

    n_samples = int(round(sample_rate * mmap_seconds))
    # Place every USV window well inside the recording span so
    # start_sample/stop_sample stay valid; widen enough that the
    # specgram (NFFT=1024) always has > NFFT samples to work with.
    win_samples = max(2048, n_samples // 4)
    start_sample = (n_samples - win_samples) // 2
    starts = np.full(n_points, start_sample / sample_rate, dtype=np.float64)
    stops = np.full(n_points, (start_sample + win_samples) / sample_rate, dtype=np.float64)
    supercats = rng.integers(1, n_categories + 1, size=n_points).astype(np.int64)

    csv_df = pls.DataFrame({
        'usv_supercategory': supercats,
        'start': starts,
        'stop': stops,
    })
    csv_df.write_csv(session_dir / f"{search_term}_usv_summary.csv")

    umap_h5_path = tmp_path / "umap_positions.h5"
    with h5py.File(umap_h5_path, mode='w') as fh:
        fh.create_dataset(session_key, data=coords)

    category_h5_path = tmp_path / "cluster_categories.h5"
    with h5py.File(category_h5_path, mode='w') as fh:
        fh.create_dataset(session_key, data=labels)

    if include_mmap:
        audio = rng.integers(-2000, 2000, size=(n_samples, 24)).astype(np.int16)
        mmap_name = f"{search_term}_{sample_rate}_{n_samples}_24_int16.mmap"
        mmap_arr = np.memmap(filename=str(session_dir / mmap_name), dtype=np.int16,
                             mode='w+', shape=(n_samples, 24), order='C')
        mmap_arr[:] = audio
        mmap_arr.flush()
        del mmap_arr

    return str(umap_h5_path), str(category_h5_path), subdir


@pytest.mark.filterwarnings("ignore:FigureCanvasAgg is non-interactive:UserWarning")
class TestPlotVocalizationEmbeddingSpace:
    """Figure-emission tests for ``plot_vocalization_embedding_space``."""

    @pytest.mark.filterwarnings("ignore:Tight layout:UserWarning")
    def test_global_manifold_writes_svg(self, tmp_path, monkeypatch):
        """With no ``x_range``/``y_range`` the global-manifold path runs the
        boundary math, scatters the categories, and writes
        ``global_vocalization_embedding_space.svg`` into ``output_dir``."""

        rng = np.random.default_rng(101)
        umap_h5, cat_h5, subdir = _write_embedding_tree(tmp_path, rng)
        # The plotter resolves its discovery root from the host config via
        # find_base_path(); point it at the synthetic tree under tmp_path.
        monkeypatch.setattr(
            "usv_playpen.visualizations.modeling_plots.find_base_path",
            lambda: str(tmp_path))
        out_dir = tmp_path / "embed_out"
        out_dir.mkdir()
        result = plot_vocalization_embedding_space(
            umap_position_file_path=umap_h5,
            cluster_category_file_path=cat_h5,
            target_subdirs=[subdir],
            save_plot=True,
            output_dir=str(out_dir),
        )
        assert result is None
        assert (out_dir / "global_vocalization_embedding_space.svg").is_file()

    def test_no_valid_coordinates_early_return(self, tmp_path, monkeypatch):
        """When the UMAP coords and the category labels disagree in length
        (so no session aligns), the function takes the documented
        ``No valid coordinates`` early-return and writes nothing."""

        rng = np.random.default_rng(102)
        session_key = "20230615_ABC_session_1"
        search_term = session_key[:15]
        subdir = "Lab/Data"
        monkeypatch.setattr(
            "usv_playpen.visualizations.modeling_plots.find_base_path",
        ...
        # CSV has a self-consistent length, but the coords HDF5 is written
        # with a *different* length than the labels HDF5 so the
        # ``len(session_coords) == len(session_labels)`` guard fails and the
        # session is skipped, leaving ``all_coordinates`` empty.
        pls.DataFrame({
            'usv_supercategory': np.ones(10, dtype=np.int64),
            'start': np.zeros(10, dtype=np.float64),
            'stop': np.ones(10, dtype=np.float64),
        }).write_csv(session_dir / f"{search_term}_usv_summary.csv")

        umap_h5_path = tmp_path / "umap_positions.h5"
        with h5py.File(umap_h5_path, mode='w') as fh:
            fh.create_dataset(session_key, data=rng.uniform(size=(10, 2)))
        cat_h5_path = tmp_path / "cluster_categories.h5"
        with h5py.File(cat_h5_path, mode='w') as fh:
            fh.create_dataset(session_key, data=np.ones(8, dtype=np.int64))

        out_dir = tmp_path / "embed_empty_out"
        out_dir.mkdir()
        result = plot_vocalization_embedding_space(
            umap_position_file_path=str(umap_h5_path),
            cluster_category_file_path=str(cat_h5_path),
            target_subdirs=[subdir],
            save_plot=True,
            output_dir=str(out_dir),
        )
    ...
    @pytest.mark.filterwarnings("ignore:Tight layout:UserWarning")
    @pytest.mark.filterwarnings(
        "ignore:This figure includes Axes that are not compatible"
        ":UserWarning")
    @pytest.mark.filterwarnings("ignore:Mean of empty slice:RuntimeWarning")
    @pytest.mark.filterwarnings(
        "ignore:invalid value encountered:RuntimeWarning")
    @pytest.mark.filterwarnings("ignore:divide by zero:RuntimeWarning")
    def test_regional_inset_writes_spectrogram_svg(self, tmp_path, monkeypatch):
        """With both ``x_range`` and ``y_range`` set to span the whole
        manifold (so candidate USVs fall in the box) and a minimal
        24-channel ``.mmap`` present, the dual-panel regional path samples
        the grid, builds spectrograms, and writes the inset SVG."""

        rng = np.random.default_rng(103)
        umap_h5, cat_h5, subdir = _write_embedding_tree(
            tmp_path, rng, include_mmap=True)
        monkeypatch.setattr(
            "usv_playpen.visualizations.modeling_plots.find_base_path",
            lambda: str(tmp_path))
        out_dir = tmp_path / "embed_regional_out"
        out_dir.mkdir()
        result = plot_vocalization_embedding_space(
            umap_position_file_path=umap_h5,
            cluster_category_file_path=cat_h5,
        ...
        assert len(list(out_dir.glob(
            "vocalization_embedding_spectrograms_inset_*.svg"))) == 1
